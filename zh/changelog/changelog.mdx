...
<Update label="2026-08-31" description="☁️ 新增华为云 AOM 告警集成">

### 华为云 AOM 告警集成

告警接入新增 **华为云 AOM** 集成：

- 在集成中心选择「华为云 AOM」即可生成专属 webhook 推送地址，AOM 告警经 SMN 主题 HTTPS 订阅推送到 Flashduty，自动完成降噪与路由
- 告警级别映射支持在集成的自定义级别映射（customSeverityMapping）中按需调整

详见 [华为云 AOM 告警集成](/zh/on-call/integration/alert-integration/alert-sources/huawei-aom)。
