...
<Update label="2026-09-11" description="💳 AI SRE 计费上线、邀请链接与二维码、告警规则的 AI 查询助手">

### AI SRE 计费上线

AI SRE 由公测免费转为正式计费，**费用中心 → 订阅 → AI SRE** 页签完成开通与计费管理：

- **开通不收费**：开通后可先用体验赠送积分；体验模式下用量照常记录但不计费，随时可「转为正式计费」，由你决定何时开始扣费
- **抵扣顺序**：体验赠送 → 最早到期的资源包 → 按量付费
- **资源包三档**：1,000 / 5,000 / 20,000 credits，售价 ¥950 / ¥4,500 / ¥17,000，自支付成功起 365 天有效、不自动续费，可随时加购
- **按量付费**：1 credit = ¥1，从账户通用钱包结算；月度上限可选不限额、设置月度上限或禁止按量付费（资源包用完后停止运行），达到上限后停止新的运行，已开始的最后一次运行可以完成
- **额度提醒**：正式计费账户本周期可用额度（体验赠送 + 资源包 + 按量上限合计）降至总容量 20% 时，向账户与已启用的管理员发送短信与邮件提醒，每个周期同一额度只提醒一次

详见 [AI SRE 计费](/zh/ai-sre/billing) 与 [AI SRE 资源包购买协议](/zh/compliance/ai-sre-credit-package-purchase-agreement)。
