...
<Update label="2026-09-10" description="💳 AI SRE 计费（credits）、行动项自动转跟进项、告警列表与详情筛选更新">

### AI SRE 计费（credits）

AI SRE 自 **2026-09-16（北京时间）** 起正式开始收费，此前的使用不收取费用；按 credits 计量**模型用量、沙箱在线时长与联网检索**（沙箱每在线小时 2 credits，联网检索每次 0.25 credits，模型按档位费率计价，当前档位为每百万 token 输入 / 输出 / 缓存命中 8 / 32 / 0.16 credits）：

- **控制台入口**：**费用中心 → AI SRE** 展示本周期额度与按量消费概况，任意成员均可查看；**仅 owner/admin 可修改上限**，保存按钮按该权限显隐
- **按量付费三种状态**：`不限额`、`设置月度上限`（填写大于 0 的金额）、`禁止按量付费（资源包用完后停止运行）`；达到上限后停止新的运行，已开始的最后一次运行可以完成
- **额度不足短信提醒**：本周期可用额度（含即将到期的资源包）降到总容量的 **20%** 时，向账户绑定的手机号发送一次提醒；同一计费周期内最多一次，周期重置后重新可提醒
- **未开通账户照常计量、不计费**：未开通 AI SRE 的账户照常记账并计量用量，费用由平台承担，不产生扣费

详见 [AI SRE 计费](/zh/ai-sre/credits)。

### 行动项自动转跟进项

关闭故障时，未完成的行动项不再被丢下，而是就地转为跟进项：

- **就地转换**：故障下所有未完成的**行动项**在**原工作项上**转为**跟进项**（翻转类型、记录转换人与转换时间），已存在复盘时自动绑定到该复盘
- **时间线批量记录**：写入一条批量记录「将 N 个未完成行动项转为跟进项」（记录内附各行动项标题），IM 通知文案为「故障已关闭，N 个未完成行动项已转为跟进项，可在故障详情继续跟踪。」
- **可编辑卡片随关闭切换**：故障处理中「行动项」卡片可编辑、「跟进项」卡片只读；故障关闭后反转——「跟进项」卡片成为可编辑的那一张
- **幂等**：重复投递的关闭事件在找不到未完成行动项时不写入任何变更
- **故障列表筛选**：「更多筛选」新增 **未完成的行动项/跟进项**，选「有」即筛出仍挂着未完成行动项或跟进项的故障

详见 [行动项与跟进项](/zh/on-call/incident/work-items)。

### 告警列表与告警详情

- **新增告警标识筛选**：告警列表筛选条件新增 **告警标识**（`alert_key`），多值自由输入（回车分隔），后端按完整值精确匹配、多值之间为「或」关系
- **告警详情新增告警标识区块**：整宽展示、等宽字体并支持一键复制（最长 255 字符）；没有 `alert_key` 时不显示
- **新增降噪记录面板**：列出该告警命中的 **静默策略** 与 **抑制策略**，可直接跳转策略详情；仅在告警出现过静默/抑制时展示，加载失败可重试，无记录时提示「暂无可用的策略记录」
- **时间条件改为预设**：提供 **最近 1 天 / 最近 7 天 / 最近 30 天**（默认 **最近 7 天**），窗口以相对时间下推，列表自动刷新时随当前时间滑动
- **告警 ID 校验**：非法 ID 在录入处即被拒绝，并提示「ID 格式错误，请检查」与该值

详见 [告警管理](/zh/on-call/incident/alert-management)。

### 查询工作台：AI 生成查询与数据源查询工具

- **AI 帮我写**：查询编辑器的查询输入框右上角新增 AI 入口，打开查询助手面板后按当前数据源、查询类型与时间范围生成查询语句并回填编辑器（回填时同步换算执行区间与数据源私有参数）
- **10 个统一查询工具**：`prometheus.query`、`mysql.query`、`postgres.query`、`oracle.query`、`clickhouse.query`、`elasticsearch.query`、`loki.query`、`victorialogs.query`、`sls.query`、`tencent_cls.query`，与诊断工具共用数据源工具调用入口
- **执行语义**：Prometheus 与 Loki 支持 `instant` / `range`（Prometheus 的 `instant` 只接受 `to_ms`）；SQL 类、SLS 与 CLS 只接受 `window`；VictoriaLogs 必须带 `from_ms`，`limit` 与 `direction`（`latest` / `earliest`）仅在 `window` 下生效；SLS 必填 `project` + `logstore`（可选 `powersql`），腾讯云 CLS 必填 `region` + `topic_id` + `syntax`（`cql` / `lucene`）；显式 `null` 字段被拒绝，未知附加字段容忍

详见 [查询工作台](/zh/monitors/explore/explore)。

### CLI 与 SDK 覆盖面

- **CLI 覆盖刷新**：OpenAPI 现有 **337 个操作**，CLI 为其中 **334 个**生成命令，其余以手写命令提供（如 `session export`）
- **新增 RUM 远程配置命令族**：`flashduty rum application-remote-config-get` / `application-remote-config-history-list` / `application-remote-config-preview` / `application-remote-config-update` / `application-remote-config-history-revert`
- **Go SDK**：新增数据源 `query` 工具的请求参数类型（`PrometheusQueryParams`、`MySQLQueryParams`、`SLSQueryParams`、`TencentCLSQueryParams` 等，可选字段为指针以区分「不传」与「显式传值」，并提供 `NewDatasourceQueryInvokeRequest` 构造入口）与 AlertRules V2 方法（`ReadInfoV2` / `WriteCreateV2` / `WriteUpdateV2`）

详见 [命令行工具（CLI）](/zh/developer/cli)。

</Update>

<Update label="2026-09-08" description="📋 静默策略复制、批量操作表单优化与监控对象页 ServiceMap 下线">
...
### 监控对象页 ServiceMap 下线与 monit-agent 主机诊断退役

- **监控对象页**：移除 ServiceMap 相关列（拓扑状态/采集模式/观测时间）与拓扑浮层；监控对象列表的列选择、每行 AI 分析等能力保留
- **monit-agent 主机诊断退役**：`fduty monit-agent` 命令树与 `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap 操作随 host-tool API 一并下架；主机诊断与数据库、中间件诊断统一经 `monit-query <datasource-id> --tool '<name>' --params ...` 发起，`monit datasource-tools-invoke` 是同一接口的 spec 镜像入口（`monit-query diagnose` 与 `monit-query data` 均已不存在）
