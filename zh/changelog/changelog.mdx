...
<Update label="2026-08-27" description="🔐 SSO 设置页重构与 Explore 工作台增强">

### SSO 设置页重构

单点登录设置页对齐系统设置页风格，配置项直接落在页面上（不再使用抽屉），按连接配置、字段映射、LDAP 同步、登录策略分区展示：

- 表单常开：页面有改动时才浮出保存条，全页只有一个「保存」和一个「取消」，「取消」可真正复位到服务端已保存的配置
- 登录策略（登录即创建账号、成员仅支持 SSO 登录、禁用 SSO 成员编辑功能）并入主表单，随「保存」统一生效，不再是切换即生效
- 字段映射新增**稳定用户 ID 字段**（可选）：配置后，成员邮箱或手机号变更仍可识别为同一成员；留空时继续按下方邮箱等字段匹配

### Explore 工作台增强

监控 Explore 工作台多项体验更新：

- **分享链接**：工具栏「分享」按钮一键复制当前查询链接，链接中的时间固定为本次查询实际运行的时间段，接收者打开看到的是同一段时间的结果
- **图例筛选**：时序图例支持按标签维度筛选曲线（形态参照 Grafana），同一标签键内为「或」，不同标签键之间为「与」，某键一个未勾则该维度不参与过滤
- **日志时间分布**：日志查询新增时间分布（Log Volume）图，展示整个查询时间范围内的日志量分布（不止列表中的 Top-N 条），支持折叠与显隐开关
