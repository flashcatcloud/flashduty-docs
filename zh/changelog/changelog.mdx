...
<Update label="2026-09-09" description="💳 AI SRE 开始计费、作战室集成失效后可解除关联与 AI SRE 排队消息合并">

### AI SRE 开始计费

- **计费开始时间**：AI SRE 自 **2026-09-16（北京时间）** 起正式开始收费；此前的使用不收取费用
- **计量方式**：按 credits 计量模型用量、沙箱在线时长与联网检索
- **额度与资源包**：额度与资源包详情将另行通知；购买前请阅读 [AI SRE credits 资源包购买协议](/zh/compliance/ai-sre-credit-package-purchase-agreement)

### 作战室集成失效后可解除关联

- **集成失效仍可见**：作战室所属 IM 集成被禁用或删除后，故障详情页的作战室条目继续展示，并显示已保存的群聊名称
- **管理员解除关联**：账号拥有者或管理员可在 PC 端故障详情页执行「解除关联」；**原 IM 群聊不会解散**
- **操作受限**：解除关联之前，加人、解散等操作不再提供
- **操作记录**：解除关联会写入故障时间线，展示为「发起解除作战室关联」

详见 [作战室](/zh/on-call/advanced/war-room)。

### AI SRE 排队消息合并

- **排队**：当前回合执行期间发送的消息进入队列，不再需要等待输入框可用
- **连续消息合并**：同一个人在队列中连续发送的多条纯文本消息会合并为一个回合交给模型处理；历史记录仍按条展示每条消息
- **↑ 召回修改**：输入框为空时按 ↑ 可将最近一条排队消息取回输入框修改（取回即从服务端队列中撤回该消息）
- **交互简化**：排队消息不再提供「立即发送」「取消」等气泡按钮，↑ 是唯一的召回入口
...
- **monit-agent 主机诊断退役**：`fduty monit-agent` 命令树与 `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap 操作随 host-tool API 一并下架；诊断统一以 `monit datasource-tools-invoke` 命名工具为准
