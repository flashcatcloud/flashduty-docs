...
- **monit-agent 主机诊断退役**：`fduty monit-agent` 命令树与 `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap 操作随 host-tool API 一并下架；主机诊断以 `monit-query diagnose`、数据库与中间件诊断以 `monit datasource-tools-invoke` 为准
