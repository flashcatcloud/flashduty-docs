...
| `--limit` | 最多拉取的会话数 | `200` |
| `--page` | 起始页码（1 开始） | `1` |
| `--output-format` | 输出格式：`jsonl`（默认，每行一个会话对象，可直接管道给 `jq`）、`json`（完整信封，**不**受 16 KiB 列表输出上限约束；需要更少的行请自行用 `--limit` / `--since` 收窄，见「输出格式」）、`toon`（紧凑编码） | `jsonl` |
...
除上述精选命令外，CLI 现已通过 spec 驱动的代码生成实现对 Flashduty OpenAPI 的**全量覆盖**。当前 OpenAPI 含 **337 个 API 操作**，CLI 为其中 **334 个** 生成对应命令，其余 3 个——流式导出的 `session-read-export` 与 multipart 表单上传的 `mapping-data-write-upload`、`skill-write-upload`——以手工实现命令提供（`session export`、`enrichment mapping-data-upload`、`safari skill-upload`），并按资源组织为顶层命令组。除 On-call 域（incident、incident-trigger-subscription、change、channel、field、status-page、template 等）外，还覆盖了：
...
由 OpenAPI 代码生成提供覆盖的生成命令（如 `safari a2a-agent-list`、`alert event-list <alert-id>`、`insight incident-list` 等），其 `json` / `toon` 输出对**列表形态**的响应按 16 KiB 上限处理。缩减分三级；当响应是**列表信封**形态时，CLI 还会在 payload 中补写一个机读标记：
...
**列表信封会被重建并写入缩减标记**。列表信封指行数组名为 `items` / `docs` / `list`、同级只有 `total`、`has_next_page`、`search_after_ctx` 等标量字段的响应（如 `safari a2a-agent-list`、`alert event-list <alert-id>`、`insight incident-list`）；一旦发生缩减，CLI 会重建该信封并在 payload 中补写字段，脚本无需解析 stderr 也能发现缩减：
...
**顶层数组形态**的响应（如生成命令 `monit datasource-list`，或精选命令 `incident list`、`insight incidents`）没有信封可承载标记：生成命令的列表形态输出同样可能是顶层数组，它同样按 16 KiB 上限缩减，但缩减只写在 stderr 上，脚本必须读 stderr 才能发现。

行数组不叫 `items` / `docs` / `list`（或同级存在非标量字段）的列表响应不会被识别为列表信封——生成命令 `safari session-list` 与精选命令 `session list` 都把行放在 `sessions[]` 下，同级还有 `suggest_init`、`total`。这类响应既不缩减、也不会补写 `truncated` / `emitted_rows`，完整输出原样打印。
