...
直接探测监控后端数据源，无需经过告警规则层。`data` 子命令支持 9 种数据源类型（Prometheus、VictoriaLogs、Loki、MySQL、SLS、Elasticsearch、PostgreSQL、Oracle、ClickHouse）；`diagnose` 与已弃用的 `rows` 支持 `prometheus`、`victorialogs`、`loki`、`mysql`。
