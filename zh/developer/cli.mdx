...
`member list` 支持 `--query`（姓名/邮箱关键字搜索）、`--member-id`（按成员 ID 精确过滤，只返回该成员）、`--role-id`、`--page`、`--limit`、`--orderby`、`--asc`。
...
远程配置按应用在线下发采集与隐私参数（采样率、Session Replay 隐私级别、自定义配置项等），无需改代码或重新发版。五条命令覆盖读取、发布、回滚与预演；配置项清单与生效语义见 [应用管理 · 远程配置](/zh/rum/quickstart/app-management#远程配置)。

```bash
flashduty rum application-remote-config-get <application-id>              # 查看当前生效配置及其版本
flashduty rum application-remote-config-history-list <application-id>    # 列出版本历史
flashduty rum application-remote-config-preview <application-id> [flags] # 用草稿配置模拟一次下发（不发布）
flashduty rum application-remote-config-update <application-id> [flags]  # 发布一版完整配置
flashduty rum application-remote-config-history-revert <application-id> --version <v>  # 把历史版本内容重新发布为新版本
```

- `get` 返回 `config`（完整配置对象）、`version`（当前生效版本，`0` 表示从未配置）与 `updated_at`。
- `history-list` 返回 `{items, has_next_page, total}` 分页信封，`items` 元素含 `version`、`content_hash`、`updated_by_name`、`reason`、`updated_at`；分页用 `--page`（0 开始）、`--limit`（默认 20，上限 100），排序用 `--orderby`（`updated_at`/`version`）与 `--asc`。
- `preview` 用 `--env`、`--app-version`、`--sdk` 描述模拟客户端（`--sdk` 形如 `web@2.4.1`），草稿 `config` 经 `--data` 传入；返回命中的规则下标 `hit_rule_index`（`-1` 表示只命中默认值）与实际下发值 `values`；只做评估，不发布、不改变线上配置。
- `update` 的 `config` 必填（经 `--data` 传入），发布一版完整配置；`history-revert` 的 `--version` 必填（最小 1）。
- `update` 与 `history-revert` 都接受 `--reason`（发布者备注，≤255 字符），返回新版本号。
...
| `--account-id` | 可选一致性校验，必须等于当前认证账户；省略时不发送该字段 |

查询工具的参数按数据源类型各自定义，但都以 `expr` 加 `execution` 为骨架：

- `execution.kind` 取 `instant`（即时求值）、`range`（区间求值）或 `window`（窗口执行）；`execution.from_ms` / `execution.to_ms` 为 Unix **毫秒**时间戳，`range` 还需 `max_data_points`。
- 类型附加项：日志类数据源可另传 `limit`、`direction`（仅原始日志读取时生效）；`sls` 必填 `project`、`logstore`；`tencent_cls` 必填 `region`、`topic_id`、`syntax`；SQL 类（`mysql`、`postgres`、`oracle`、`clickhouse`）传单条只读语句并以 `window` 执行。
- CLI 只负责组装请求与渲染结果，工具名与参数合法性由服务端校验；请按各数据源类型的实际 schema 填写，不要从命令行列表猜测参数。

查询工具要求所选集群的 Edge 支持 Explore 协议 v0.68.0；版本不满足时原样返回 `edge_upgrade_required`、`mixed_edge_versions` 或 `edge_version_unknown`，CLI 不会自动轮换 Edge 或回退到其他端点。

查询工具返回完整的 `explore_result.v1` 结构化结果：`format` 固定为 `explore_result.v1`，`result.kind` 为 `samples`（带完整标签集的即时样本）、`frames`（类型化表格/时序帧）或 `logs`（原始日志，含 `applied_limit` 与 `has_more`）三者之一；查询工具不会合成 `summary` 与 `truncated`（诊断工具可能有）。诊断工具的返回结构见下文 `monit datasource-tools-invoke`。
...
除上述精选命令外，CLI 现已通过 spec 驱动的代码生成实现对 Flashduty OpenAPI 的**全量覆盖**。CLI 当前锁定的 OpenAPI 规范（go-flashduty v0.15.2）含 **337 个 API 操作**，分布在 **37 个 tag** 下；生成器为其中 **334 个** operationId 生成了命令，组织为 **37 个** 生成命令组，其余操作（如 `session-read-export`）以手工实现命令提供（`session export` / `safari session-export`），并按资源组织为顶层命令组。除 On-call 域（incident、incident-trigger-subscription、change、channel、field、status-page、template 等）外，还覆盖了：
