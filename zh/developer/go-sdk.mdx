...
sidebarTitle: Go SDK
description: "go-flashduty 是 Flashduty 官方开源的 Go SDK，与 Open API 严格 1:1 的类型化封装，当前覆盖全部 335 个 API 操作、38 个服务。"
...
SDK 当前覆盖 **335 个 API 操作**、**38 个服务**，全部由 Flashduty OpenAPI 规范生成，经单元测试覆盖，并针对线上 API 做过端到端验证。
...
少数接口不是 JSON 请求-响应：文件上传走 `multipart/form-data`，文件下载（如 CSV 导出）返回原始文件流。SDK 对这两类接口提供了对应的支持。

### 文件上传

multipart 上传接口无法由生成代码的 JSON 路径承载，SDK 以手写方法提供，文件内容以 `io.Reader` 流式传输，不会整段缓冲进内存：

| 方法 | 接口 | 说明 |
|------|------|------|
| `AlertEnrichment.MappingDataWriteUpload` | `POST /enrichment/mapping/data/upload` | 通过 CSV 批量导入标签映射数据 |
| `Skills.WriteUpload` | `POST /safari/skill/upload` | 上传 Skill 归档（`.skill` / `.zip` / `.tar.gz` / `.tgz`）创建或替换 Skill |

`MappingDataUploadRequest` 字段：

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `SchemaID` | string | - | 目标映射表的 ID（必填） |
| `DoNotTruncateFirst` | bool | `false` | 默认先清空表内既有数据再导入；置 `true` 则追加 |
| `File` | io.Reader | - | CSV 内容（必填，最大 100 MB；表头必须包含映射表的全部源/结果标签名） |
| `Filename` | string | `"mapping.csv"` | multipart 文件名 |

`SkillUploadRequest` 字段：

| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `File` | io.Reader | - | Skill 归档内容（必填，最大 100 MB） |
| `Filename` | string | `"skill.zip"` | multipart 文件名 |
| `TeamID` | int64 | `0` | Skill 归属团队；`0` 表示账户级。通过 `SkillID` 替换指定 Skill 时忽略 |
| `Replace` | bool | `false` | 名称冲突时覆盖既有 Skill；提供 `SkillID` 时必须为 `true` |
| `SkillID` | string | - | 指定要替换的 Skill ID（需配合 `Replace`） |
...
CSV 导出等接口返回的不是 JSON 信封，而是原始文件流。这类方法返回 `(*Response, error)`，文件内容通过 `Response.Raw`（`[]byte`）读取：
...
// resp.Raw 即为 CSV 文件内容
os.WriteFile("mapping-export.csv", resp.Raw, 0o644)
```

<Note>
`v0.14.3` 曾错误地下架了二进制下载方法并生成了无参数的上传方法，该版本已被 retract——请使用 `v0.14.4` 或更高版本。
</Note>
