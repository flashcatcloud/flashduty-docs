...
`client.Members.MemberNotify`（`POST /member/notify`，`memberNotify`）以调用方身份向账户成员发送邮件，仅可使用 AI SRE 会话凭据调用——其他凭据（包括普通 `app_key`）一律以 `AccessDenied` 拒绝（用 `IsAccessDenied` / `ErrorCodeOf` 判定）。请求字段：`subject`（必填，1–200 字符）与 `html`（必填，收件人看到的完整邮件正文；服务端会清洗，移除 `script`、`style`、`iframe` 等标签以及全部 `on*` 事件属性，图片仅保留 `src` 为 `https` 的，链接仅允许 `http` / `https` / `mailto`），`person_ids` 可选（至多 20 个且不可重复，省略或为空时只发送给调用方本人）。发送前先把 `dry_run` 设为 `true` 调一次：此时不入队，响应中的 `html` 就是收件人将收到的完整邮件，可据此确认清洗没有删掉正文依赖的内容，也不占用每小时限额与同轮次去重。投递是异步的，`accepted` 只表示邮件已入队；响应按收件人逐条返回 `recipients[]`（`person_id` 与 `status`），`status` 为 `accepted` 或 `skipped`，后者带 `reason`：`not_member`、`no_email`、`email_disabled`、`duplicate`、`rate_limited`（同一收件人每小时最多 20 封）、`send_failed`。
