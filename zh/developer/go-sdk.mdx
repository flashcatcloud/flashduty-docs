...
`<type>.query` 查询工具的 `params` 在 Go 侧有对应的请求参数类型，用 `flashduty.NewDatasourceQueryInvokeRequest(datasourceID, tool, params)` 构造请求：它把参数结构体序列化进 `DatasourceToolInvokeRequest.Params`，`params` 为 `nil` 时直接返回错误，查询工具不允许省略参数。

这些参数类型的可选字段一律为指针：留 `nil` 时该键不出现在请求里，服务端沿用默认值；显式传 `null` 会被服务端拒绝，因此没有发送 null 的方式。要显式送出零值（例如 `powersql` 的 `false`），用 `flashduty.Bool` / `flashduty.Int64` / `flashduty.String` 取指针。

| 工具 | 参数类型 | 类型专属字段 |
|------|----------|--------------|
| `prometheus.query` | `flashduty.PrometheusQueryParams` | 无 |
| `mysql.query` | `flashduty.MySQLQueryParams` | 无 |
| `postgres.query` | `flashduty.PostgresQueryParams` | 无 |
| `oracle.query` | `flashduty.OracleQueryParams` | 无 |
| `clickhouse.query` | `flashduty.ClickHouseQueryParams` | 无 |
| `elasticsearch.query` | `flashduty.ElasticsearchQueryParams` | 无 |
...
每个参数类型都由 `Expr` 与 `Execution`（`flashduty.DatasourceQueryExecution`）组成。`Expr` 的方言随数据源类型而定：`prometheus` 为 PromQL，`loki` 为 LogQL，`victorialogs` 为 LogsQL，`mysql` / `postgres` / `oracle` / `clickhouse` 为单条只读 SQL，`elasticsearch` 为单条 SQL（不支持 ES DSL 查询），`sls` / `tencent_cls` 为各自的检索语法（CLS 的 `Syntax` 取 `cql` 或 `lucene`）。
...
// res.Data 为工具专属 JSON，res.Summary 可选。
fmt.Println(string(res.Data))
```

`DatasourceQueryExecution.Kind` 决定求值方式：

| `kind` | 含义 | 时间字段 |
|--------|------|----------|
| `instant` | 在单个时间点求值 | `ToMS` 为求值时刻；`FromMS` 只有 Loki 可选（`instant` 保留完整时间上下文，便于 `$__auto` 范围解析），Prometheus 拒绝该字段 |
| `range` | 在 `[FromMS, ToMS]` 上按步长求值时间序列 | `FromMS`、`ToMS` 必填；`MaxDataPoints` 必填，服务端据此计算实际步长；`MinStepSeconds` 为步长的正数下限，可选 |
| `window` | 在单个有界窗口内求值 | `FromMS`、`ToMS` 必填 |

各类型接受的 `kind` 不同：`prometheus` 与 `loki` 接受 `instant` / `range`；`mysql`、`postgres`、`oracle`、`clickhouse`、`elasticsearch`、`sls`、`tencent_cls` 只接受 `window`；`victorialogs` 用 `window` 取原始日志（可用 `Limit` / `Direction`），用 `instant` / `range` 做统计查询时必须带 `FromMS`，且拒绝 `Limit` / `Direction`。`Limit` 与 `Direction`（`latest` / `earliest`）只作用于原始日志结果，不限制 SQL 行数或扫描量：`loki`、`victorialogs`、`tencent_cls` 为 1–1000，`sls` 为 1–100。`step_seconds` 是保留字段，SDK 未建模，请勿发送。

这些类型是手写的、不参与代码生成：`params` 以原始 JSON 承载，它们不在任何 requestBody 的可达范围内，生成器只能把它们当作响应侧类型（epoch 毫秒会变成序列化成 RFC3339 字符串的 `TimestampMilli`，可选字段也会丢掉 `omitempty`）。

`client.Applications`（RUM 应用）的 remote-config 方法组用于读写 RUM 应用的远程配置：`RemoteConfigReadGet`（取当前生效的远程配置及其存储版本）、`RemoteConfigReadHistoryList`（列出已发布的配置版本）、`RemoteConfigReadPreview`（在给定客户端上下文里评估一份草稿配置，不发布）、`RemoteConfigWriteUpdate`（发布一个完整的新版本）、`RemoteConfigWriteHistoryRevert`（把更早版本的内容作为新版本重新发布）。

`client.Schedules.ByPerson`（`POST /schedule/by-person`）返回某个成员的当前与下一次值班，以及他参与的全部已启用排班。
...
`client.AlertRules` 的规则读写已切到 V2：取详情用 `ReadInfoV2`、创建用 `WriteCreateV2`、更新用 `WriteUpdateV2`，三者都收发 `*flashduty.AlertRuleV2`（含 V2 的恢复与结束模式）；若只想拿规则基础信息，`ReadList` 仍足够，需要完整配置再调 `ReadInfoV2`。旧的非 V2 `ReadInfo` / `WriteCreate` / `WriteUpdate` 已从 SDK 移除，`ReadCounterNode`、`ReadDstypes` 也一并删除，升级 SDK 时按此改名。

另外，请求类型上的可选数组字段是按「是否存在」生效的：留 `nil` 时该键不出现在请求里，服务端保留已存储的值；传空切片则发送 `[]`，清掉该字段。目前此类字段只有 `AlertRuleV2.InvestigationTargets`（至多 20 项），即要清空下钻入口必须显式传一个空切片。
