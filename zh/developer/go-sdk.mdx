...
| Option | 说明 |
|--------|------|
| `WithBaseURL(raw string)` | 覆盖 API 基础地址（默认 `https://api.flashcat.cloud`）。可带路径前缀，例如 `https://gateway.example.com/api`，请求会打到 `/api/<endpoint>`（路径会自动规范化为以 `/` 结尾，因此端点路径是追加而不是替换最后一段）；私有化部署用它指向自有网关；非法 URL 会在 `NewClient` 阶段报错 |
...
<Info>
**私有化部署**：用 `WithBaseURL` 将客户端指向您自有的 Flashduty 网关地址即可,其余调用方式完全不变。网关地址带路径前缀（如 `https://gateway.example.com/api`）也能正常工作：SDK 保留该前缀并把路径规范化为以 `/` 结尾，请求打到 `/api/<endpoint>`。
</Info>
