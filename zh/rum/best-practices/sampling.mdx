...
- **会话重放需要显式对齐**：`sessionSampleRate` 被设成 100 之后，SDK 会在全部会话上抽取回放，不按上面的做法把分桶结果传给它，有回放的会话未必有完整行为数据。

与上一个问题的区别：上一个是把非错误数据**整体关掉**，适合只关心错误的场景；这里是**保留两成完整现场**，适合既要异常不漏、又要保留性能与行为分析能力的场景。
