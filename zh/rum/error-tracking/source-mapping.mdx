...
RUM 异常追踪支持结合 `sourcemap`、Android mapping 文件和 iOS dSYM 还原错误堆栈，在异常详情中查看原始源码位置，精确定位问题。
...
<Steps>
  ...
  <Step title="关联符号文件">
    当错误堆栈中的文件路径、行号或地址信息与已上传的 `sourcemap`、Android mapping 文件或 iOS dSYM 文件匹配时，Flashduty 会自动将压缩、混淆或编译后的错误位置映射到原始源代码。
