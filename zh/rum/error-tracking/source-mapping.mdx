...
源码管理页面根据应用类型（Web、iOS、Android）提供不同的上传面板和元数据字段：

<Tabs>
  <Tab title="Web (Browser)">
    Web 平台使用标准的 JavaScript SourceMap 文件（`.map`），通过 Flashduty CLI 上传。

    {/* REVIEW: 请补充 Web 平台上传界面截图和说明 */}
  </Tab>
  <Tab title="iOS">
    iOS 平台需要上传 dSYM 文件，并提供以下元数据：

    | 字段 | 说明 |
    |------|------|
    | **UUID** | {/* REVIEW: dSYM 文件的 UUID，用于匹配崩溃堆栈 */} |

    {/* REVIEW: 请补充 iOS 上传面板截图和操作步骤 */}
  </Tab>
  <Tab title="Android">
    Android 平台需要上传 ProGuard mapping 或 R8 mapping 文件，并提供以下元数据：

    | 字段 | 说明 |
    |------|------|
    | **Build ID** | {/* REVIEW: 构建标识符 */} |
    | **Variant** | {/* REVIEW: 构建变体（如 release、debug） */} |
    | **Version Code** | {/* REVIEW: 应用版本号 */} |

    {/* REVIEW: 请补充 Android 上传面板截图和操作步骤 */}
  </Tab>
</Tabs>

### 源码映射列表筛选

{/* REVIEW: 请补充源码映射列表的筛选和排序功能说明 */}

在源码管理页面，您可以通过以下条件筛选和排序已上传的源码映射文件：
