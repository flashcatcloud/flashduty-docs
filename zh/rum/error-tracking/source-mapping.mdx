...
- **React Native**：Android 通过 SDK 自带的 Gradle 脚本在 release 构建时自动上传 JS bundle sourcemap；iOS 通过 [Flashduty CLI](https://github.com/flashcatcloud/flashcat-cli)（≥ 0.4.0）上传 `main.jsbundle` 与 `main.jsbundle.map`（`sourcemaps upload-react-native`）。详见下文 [上传 React Native SourceMap](#上传-react-native-sourcemap)
...
<Steps>
  ...
  <Step title="配置上传参数">
    ...
    <ParamField path="自定义上传 Endpoint" type="string">
      仅私有化部署显示。私有化部署时面板会自动填入部署下发的上报地址，可手动覆盖（协议 + 域名，不带路径，例如 `https://rum.example.com`）；留空则默认上传到 Flashcat SaaS。填写后，生成的上传命令会在开头带上 `FLASHCAT_SOURCEMAP_INTAKE_URL=<endpoint>` 环境变量。
    </ParamField>
  </Step>
  <Step title="执行上传">
...
<Steps>
  ...
  <Step title="构建后执行上传">
    在构建产物生成后执行上传任务。**用环境变量配置时必须加 `--no-daemon`**，否则 hvigor 复用的守护进程会拿到陈旧的环境变量，导致上传被静默跳过或传到错误的版本号下：
...
## 上传 React Native SourceMap

React Native 的 JS 错误堆栈通过 sourcemap 还原。与 Flutter 类似，React Native 应用在同一份 JS 代码之上还有 Android/iOS 两端原生层：两端 sourcemap 按平台分开存储，同名的 bundle 不会互相覆盖——服务端按 `react-native` 类型 + 平台（iOS / Android）来匹配，因此 iOS 与 Android 可以使用相同的 bundle 文件名。

在「应用管理」-「源码管理」中选择「React Native」标签页，上传面板会按所选平台生成对应的脚本：

| 字段 | 说明 |
|------|------|
| 平台 | `Android` 或 `iOS` |
| API Key | 上传凭证，对应环境变量 `FLASHCAT_API_KEY` |
| 服务名 | 需与 SDK 初始化时设置的 `serviceName` 完全一致；不设置时服务端默认取 `applicationId`，与 SDK 上报的 service 对不上会存错归类 |
| 发布版本 | 仅 iOS：对应 `CFBundleShortVersionString`（如 `1.0.0`），是匹配 key 的一部分，必填；Android 从构建的 `versionName` 读取 |
| 构建号 | 仅 iOS，可留空：对应 `CFBundleVersion`（如 `42`），映射为 `--build-version` |
| 自定义上传 Endpoint | 仅私有化部署：自定义上报地址，对应环境变量 `FLASHCAT_SOURCEMAP_INTAKE_URL`，留空表示 Flashcat SaaS |

### Android：构建时自动上传

Android 由 SDK 自带的 Gradle 脚本在 release 构建中完成上传（脚本会调用 flashcat-cli 上传 Metro 刚产出的 bundle）。在 `android/app/build.gradle` 固定服务名并应用脚本：

```groovy
// android/app/build.gradle
// 必须与 SDK 初始化时传入的 serviceName 一致；未设置时默认取 applicationId
...
然后带凭证执行 release 构建即可，上传发生在构建过程中：
...
<Note>
上传对象是 `index.android.bundle` 及对应 sourcemap。SDK 的 Metro 配置（Debug ID）也是 `withDatadogMetroConfig` 包装的，详见 [React Native SDK 接入](/zh/rum/sdk/react-native/sdk-integration)。
</Note>

### iOS：构建后手动上传

iOS 没有构建钩子，需要在 release 构建后手动上传。先让 `react-native-xcode.sh` 产出 sourcemap——它只有在 `SOURCEMAP_FILE` 已设置时才会生成，因此要在 Xcode 构建阶段「Bundle React Native code and images」中、`react-native-xcode.sh` 之前导出：

```bash
# Xcode > Build Phases > "Bundle React Native code and images"
# 放在 react-native-xcode.sh 之前
export SOURCEMAP_FILE="$DERIVED_FILE_DIR/main.jsbundle.map"
```

再安装 CLI（≥ 0.4.0）并上传 bundle 与 sourcemap：
...
<Warning>
- 上传的 bundle 文件名必须与应用运行时加载的名字一致（Android 为 `index.android.bundle`，iOS 为 `main.jsbundle`）。符号化按文件名匹配堆栈帧，CI 中用 `--bundle-output` 重命名过的 bundle 永远不会被匹配到。
- `--release-version` 必须与上报的版本一致（SDK 的 `config.version` 覆盖或默认值），否则匹配不到。
- 服务名如果写错，上传文件会归入错误的 service，问题列表与符号表两边都对不上。
- 更完整的 React Native 接入说明（含 `@flashcatcloud/mobile-react-native` 包版本要求与私有化部署）请参阅 [React Native SDK 高级配置](/zh/rum/sdk/react-native/advanced-config)。
</Warning>
...
| 功能 | 说明 |
|------|------|
| 查看已上传的符号文件 | 列出所有已上传的文件（SourceMap、小程序 SourceMap、ProGuard mapping 文件、dSYM、NDK 原生符号文件、Flutter 符号文件、Electron Breakpad 符号文件、React Native bundle sourcemap），包括路径、服务名、版本号、大小和上传时间。Android 标签页同时展示 ProGuard mapping 文件和 NDK 原生符号文件，NDK 行会显示 build_id、arch 和 lib_name 列；Electron 标签页每个原生模块一行，展示模块（lib_name）、Debug ID（build_id）、架构（arch）、服务、版本和大小列，并支持按 Debug ID 搜索 |
| 按平台筛选 | 在 Web、iOS、Android、微信小程序、HarmonyOS、Flutter、Electron 和 React Native 标签页之间切换，查看不同平台的符号文件 |
