...
<img
  src="https://docs-cdn.flashcat.cloud/imges/png/7fbb901611a71dd559da8d59c6251472.png"
  alt="告警设置"
/>
...
{/* REVIEW: 功能概述 — 告警设置页面顶部展示了可视化的处理流水线，帮助用户理解告警处理的完整链路 */}

在告警设置页面，您可以看到一条可视化的处理流水线，展示了从 Error 到告警分派的完整流程：
...
- 点击 **Issue** 节点可跳转到过滤和分级配置区域
- 点击 **Alert** 和 **Dispatch** 节点可跳转到 Flashduty 协作空间的对应配置页面

<Tip>
处理流水线是告警设置的导航辅助工具，帮助您快速定位到需要配置的环节。
</Tip>
{/* END REVIEW */}
...
配置告警分级规则和数据过滤规则时，请注意以下限制：

| 限制项 | 上限 |
|--------|------|
| **每种类型的规则数** | 6 条 |
| **每条规则的 OR 条件组数** | 2 组 |
| **每个 OR 组内的 AND 条件数** | 3 个 |
| **每个筛选条件的匹配值数** | 8 个 |
| **自定义字段（context）嵌套层级** | 3 层 |

### 筛选条件的逻辑结构

{/* REVIEW: 请验证并补充筛选条件的 OR/AND 逻辑结构说明 */}

每条规则的筛选条件采用 **OR-of-AND** 结构：

- 规则内可包含多个 **OR 条件组**，组间为「或」关系
- 每个 OR 条件组内可包含多个 **AND 条件**，条件间为「且」关系
- 匹配值支持以下语法：精确匹配、通配符（`*` 匹配多个字符、`?` 匹配单个字符）、正则表达式（`/pattern/`）
{/* END REVIEW */}
...
<CardGroup cols={2}>
  <Card
    title="RUM 告警降噪"
    icon="sliders"
    href="/zh/rum/best-practices/alert-noise-reduction"
  >
    典型场景配置方案，快速减少无效告警干扰
  </Card>
  <Card
