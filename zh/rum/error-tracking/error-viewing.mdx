---
title: "Issue 概览与详情"
description: "了解 Flashduty RUM 如何将相似错误聚合为 Issue，并查看发生记录、堆栈、影响用户和关联上下文。"
...
| 服务名称 | 关联的服务 |
| 错误原因 | 系统推断的可能根因 |
| 优先级 | Issue 的优先级（`Critical`、`Warning`、`Info`），由告警分级规则或系统评分自动判定，历史数据可能为空并显示为 `—` |
...
<Note>
Issue 列表支持按**优先级**排序，筛选条件默认包含「优先级」；每条 Issue 错误消息左侧的竖条颜色反映其优先级（`Critical` 为红色、`Warning` 为橙色、`Info` 为浅橙色，无优先级数据时为灰色）。优先级由告警分级规则或系统评分判定，不可手工修改；进入 Issue 详情页同样可在头部看到该优先级。
</Note>
...
Issue 列表右上角提供 **CSV** 导出按钮，导出内容与当前列表的筛选、排序条件保持一致。

点击 CSV 按钮后，可在弹层中选择导出内容：

| 选项 | 默认值 | 说明 |
|------|--------|------|
| 包含错误样例 | 开 | 每条 Issue 附上最近一次上报的错误样例列（发生时间、版本、页面、会话 ID、设备、系统、浏览器、堆栈），前缀为 `sample_` |
| 同时包含最早一次样例 | 关 | 额外附上最早一次上报的错误样例列（`sample_first_*`），用于追查问题首次出现的版本；仅在「包含错误样例」开启时可勾选 |

<Note>
错误样例只在错误数据保留期内存在，更早的 Issue 导出后样例列会为空。
</Note>

导出文件始终包含 Issue 的 18 个字段（问题 ID、问题链接、应用、服务、错误类型、错误信息、状态、优先级、是否崩溃、错误数、影响会话数、首次出现、首次出现版本、最近出现、最近出现版本、涉及版本、疑似原因、解决时间），样例列在「包含错误样例」开启后附加。

<Warning>
- 单次最多导出 **100 条**。匹配数量超过 100 时，按钮上方会提示「最多可导出 100 条，请缩小时间范围或增加筛选条件」；导出完成后若文件被截断，页面会再次提示已导出的条数。
- 时间列（首次出现 / 最近出现 / 解决时间 / 样例发生时间）的列头会附加时区标识（如 `首次出现 (Asia/Shanghai)`），列内值为该时区下的 `YYYY-MM-DD HH:mm:ss` 文本，便于电子表格按日期排序。
- 样例堆栈截取前 20 行。完整堆栈可在 Issue 详情页查看（那里会按平台做符号化还原）；导出不做符号化，给出的是上报的原始帧。
- 为防止公式注入，以 `=`、`+`、`-`、`@` 开头的单元格会被加上引号前缀。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="错误样例">
    ...
    对于 Native 崩溃，样例导航列表中的每条样例都会标注符号化状态（「已解析」/「未解析」徽标）。当最新一条样例尚未解析、而更早的样例可以符号化时，详情会自动切换到可符号化的样例，并提示「已自动切换到可符号化的样例（最新一条尚未解析）」；点击「回到最新」可切回最新上报的样例。自动切换最多尝试 5 条更早的样例，若这些样例均无法符号化，详情会切回最新一条样例，不再继续自动切换。
  </Accordion>
  ...
  <Accordion title="错误堆栈（Native — Android/iOS）">
    ...
    **Flutter 支持**
    ...
    Flutter 原生崩溃（`source_type` 为 `ndk`、`android` 或 `ios`）携带线程堆栈和 Binary Images，与 Android/iOS 原生崩溃一样使用上述 Native 渲染展示；Dart 异常则按堆栈中的 build_id 匹配已上传的 Flutter 符号文件，进行符号化还原。

    **React Native 支持**

    React Native 与 Flutter 的形态一致：JS 错误按 Web 堆栈展示，并以上传的 JS sourcemap 还原原始源码；原生崩溃（`source_type` 为 `ios`、`android` 或 `ndk`）携带线程堆栈和 Binary Images，按上述 Native 堆栈视图展示，与 Android/iOS 原生崩溃一样通过 dSYM / NDK 符号文件还原。JS 错误的 sourcemap 匹配范围为 `react-native` 类型，并按错误的 OS（`os_name`）推算平台（iOS / Android）进一步收窄——两端 bundle 可能同名，平台收窄可以避免匹配到另一端的文件。

    **Electron 支持**

    Electron 错误按类型分流展示：主进程和渲染进程的 JavaScript 错误是 V8 堆栈，与浏览器错误一样按 Web 堆栈展示（上传 sourcemap 后还原原始源码）；minidump 原生崩溃则是地址形式的 Native 堆栈（含线程与 binary_images），按上述 Native 堆栈视图展示，上传匹配的 Breakpad 符号后还原函数名、文件名和行号。详见 [Electron 错误符号化](../sdk/electron/error-symbolication)。
  ...
  <Accordion title="属性">
    ...
    Electron 错误的属性面板会在 Other 属性组额外展示一个虚拟 `process` 属性（主进程 / 渲染进程），标识错误来自哪个运行时；非 Electron 错误不展示该属性。
  </Accordion>
</AccordionGroup>
