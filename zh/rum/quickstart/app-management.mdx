...
<!-- REVIEW: 功能概述 — 说明地理信息采集控制的用途和隐私合规场景（如 GDPR） -->
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `no_geo` | boolean | `false` | <!-- REVIEW: 是否禁用地理位置数据采集，禁用后不再采集 geo_country、geo_city 等字段 --> |
| `no_ip` | boolean | `false` | <!-- REVIEW: 是否禁用客户端 IP 采集，禁用后不再记录用户 IP 地址 --> |

<!-- REVIEW: 请补充以下内容：
- 各 geo 字段（geo_country、geo_city、geo_region 等）的详细说明
- no_geo 和 no_ip 的关联关系（禁用 IP 是否自动禁用 geo）
- 隐私合规建议（GDPR 等场景下的推荐配置）
- 默认状态说明（默认开启采集）
-->

### 使用方法

<!-- REVIEW: 请补充操作步骤，包括在应用创建和应用配置中的开关位置 -->
<!-- END REVIEW -->
