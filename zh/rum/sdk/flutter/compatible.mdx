...
| 自动 resource | 支持（需伴生包） | 通过 `flashcat_tracking_http_client` 的 `enableHttpTracking()`；0.1.0 存在已知问题，见下方限制 |
...
| Session Replay | 不支持 |
| resource 采集已知问题 | `flashcat_tracking_http_client` 0.1.0 中，用 `drain()` 或 `asFuture()` 消费响应体的请求不会产生 resource 事件（这两个方法会覆盖 SDK 注册的 `onDone` / `onError`）。改用 `toList()`、`transform()` 或自带 `onDone` 的 `listen()` 可规避 |
