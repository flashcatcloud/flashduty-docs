...
<Warning>
**已知问题（`flashcat_tracking_http_client` 0.1.0）**：若用 `drain()` 或 `asFuture()` 消费响应体，该请求不会产生 resource 事件。这两个方法会覆盖 SDK 注册在响应流上的 `onDone` / `onError` 回调，导致资源采集只开始、不结束，且没有任何报错。改用 `toList()`、`transform()` 或自带 `onDone` 的 `listen()` 可正常上报。
</Warning>
