...
<Note>
私有化部署时，在命令前加上 `FLASHCAT_SOURCEMAP_INTAKE_URL` 环境变量即可将符号文件上传至自定义入口（协议 + 域名，不带路径），例如 `FLASHCAT_SOURCEMAP_INTAKE_URL=https://rum.example.com`；未设置时默认上传到 Flashcat SaaS。
</Note>
