...
<Note>
公有云符号上传地址是 `https://ci.flashcat.cloud`（**不是** RUM 上报用的 `browser.flashcat.cloud`）。省略 `endpoint` 时插件默认打到这里；私有化部署请改成控制台源码管理面板下发的地址（协议 + 域名，不带路径），或设置环境变量 `FLASHCAT_SOURCEMAP_INTAKE_URL`。`flashcatSymbolUploadPlugin()` 还接受两个可选参数：`buildDir`（构建产物目录，默认 `build/default`）和 `pluginVersion`（写入上传请求头 `DD-EVP-ORIGIN-VERSION` 的版本号，默认与插件版本一致，当前为 `0.1.3`）。一般无需设置。
</Note>
