...
宿主应用需要在恢复启动的 `Want` 中读取这些参数，并且只恢复可以安全继续的状态。该能力是尽力而为的状态恢复，依赖宿主实现 `onSaveState`。SDK 编排下的状态保存和 `wantParam` 恢复仅在 API 24 模拟器上完成验证，尚未完成真机验证。
...
| TaskPool 或 Worker 抛出异常 | 在已验证场景中不会到达 error observer，不会导致宿主进程退出，不属于崩溃策略覆盖范围 |
