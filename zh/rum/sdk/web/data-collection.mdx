---
title: "Web SDK 数据收集"
description: "详细了解 FlashCat Web RUM SDK 收集的数据类型、事件属性、性能指标以及数据存储机制"
keywords: ["RUM", "Web SDK", "用户监控", "前端监控"]
---

<Info>
FlashCat Web RUM SDK 自动收集丰富的用户行为和性能数据。每个 RUM 事件都包含默认属性（如 `view.url`、`device.type`、`geo.country`）以及特定事件类型的附加指标和属性。
</Info>
...
<Card title="SDK 接入" icon="plug" href="/zh/rum/sdk/web/sdk-integration">
  了解如何在 Web 应用中接入 FlashCat RUM SDK
</Card>
