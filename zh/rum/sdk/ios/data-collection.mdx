---
title: "iOS SDK 数据收集"
description: "全面了解 Flashduty iOS RUM SDK 自动收集的性能指标、事件属性和设备信息"
keywords: ["RUM", "iOS SDK", "用户监控", "前端监控"]
---

<Info>
Flashcat iOS RUM SDK 会自动生成包含相关指标和属性的事件。每个 RUM 事件都包含默认属性（如 `view.url`、`device.type`、`geo.country`）以及事件特定的指标和属性（如 `view.time_spent`、`resource.method`）。
</Info>
...
Flashcat iOS RUM SDK 为所有事件自动附加默认属性，帮助您了解用户设备、网络状态和应用上下文。
...
在上传到 Flashcat 之前，数据以明文形式存储在应用沙盒的缓存目录（`Library/Caches`）中。
