...
3. 选择版本规则：
   - **推荐**：选择 **Up to Next Major Version**，并输入当前最新版本号（如 `0.6.0`）
...
```ruby Podfile
pod 'FlashcatCore', '~> 0.6.0'    # 核心 SDK（必选）
pod 'FlashcatRUM', '~> 0.6.0'     # RUM 功能（必选）
pod 'FlashcatTrace', '~> 0.6.0'   # Trace 功能（启用 Trace feature 或手动 span 时需要）

# 可选模块
pod 'FlashcatWebViewTracking', '~> 0.6.0'   # WebView 追踪
pod 'FlashcatCrashReporting', '~> 0.6.0'    # 崩溃报告
```
