---
title: "iOS SDK 高级配置"
description: "深入配置 Flashcat iOS RUM SDK 的高级功能，包括自定义事件、用户追踪、采样控制和数据安全"
keywords: ["RUM", "iOS SDK", "用户监控", "前端监控"]
---

Flashcat iOS RUM SDK 提供丰富的高级配置选项，帮助您根据业务需求定制数据收集和上下文信息。
...
Flashcat iOS SDK 会自动追踪通过启用的 `URLSession` 发起的网络请求。您也可以手动追踪自定义资源（如第三方 API、GraphQL 查询等）。
...
Flashcat iOS SDK 会自动捕获默认 RUM 属性。您还可以向 RUM 事件添加额外的上下文信息，例如自定义属性。
...
<Card title="数据收集" icon="database" href="https://docs.flashcat.cloud/zh/flashduty/rum/ios-data-collection">
  了解 SDK 收集的数据类型
</Card>
