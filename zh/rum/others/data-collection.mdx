...
title: "RUM 数据收集机制"
sidebarTitle: "数据收集"
description: "本文档详细介绍 Flashduty RUM 的数据收集机制，包括事件类型、属性、指标以及数据保留期等信息。"
keywords: ["RUM", "其他", "用户监控", "前端监控"]
---
...
<Tabs>
  ...
  <Tab title="Resource">
    ...
    | `resource_protocol` | 字符串 | 资源获取使用的网络协议（如 `h2`、`h3`、`http/1.1`） |
    | `resource_delivery_type` | 字符串 | 资源投递方式（`cache`、`navigational-prefetch`、`other`） |
    | `resource_render_blocking_status` | 字符串 | 资源是否阻塞渲染（`blocking`、`non-blocking`） |
    | `resource_transfer_size` | 数字 | 实际传输大小（字节），可能小于完整资源大小（如命中缓存或经过压缩） |
  </Tab>
  <Tab title="Long Task">
    ### 长任务指标
