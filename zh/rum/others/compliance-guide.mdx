...
| 受托处理者 | **北京快猫星云科技有限公司（Flashduty）** | 仅在为您提供应用性能监控（RUM）服务的必要范围内处理数据，不用于约定外目的 |
...
| --- | --- | --- | --- | --- |
| 设备信息 | 设备型号、品牌、设备名称、CPU 架构、屏幕信息 | 区分机型上的崩溃与性能表现 | SDK 自动采集 | 否（基础属性） |
| 操作系统信息 | 系统名称、版本号 | 区分系统版本的兼容性与故障分布 | SDK 自动采集 | 否（基础属性） |
| 网络信息 | 网络连接状态、网络接口类型、蜂窝制式、运营商名称 | 分析网络环境对加载与请求耗时的影响 | SDK 自动采集 | 否（基础属性） |
| 应用运行信息 | 包名、版本号、构建号、运行环境、SDK 版本 | 关联问题与发布版本 | SDK 自动采集 | 否（必需） |
| 崩溃与日志信息 | 崩溃堆栈、错误信息、ANR、自定义日志 | 还原崩溃现场、定位缺陷 | 发生崩溃/错误或主动打点时采集 | 可（关闭错误追踪） |
| 行为与性能信息 | 页面访问路径与停留时长、用户操作、资源加载耗时、卡顿、启动耗时 | 还原用户路径、分析性能瓶颈 | 用户交互/页面跳转时采集 | 可（关闭自动追踪或脱敏） |
| 会话标识 | SDK 随机生成的会话 ID（`session.id`） | 串联同一次使用的事件以便回溯 | SDK 自动生成 | 否（已匿名化） |
| 网络地址 | 上报来源 IP 派生的国家/地区 | 按地域统计性能与故障 | 服务端处理时派生 | 可（关闭 IP / 地理位置） |
...
<AccordionGroup>
  ...
  <Accordion title="采集可控（关闭 / 采样 / 脱敏）">
    ...
    - **采样**：通过会话采样率降低采集与上报比例；
    - **属性脱敏**：通过 `EventMapper` 在事件上报前修改或丢弃字段，移除可能混入的 PII（如操作名、URL 中的个人信息）；
  ...
  <Accordion title="代理上报">
    支持通过您自有的代理服务器转发全部 RUM 事件，使终端设备不直接与 Flashduty 通信，便于您统一管控出口流量。
  </Accordion>
  <Accordion title="撤销授权与数据销毁">
    当用户撤回同意、SDK 停止服务或账户注销后，您可将同意状态切换为 `NOT_GRANTED` 立即停止采集；已上报数据由 Flashduty 按约定保留期到期后删除或匿名化。
  </Accordion>
</AccordionGroup>

## 接入方的合规义务

本节展开 [合规使用的两项关键要求](#合规使用的两项关键要求) 的具体做法。
...
收集的个人信息：设备信息（型号、品牌、操作系统及版本、CPU 架构）、
  网络信息（网络状态、网络制式、运营商名称）、应用运行与崩溃日志信息、
  应用使用过程中的页面访问与操作行为、随机生成的会话标识
官网 / 隐私政策：https://flashcat.cloud
```
</CodeGroup>
...
<Steps>
  ...
  <Step title="提供撤回入口">
    在 App 设置中提供撤回同意入口，撤回后调用 `setTrackingConsent(NOT_GRANTED)`。
  </Step>
  <Step title="避免传入敏感信息">
