---
title: "Native"
description: "深入了解 Flashduty Native（Android/iOS）RUM 分析看板的核心功能，掌握移动应用性能监控、异常追踪和资源分析的关键指标"
keywords: ["RUM", "分析看板", "用户监控", "前端监控"]
---
...
Native 看板适用于 Android、iOS、HarmonyOS、Flutter 和 React Native 应用，不同平台存在以下差异：

- **Flutter / React Native**：一个 Flutter 或 React Native 应用同时覆盖 Android 和 iOS 两端，筛选栏会额外常驻 `os_name` 筛选项，便于按设备系统切片分析；概览与异常分析中，**ANR 率**（来自 Android 设备）与**无响应时长率**（来自 iOS 设备的 App Hang）两张卡片并列展示。
...
- **核心健康指标** - 突出显示三个移动应用核心指标：崩溃次数、无崩溃率、无响应时长率，快速识别应用稳定性问题
...
- **帧率（P75）**：展示应用运行时帧率的 P75 分位数，衡量画面流畅度。目标为 60fps，数值越高表示交互越流畅。SDK 会把高刷新率屏幕（如 ProMotion、120Hz Android 设备）的逐帧采样值归一化到 60fps 基准并封顶 60，因此该指标反映的是相对流畅度，而不是屏幕的物理刷新率。
- **CPU 消耗（P75）**：追踪 CPU 占用率的 P75 分位数，识别计算密集型操作。过高的 CPU 消耗会导致设备发热和耗电增加。
- **内存使用（P75）**：监控应用内存占用的 P75 分位数，及时发现内存泄漏或异常增长。
- **JS 线程帧率（P75）**：监控 React Native JS 线程帧率的 P75 分位数（FPS）。相比原生帧率，它更直接反映业务 JavaScript 造成的卡顿（重渲染、桥上的同步工作）。仅 React Native SDK 上报。
...
- **JS 线程帧率**：追踪 React Native 各视图 JS 线程的帧率表现（FPS），识别业务 JavaScript 卡顿；仅 React Native SDK 上报，其他平台的视图该列无数据。
...
- **慢帧占比**：测量时间中被慢帧占用的比例。慢帧指渲染耗时超过屏幕帧预算（60fps 下约 16.67ms，高刷屏更短）但未达到冻结帧标准的帧。该值越高，用户感知到的界面不流畅越明显。移动端 SDK 只上报这个比例，不上报慢帧计数。
- **冻结帧数**：主线程单条任务耗时超过 700ms 的次数，表现为界面完全卡住、无法交互。
- **长任务数**：主线程上耗时超过阈值的任务数量（Android 与 iOS 默认均为 100ms，可配置），用于定位性能瓶颈。长任务会阻塞用户交互和界面更新。
- **卡顿频率**：平均每秒发生的冻结帧次数（冻结帧数 ÷ 视图停留时长），评估整体流畅度表现。
- **JS 帧率（平均）**：该视图各次更新的 JS 线程平均帧率，低于 50 FPS 高亮。仅 React Native SDK 上报。
- **JS 帧率（最低）**：该视图任一次更新记录到的最低 JS 线程帧率，即用户遇到的最差卡顿，低于 30 FPS 高亮。仅 React Native SDK 上报。

<Warning>
**冻结帧数、长任务数、卡顿频率三者同源。** 在移动端，冻结帧并不是从渲染帧里统计出来的，而是「耗时超过 700ms 的长任务」这一子集；卡顿频率又由冻结帧数算得。因此只要在 SDK 里关闭长任务采集，这三列会**同时恒为 0**——这表示「未采集」，而不是「没有卡顿」。

慢帧占比走的是另一条独立链路，关闭长任务采集不影响它。所以「慢帧占比有数值、其余三列全为 0」是关闭长任务采集后的预期现象，不是数据丢失。

如果你关闭长任务采集是为了减少事件量，更好的做法是**调高阈值而不是关闭**，详见 [Android 性能影响](/zh/rum/sdk/android/performance-impact)。
</Warning>

<Note>
**慢帧占比不能跨平台横比。** 分子都是慢帧耗时，但分母不同：Android 除以渲染总耗时，iOS 除以视图停留时长。同一种用户体验在 Android 上的数值通常明显更高，因为静止界面几乎不产生帧，分母很小，而首帧本身又必然是慢帧。请在同一平台内部做纵向对比。
</Note>
...
<Frame>
  <img src="https://docs-cdn.flashcat.cloud/imges/png/b5868b87e52ddc7b82c61f685b2d0bd9.png" alt="异常分析看板展示崩溃次数、无崩溃率、ANR 率和无响应时长率" />
</Frame>
...
- **无响应时长率**：每小时视图时长内主线程处于无响应状态的秒数（秒/小时），按视图时长加权平均，且仅统计 SDK 已完成测量的视图。**该卡片只在 iOS 和 Flutter 应用中展示**，纯 Android 应用对应展示的是上方的 ANR 率。详见下方「如何降低无响应时长率？」。

#### 错误数据统计

- **错误数**：展示错误总数和时序趋势，了解应用健康状况的整体变化。
- **错误类型分布趋势图**：通过堆叠柱状图展示崩溃错误（crash_count）与非崩溃错误（non_crash_count）随时间的分布变化，快速识别异常时段和错误类型变化趋势。
...
- **错误类型分布趋势（堆叠柱状图）**：监控各错误类型随时间的变化趋势，及时发现新增错误类型或某类错误的异常增长。

#### 版本 Crash 分布

- **版本 Crash 分布（饼图）**：统计不同应用版本的崩溃分布情况，识别高风险版本。
- **版本 Crash 分布趋势（堆叠柱状图）**：监控各版本崩溃随时间的变化，评估新版本质量，必要时进行热修复或回滚。

#### 系统版本异常分布

- **系统版本异常分布（饼图）**：统计不同操作系统版本（如 Android 11、Android 12、iOS 15 等）的异常分布情况，识别系统兼容性问题。
- **系统版本异常趋势（堆叠柱状图）**：监控各系统版本异常随时间的变化，为系统兼容性优化提供数据支撑。
...
| 指标类型 | 阈值 | 用户体验影响 | 优先级 |
|----------|------|--------------|--------|
| **慢帧（Slow Frame）** | 渲染超时但未达冻结帧标准（60fps 下约 > 16.67ms，高刷屏更短） | 轻微卡顿 | 偶尔出现可接受，频繁出现需要优化 |
| **冻结帧（Frozen Frame）** | 主线程单条任务 > 700ms | 界面完全冻结，用户无法交互 | 严重影响体验，必须修复 |
| **长任务（Long Task）** | 主线程执行超过阈值（默认 100ms，可配置） | 阻塞用户交互和界面更新 | 需要优化 |
...
- **iOS**：来源是 App Hang，由 SDK 的 `appHangThreshold` 控制。该参数**默认为 `nil`，即不开启**，必须显式配置后才会产生数据，建议设为 0.25–0.5 秒。参见 [iOS 高级配置](/zh/rum/sdk/ios/advanced-config)。
- **Android**：来源是主线程冻结，达到 5 秒才计入，阈值固定，无需也无法配置。纯 Android 应用不展示本指标，请改看 ANR 率。
- **Flutter**：一个应用同时覆盖两端，ANR 率与无响应时长率并列展示，两者阈值不同，不要直接横向比较。
</Note>
...
| 指标 | 采集字段 | 说明 |
|------|---------|------|
| UV | usr_anonymous_id / usr_id | 去重后的用户总数，详见下方口径说明 |
...
| 指标 | 采集字段 | 良好 | 中等 | 差 |
|------|---------|------|------|-----|
| 应用启动时间 | vital_duration（vital_type=app_launch、vital_app_launch_metric=ttid） | 2s 以内 | 4s 以内 | 超过 4s |
...
| 内存使用 | view_memory_average | 低于 200 MB | 低于 400 MB | 400 MB 以上 |
| 峰值内存 | view_memory_max | 低于 200 MB | 低于 400 MB | 400 MB 以上 |
| JS 线程帧率 | view_js_refresh_rate_avg | 55 FPS 以上 | 50 FPS 以上 | 低于 50 FPS |

### 流畅度指标

| 指标 | 定义 | 采集字段 |
|------|-----|---------|
| 慢帧占比 | 测量时间中被慢帧占用的比例（Android 分母为渲染总耗时，iOS 为视图停留时长，不可横比） | view_slow_frames_rate |
| 冻结帧数 | 主线程单条任务耗时超过 700ms 的次数；关闭长任务采集后恒为 0 | view_frozen_frame_count |
| 长任务数 | 主线程上耗时超过阈值的任务数量（默认 100ms，可配置） | view_long_task_count |
| 卡顿频率 | 冻结帧数除以视图停留时长，即平均每秒发生的冻结帧次数；关闭长任务采集后恒为 0 | - |
| JS 帧率（平均） | 该视图各次更新的 JS 线程平均帧率，低于 50 FPS 高亮；仅 React Native SDK 上报 | view_js_refresh_rate_avg |
| JS 帧率（最低） | 该视图任一次更新记录到的最低 JS 线程帧率，即用户遇到的最差卡顿，低于 30 FPS 高亮；仅 React Native SDK 上报 | view_js_refresh_rate_min |
...
| 无崩溃率 | 按会话统计：1 减去「至少发生一次崩溃的会话数 ÷ 总会话数」 | 建议保持在 99% 以上；一个会话崩溃多次只计一次。Android ANR 不属于崩溃，不影响该指标 |
| ANR 率 | ANR 会话数除以总会话数 | UI 线程阻塞超过 5 秒时触发（Android） |
| 无响应时长率 | 主线程无响应总时长除以视图总时长，换算为秒/小时 | iOS 统计 App Hang，需配置 appHangThreshold；Android 统计达到 5 秒的主线程冻结 |
