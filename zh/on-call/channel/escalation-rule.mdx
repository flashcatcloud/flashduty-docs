...
- **关闭（默认）**：所有故障均可能命中该策略
- **开启**：仅符合筛选条件的故障才会命中该策略，未匹配的故障将继续匹配后续策略。你可以添加多个条件组，组内条件为「且」关系，条件组之间为「或」关系；每个条件支持 **匹配** / **不匹配** 两种操作符，可选 Key 包括：
  - **集成来源（首条告警）**：按故障首条告警的来源集成匹配，候选项为当前协作空间可用的集成（含共享和专属）
  - **严重程度**：多选 Critical / Warning / Info
  - **标题**、**描述**：支持精确、通配、正则、CIDR、数值比较等多种匹配模式
  - **标签**：按告警标签键值匹配（如 `service=payment`），Key 可从近期告警数据中动态加载，也支持手动输入
...
<Tabs>
  ...
  <Tab title="群聊（触达群组）">
    ...
    <Note>
    **不使用上述 IM 平台？** 您可以选择任意一种机器人类型，将 Webhook 地址填写为您自己的服务端 URL（系统不会校验域名），然后在服务端实现对应的消息协议即可。详见[常见问题](/zh/on-call/quickstart/faq#custom-bot-notifications)。
    </Note>
  </Tab>
</Tabs>
