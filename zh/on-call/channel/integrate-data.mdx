...
<Frame caption="专属集成配置">
  <img src="https://docs-cdn.flashcat.cloud/images/png/436399.png" alt="专属集成" />
</Frame>

<Note>
专属集成归属于所在协作空间：**编辑、启停与删除**需要「协作空间管理」或「集成管理」功能权限，并同时满足该协作空间的数据权限——仅**账户管理员、协作空间创建者或协作空间管理团队**允许操作。不满足时对应按钮为禁用状态，悬停会说明原因。
</Note>
...
<Note>
超出任一维度的限制将返回 `429` 状态码，请等待后重试。如需扩大限制，请联系技术支持。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="订阅过期后集成会怎样？" icon="circle-question">
    订阅过期后，所有集成（含专属集成与共享集成）都会停止接收告警事件，集成中心顶部会常驻一条订阅过期提示，新建集成的卡片同时被禁用（悬停可见原因）。已配置的集成不会被删除，续费后自动恢复接入，无需重新配置。
  </Accordion>
</AccordionGroup>
