...
keywords: ["新奇故障", "异常检测", "AI识别", "故障分析", "智能告警"]
---
...
<Tip>
**相似度阈值可调**：默认阈值为 **90%**，可在协作空间的智能告警聚合配置中，通过聚合组的 `i_score_threshold` 参数进行调整，取值范围为 `0.5` 到 `1.0`。阈值越高，判定为相似故障的条件越严格，新奇故障的识别会更敏感。
</Tip>

除了 **新奇（rare）** 分类外，系统在内部还维护了一个 **高频（frequent）** 分类：当过去 30 天内发生过 **21 次及以上** 相似故障时，系统会标记为高频故障。该分类当前仅作为内部元数据保存，尚未在控制台 UI 中呈现。
