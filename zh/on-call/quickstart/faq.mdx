---
title: "Flashduty On-call 常见问题"
sidebarTitle: "常见问题"
description: "了解使用 Flashduty On-call 中最常见的问题"
keywords: ["常见问题", "FAQ", "On-call", "故障排查", "使用帮助"]
---
...
<AccordionGroup>
  ...
  <Accordion title="On-call 订阅过期后会怎样？">
    订阅过期后，平台冻结**新建**类操作，已有的数据和配置不受影响：

    - **告警停止接入**：所有集成不再接收新的告警事件，也不会再产生新故障
    - **历史数据仍可用**：历史告警仍可查看与处理，已有故障仍可正常处理
    - **新建入口被冻结**：新建协作空间、值班表、手动创建故障、创建作战室等入口会被禁用，悬停可查看原因
    - **页面常驻提示**：告警列表、故障列表与侧边栏会常驻显示订阅过期提示
    - **续费后自动恢复**：续费成功后自动恢复告警接入，无需重新配置
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion id="custom-bot-notifications" title="我没有飞书/钉钉/企微/Telegram，如何接收机器人通知？">
