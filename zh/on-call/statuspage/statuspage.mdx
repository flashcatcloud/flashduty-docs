...
系统通过回放事件时间线中的组件状态变更来精确计算每个组件的影响时段。**维护**事件遵循独立的计算规则：组件仅在维护实际执行窗口内（状态变为"进行中"到关闭）被标记为"维护中"，尚未开始的计划维护不会影响可用性图表与日历展示。被设为"完全隐藏"的组件和分组不参与可用性统计；被设为"隐藏可用性"的组件和分组参与计算但不对外展示统计结果。你可以在状态页设置中选择可用性统计的展示模式：同时展示图表与百分比、仅展示图表、或完全不展示。
</Accordion>
...
<Accordion title="自定义域名如何获得 HTTPS 证书？" defaultClose>
配置自定义域名后，Flashduty 会自动为域名**签发、续期并发布 HTTPS 证书**。签发前需要完成两类 DNS 校验记录，配置完成后在状态页设置中点击**检查配置**即可查看校验状态：

- **访问入口记录**（CNAME）：将域名指向托管访问入口，大陆访问入口为 `cn.statuspage.flashduty.com`，国际访问入口为 `statuspage.flashduty.com`。
- **证书托管记录**（dns-01 委派）：在 `_acme-challenge` 前缀下添加 CNAME 记录，将证书校验委派给 Flashcat 托管 DNS，具体记录值以**检查配置**显示为准。

两类记录均校验通过后，系统开始签发并发布证书，域名即可通过 HTTPS 访问。

如果域名仍指向旧兼容入口 `statuspage.flashcat.cloud`，只要证书托管记录校验通过，系统也会提前签发并发布证书，之后将 DNS 切换到托管访问入口时不会出现 HTTPS 中断。
</Accordion>
