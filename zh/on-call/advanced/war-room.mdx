  ...
  <Step title="创建作战室">
    ...
    <Tabs>
      ...
      <Tab title="IM 应用">
        ...
        使用飞书应用通知时，故障卡片底部的 **👥 创建作战室** 按钮由[通知模板](/zh/on-call/configuration/templates)中的开关控制：在 **模板管理** 中编辑飞书应用模板，于 **附加信息** 页签的卡片字段面板中开启 **创建作战室**（默认关闭），满足建群条件时卡片底部才会展示该按钮；卡片底部的 **更多操作** 已改为下拉菜单（自定义操作、暂缓、取消认领），作战室不再位于其中。
      </Tab>
      <Tab title="移动端 App">
...
<Warning>
若作战室所用的 IM 集成事后被禁用或删除，控制台与移动端的 **添加关注人员进入作战室**、**解散作战室** 都会隐藏。此时请由账号所有者或账号管理员在故障详情页打开 **查看作战室** 弹窗执行 **确认解除关联**：该操作只解除故障与原群聊的关联，**不会解散原群聊**；时间线记录为「发起解除作战室关联」。如需重新建群，请先确认原群已妥善处理。
</Warning>
...
- **添加关注人员进入作战室 / 解散作战室**：同样位于 **更多操作** 中，故障关闭后仍可用（便于复盘），解散需二次确认；但要求作战室所在的 IM 集成仍可用。集成被禁用或删除后，这两项从 **更多操作** 中消失，需由管理员在控制台的故障详情页解除关联；概览页的 **作战室** 入口卡片仍会保留，点击后提示「原集成已禁用或删除。请管理员在 PC 端故障详情页解除关联，原群聊不会解散。」
