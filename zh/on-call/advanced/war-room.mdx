...
<Note>
  时间线中的作战室操作记录会展示群聊名称（支持一键复制）；当 IM 集成提供分享链接时，记录中还附有 **进入作战室** 直达链接，点击即可在新窗口打开群聊。
</Note>
...
- **创建成功弹窗**：创建成功后弹出成功提示，展示群聊名称；当 IM 集成提供分享链接时，同时展示 **邀请链接**（可分享给其他成员加入作战室），两者均可一键复制，也可点击 **进入作战室** 直接打开群聊。群聊名称与邀请链接仅在该弹窗中展示一次，关闭后无法再次查看；集成未提供分享链接时，弹窗会提示您在对应 IM 应用中搜索群聊名称进入
- **添加关注人员进入作战室 / 解散作战室**：同样位于 **更多操作** 中。故障关闭后仍可用（便于复盘），且不依赖集成列表——即使 IM 集成事后被关闭，仍可继续添加关注人员或解散已创建的作战室；解散需二次确认
- **操作权限**：**更多操作** 中的创建、添加关注人员、解散操作均要求故障更新（IncidentUpdate）权限；无权限时操作不会执行并提示无权限
