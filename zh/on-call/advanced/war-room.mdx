  ...
  <Step title="创建作战室">
    ...
    <Tabs>
      ...
      <Tab title="IM 应用">
        ...
        使用飞书 / 钉钉 / Slack 应用通知时，故障卡片底部的 **👥 创建作战室** 按钮由[通知模板](/zh/on-call/configuration/templates)中的开关控制：在 **模板管理** 中编辑对应渠道的应用模板，于 **附加信息** 页签的卡片字段面板中开启 **创建作战室**（默认关闭），满足建群条件时卡片底部才会展示该按钮；卡片底部的 **更多操作** 已改为下拉菜单（自定义操作、暂缓、取消认领），作战室不再位于其中。
      </Tab>
      <Tab title="移动端 App">
...
作战室与 **AI SRE** 深度联动：作战室创建后，AI SRE 会自动进行一轮初步诊断并把结论回贴到群里；你也可以在群聊中 **@ AI SRE** 继续追问或发起新的排查。详见 [AI SRE 产品介绍](/zh/ai-sre)与 [IM 平台](/zh/ai-sre/im)。
