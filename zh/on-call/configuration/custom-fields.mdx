---
title: "自定义字段"
description: "为故障添加自定义元数据，记录特定业务信息"
keywords: ["自定义字段", "元数据", "故障属性", "业务信息", "字段配置"]
---
...
<Note>
自定义字段数量受账户配额限制：默认最多 **15 个**，部分账户配额更高（最多 **50 个**）或不限量，以页面显示的额度为准。达到限额时创建按钮会被禁用并提示“已达到创建上限”。
</Note>
...
<Card title="自定义表单" icon="file-lines" href="/zh/on-call/configuration/custom-forms">
在创建、认领和关闭故障时收集补充信息
</Card>
</CardGroup>
