...
在分派策略中配置群通知的机器人 Webhook 地址时，除了**本空间及本团队**已使用过的地址，还可以切换到**其他团队（告警将发往对方群聊）**，从其他团队的协作空间中选择 Webhook 地址：

- 选择后，告警通知将发送到**对方团队的群聊**，适用于跨团队协同处理故障的场景
- 选中其他团队的地址时，系统会弹出确认提示，告知该地址来自哪个团队的协作空间；地址的**别名会自动带来源前缀**（可修改），并在列表中标记为**跨团队**，便于识别
- 如果您选择了在本空间及本团队从未使用过的地址，系统会再次弹出确认框，列出这些地址并说明告警将发送到对应团队的群聊，确认后才会生效
