...
编辑飞书应用模板时，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、严重程度、处理人员、故障详情按钮、AI 分析等系统字段是否渲染在卡片底部。
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮、AI 分析等系统字段是否渲染在卡片底部；钉钉应用同样提供 **创建作战室** 开关（对应字段 `dingtalk_app_war_room_enabled`，默认**关闭**，开启后需保存模板生效）。
</Tab>
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮、AI 分析等系统字段是否渲染在卡片底部；Slack 应用同样提供 **创建作战室** 开关（对应字段 `slack_app_war_room_enabled`，默认**关闭**，开启后需保存模板生效）。
</Tab>
