---
title: 通知模板
description: 通知模板用于控制故障通知的内容格式，系统在分派故障时，使用模板渲染故障信息并发送通知。
keywords: ["通知模板", "模板配置", "消息格式", "模板变量", "通知内容"]
...
- 使用示例：查看对应的 `_test.go` 文件，如 [date_test.go](https://github.com/flashcatcloud/sprig/blob/master/date_test.go)
</Accordion>
</AccordionGroup>
...
编辑飞书应用模板时，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮、AI 分析共 7 个系统字段是否渲染在卡片底部。

其中 **创建作战室** 开关在飞书应用、钉钉应用、Slack 应用模板中均可配置（分别对应字段 `feishu_app_war_room_enabled`、`dingtalk_app_war_room_enabled`、`slack_app_war_room_enabled`，默认**关闭**，开启后需保存模板生效）。开关文案即 **创建作战室**，面板中的说明为「开启后，满足建群条件时，在卡片底部展示独立的创建作战室按钮。」

- **开启**：故障未关闭、未创建作战室且存在处理人员时，对应渠道的故障卡片底部会展示独立的 **👥 创建作战室** 按钮，点击按钮即可发起建群
- **关闭**：卡片底部不展示该按钮

**故障关闭后保留自定义操作** 开关在飞书应用、钉钉应用、Slack 应用、Microsoft Teams 模板中均提供，企业微信应用模板不提供（面板不渲染该开关）。对应字段 `incident_card_closed_action_apps` 为字符串数组，存放已开启该能力的渠道 key（如 `["feishu_app"]`），默认**关闭**（数组为空即关闭），开启后需保存模板生效。

- **开启**：面板说明为「已开启，故障关闭后 3 天内可从消息卡片中执行已配置的自定义操作，超过有效期请进入详情页执行。」故障关闭后 3 天内已配置的自定义操作仍可从消息卡片直接执行，超过 3 天需进入故障详情页执行
- **关闭**：面板说明为「已关闭，故障关闭后将隐藏消息卡片中的自定义操作入口，仍可进入详情页执行。」故障关闭后消息卡片不再展示自定义操作入口，仍需进入故障详情页执行

加上上文 7 个卡片字段开关，飞书应用、钉钉应用、Slack 应用模板的 **附加信息** 面板各有 **9 个开关**，Microsoft Teams 模板 **7 个**，企业微信应用模板 **6 个**（见下方各渠道模板说明）。

同时，飞书应用卡片底部的操作区已调整：**更多操作**（占位文案即「更多操作」）改为下拉选择菜单，菜单内为自定义操作、暂缓时长与取消认领等选项，作战室按钮不再位于该菜单中。
</Tab>
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮、AI 分析共 7 个系统字段是否渲染在卡片底部；钉钉应用同样提供 **创建作战室** 开关（对应字段 `dingtalk_app_war_room_enabled`，默认**关闭**，开启后需保存模板生效）与 **故障关闭后保留自定义操作** 开关（记入 `incident_card_closed_action_apps` 渠道 key 数组，默认**关闭**），**附加信息** 面板共 **9 个开关**。
</Tab>
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮共 6 个系统字段是否渲染在卡片底部。企业微信应用模板不提供 **AI 分析**、**创建作战室**、**故障关闭后保留自定义操作** 开关，**附加信息** 面板共 **6 个开关**。
</Tab>
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮、AI 分析共 7 个系统字段是否渲染在卡片底部；Slack 应用同样提供 **创建作战室** 开关（对应字段 `slack_app_war_room_enabled`，默认**关闭**，开启后需保存模板生效）与 **故障关闭后保留自定义操作** 开关（记入 `incident_card_closed_action_apps` 渠道 key 数组，默认**关闭**），**附加信息** 面板共 **9 个开关**。
</Tab>
...
与飞书应用一致，**正文内容** 旁的 **附加信息** 页签提供卡片字段开关面板（面板提示「以下内容由系统渲染，可通过开关控制是否展示在消息底部。」），用于控制协作空间、暂缓截止时间、严重程度、聚合告警数、处理人员、详情按钮共 6 个系统字段是否渲染在卡片底部，另有 **故障关闭后保留自定义操作** 开关（记入 `incident_card_closed_action_apps` 渠道 key 数组，默认**关闭**），**附加信息** 面板共 **7 个开关**。Microsoft Teams 模板不提供 **AI 分析**、**创建作战室** 开关。
</Tab>
</Tabs>
...
<Tab title="企业微信机器人">
企业微信机器人支持 Markdown v1 和 Markdown v2 两种格式。已有模板默认继续使用 v1；您可以按模板开启 v2，不会影响其他模板。

| 对比项 | Markdown v1 | Markdown v2 |
| :--- | :--- | :--- |
| 启用方式 | 默认格式，无需设置 | 在企业微信机器人模板中开启 **使用 Markdown v2 格式** |
| Markdown 能力 | 使用企业微信旧版 Markdown 语法（[语法限制](https://developer.work.weixin.qq.com/document/path/91770#markdown%E7%B1%BB%E5%9E%8B)） | 支持标准 Markdown，包括标题、列表和表格 |
| 链接打开方式 | 通常在企业微信内置浏览器中打开 | 通常使用系统默认浏览器打开；最终行为取决于企业微信客户端版本和设置 |
| 消息长度 | 最大 4000 字节，超出后截断并保留详情链接 | 最大 4096 字节；运行时超出限制时自动发送精简消息 |
| @ 提醒 | 沿用分派策略中的机器人 @ 设置 | 沿用相同设置；正文发送成功后再发送 @ 提醒，正文失败时不发送 |

### 开启 Markdown v2

<Steps>
<Step title="进入模板管理">
打开 **模板管理**，创建自定义模板或复制已有模板。
</Step>
<Step title="选择企业微信机器人">
在模板内容中选择 **企业微信机器人**，开启 **使用 Markdown v2 格式**。
</Step>
<Step title="编辑并保存模板">
使用标准 Markdown 编辑内容并保存。该设置随模板保存，仅对使用此模板的通知生效。
</Step>
</Steps>

<Note>
关闭开关后，模板恢复使用 Markdown v1 格式，现有 v1 模板内容和发送逻辑保持不变。
</Note>

<Warning>
Markdown v2 按 UTF-8 字节计算，消息最大为 **4096 字节**。中文通常占 3 个字节。模板中的动态字段可能使实际通知超过限制；发生这种情况时，Flashduty 不会丢弃通知，而会发送标有“原消息内容过长，已精简展示”的精简消息，并保留故障编号、标题、协作空间、严重程度、触发时间、告警对象、处理人员、详情和认领链接。
</Warning>
...
> **严重程度：** {{.IncidentSeverity}}
> **触发时间：** {{date "2006-01-02 15:04:05" .StartTime}}
{{if gt .AlertCnt 1}}> **聚合告警：** {{.AlertCnt}}条{{end}}
{{if .Labels.resource}}> **告警对象：** {{(joinAlertLabels . "resource" ", ")}}{{end}}
{{if .Labels.body_text}}> **故障描述：** {{.Labels.body_text}}{{else if .Description}}> **故障描述：** {{.Description}}{{end}}
{{if gt (len .Responders) 0}}> **处理人员：** {{range .Responders}}@{{.PersonName}} {{end}}{{end}}
