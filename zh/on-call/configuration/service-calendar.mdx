---
title: 服务日历
description: 服务日历用于定义工作日和休息日，配合分派策略或静默规则实现按日期类型的差异化处理。典型场景如证券交易系统，仅在交易日（工作日）触发告警通知。
keywords: ["服务日历", "工作日", "休息日", "节假日", "差异化处理"]
...
进入 **配置中心 → 服务日历**，点击 **新增日历**。

| 配置项 | 说明 |
| --- | --- |
| **日历名称** | 必填，长度为 1~40 个字符，建议按业务维度命名，如「结算业务系统」 |
| **管理团队** | 必填。日历归属于该团队，团队成员将获得编辑该日历的权限 |
| **日历描述** | 概述业务特性，便于团队成员快速了解 |

<Note>
每个账户默认最多可创建 5 个服务日历。如需创建更多，请联系 Flashduty 技术支持。
</Note>

<Tip>
新建日历默认全部为工作日。创建成功后进入日历详情页关联国家节假日，系统会自动将法定假日标记为休息日，您也可以手动调整。
</Tip>
...
| --- | --- |
| **修改基础信息** | 日历名称、描述、管理团队 |
| **关联节假日** | 在日历详情页关联国家节假日日历，系统自动将法定假日标记为休息日，可随时调整或取消关联 |
