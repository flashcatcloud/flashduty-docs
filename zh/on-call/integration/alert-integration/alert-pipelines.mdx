...
Pipeline 支持在集成层配置静默规则，与协作空间的静默策略功能一致，但生效位置更靠前。适用于需要在数据源头统一屏蔽特定告警的场景。

<!-- REVIEW: 功能概述 — 补充集成层静默与协作空间层静默的差异说明，以及典型使用场景 -->

### 集成层与协作空间层的区别

| 对比项 | 集成层（Pipeline）静默 | 协作空间层静默 |
| :--- | :--- | :--- |
| 生效层级 | 集成层 | 空间层 |
| 适用场景 | **全局性**的静默逻辑，如计划维护期间屏蔽所有告警 | 特定协作空间的静默规则 |

<!-- REVIEW: 请验证上述对比内容是否准确，并补充更多差异点 -->

### 配置说明

<!-- REVIEW: 以下为配置步骤骨架，请验证并补充具体操作截图和字段说明 -->

<Steps>
<Step title="进入集成的告警处理页签">
进入 **集成中心** => 选择已创建的集成 => **告警处理** 页签
</Step>
<Step title="添加静默规则">
点击添加规则，选择"告警静默"动作类型
</Step>
<Step title="配置静默条件和时间">
设置匹配条件（严重程度、标题、标签等）和静默时间窗口
</Step>
</Steps>
