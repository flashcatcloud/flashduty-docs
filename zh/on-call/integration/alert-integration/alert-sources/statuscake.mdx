...
description: "通过联系组 Webhook 将 StatusCake Uptime 测试的宕机和恢复通知同步到 Flashduty On-call。"
keywords: ["告警集成", "StatusCake", "Uptime", "Contact Group", "Webhook", "TestID"]
---

通过 StatusCake 联系组（Contact Group）的 Webhook，把 Uptime 测试的宕机（`Down`）和恢复（`Up`）通知同步到 Flashduty On-call。每个 StatusCake 测试对应一条 Flashduty 告警：测试宕机时触发，重复宕机通知合并到同一条告警，测试恢复时关闭这条告警。
...
1. 登录 StatusCake，进入 **Alerting → Contact Groups**，点击 **New Contact Group**
2. **Group Name** 可填写 `Flashduty`
3. 将 Flashduty 集成的完整推送地址粘贴到 **Webhook URL**
4. **Webhook Method** 选择 **POST**
5. 点击保存
...
编辑需要接入的 Uptime 测试，在 **Contact Groups** 中选择刚创建的 `Flashduty` 联系组并保存。一个联系组可以关联多个测试。
...
不会。联系组开启了重复提醒（Repeat Alerts）时，测试持续宕机期间的每次通知都带相同的 `TestID`，会合并到同一条告警中。

</Accordion>

<Accordion title="SSL、PageSpeed、域名等其他类型的检查可以接入吗？">

本集成按 Uptime 测试（含 Heartbeat）的 `Down` / `Up` 通知设计。其他类型的检查如果也通过联系组推送，只有请求中带 `TestID`，且 `Status` 为 `Down` 或 `Up` 时才会被接收，否则 Flashduty 返回参数错误。

</Accordion>

<Accordion title="联系组测试成功，但真实告警没收到？">

联系组的测试只验证地址可达。请确认测试已关联该联系组、测试没有暂停，并且宕机持续时间达到了测试的确认次数（Confirmation）。
...
- **告警没有恢复**：确认测试恢复时仍关联同一个联系组；恢复通知与宕机通知的 `TestID` 必须相同
