...
<Warning>
不要在规则中过滤掉恢复状态，否则告警无法自动恢复。例如只匹配 `"state": {"value": ["ALARM"]}`、`"complianceType": ["NON_COMPLIANT"]`、Security Hub 的 `"Workflow": {"Status": ["NEW"]}` 或 Inspector 的 `"status": ["ACTIVE"]`，都会丢掉恢复事件。
</Warning>
...
- 有专门处理的来源使用发现或告警的标题，并附加 `finding_id`、`finding_arn`、`event_arn`、`config_rule_name`、`insight_id` 等来源字段，`resource` 取受影响资源
