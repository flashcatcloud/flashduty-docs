...
<Note>
订阅可以开启或关闭 **Raw message delivery**，两种方式都能接收。开启后 SNS 不再附带签名，只能依靠推送地址中的 `integration_key` 鉴权，建议保持关闭。
</Note>
...
- **Flashduty 返回签名错误**：请求不是 SNS 发出的，或消息在转发中被改写。请让 SNS 直接推送到 Flashduty
