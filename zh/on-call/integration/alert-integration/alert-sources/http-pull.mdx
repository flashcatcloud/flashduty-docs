---
title: "HTTP 拉取告警 (HTTP Pull)"
description: "Flashduty 主动按周期访问外部 HTTP 接口，将返回的告警事件拉取入库；适用于无法主动推送告警的系统，或希望将拉取与推送解耦的场景。"
...
「专属集成」（在某个协作空间的 **配置 → 集成数据 → 专属集成** 中创建）会自动绑定到所在空间，不需要单独配置默认路由。
