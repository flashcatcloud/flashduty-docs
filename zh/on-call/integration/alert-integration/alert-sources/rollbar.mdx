...
在 Webhook 设置页添加规则，每条规则对应一种触发条件。建议添加以下规则：
...
规则中可以按环境、等级等条件过滤，只推送需要值班处理的 Item。**Every occurrence**（`occurrence`）会在每次错误发生时推送一次，同样合并到该 Item 的告警中，但事件量大，一般不建议开启。

<Warning>
必须添加 **Item resolved** 规则，否则 Flashduty 中的告警不会随 Item 解决而恢复。规则的 Payload 格式保持默认的 JSON，Flashduty 不接收 XML 格式。
</Warning>
...
- **告警没有恢复**：确认添加了 **Item resolved** 规则。将 Item 设为 **Muted** 不会发送 Webhook，对应告警需在 Flashduty 中手动关闭
