---
title: "标签映射 API"
description: "配置标签映射 API，当告警事件到达时，Flashduty 将自动调用您配置的外部 API 获取增强标签，实现告警信息的动态丰富和关联。通过此功能，您可以将 CMDB、HR 系统等外部数据源的信息自动附加到告警上。"
keywords: ["标签映射", "API", "告警增强", "CMDB", "标签丰富", "外部数据源"]
---
