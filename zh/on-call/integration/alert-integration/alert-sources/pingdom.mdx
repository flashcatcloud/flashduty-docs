...
1. 登录 My Pingdom，进入 **Integrations → Integrations**
2. 点击右上角 **Add new**，类型选择 **Webhook**
3. 名称可填写 `Flashduty`
...
- **Uptime 检查**：进入 **Monitoring → Uptime**，编辑需要告警的检查，在集成列表中勾选 `Flashduty`
...
- **收到测试消息**：My Pingdom 中的测试 Webhook 的 `description` 为 `test`，只用于验证连通性；如果生成了告警，请在 Flashduty 中手动关闭
