...
请求体、标签与描述都有硬上限，超出后按下表处理：

| 项目 | 上限 | 超出时的表现 |
| :--- | :--- | :--- |
| 请求体 | 1 MiB | 直接返回参数错误，不再解析内容 |
| `tags` 数量 | 50 个 | 只保留前 50 个非空标签 |
| `tags` 单个值 | 128 字节 | 按 128 字节截断 |
| 其它标签值 | 8192 字节 | 按 8192 字节截断 |
| 告警描述 | 2048 字符 | 按错误信息、响应状态、响应时间、开始时间、结果链接的顺序拼接后截断末尾 |
