...
## 三、常见问题
<Warning>
同一时间仅支持在一个 IM 集成中开启作战室功能。如果你已在其他 IM 集成（如钉钉、飞书、企业微信）中启用了作战室，需要先在该集成中关闭后，才能在当前 Slack 集成中开启。
</Warning>
...
点击 **重新授权** 链接后，系统会跳转到 Slack 授权页面，请求作战室功能所需的额外权限（包括频道管理、消息读写、用户信息读取等）。完成授权后，页面会自动返回 Flashduty，此时即可正常开启作战室。
