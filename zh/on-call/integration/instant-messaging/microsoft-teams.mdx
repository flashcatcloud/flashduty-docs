...
<Step title="解除团队关联">
如需解除该 Team 的关联，请在同一频道中发送 `@Flashduty unlinkTeam {ID}`。在响应卡片中选择 **Unlink in FlashDuty**，然后在 Flashduty 控制台确认解除。
</Step>
</Steps>
...
<Step title="解除群聊关联">
如需解除该群聊的关联，请在同一群聊中发送 `@Flashduty unlinkChat {ID}`。在响应卡片中选择 **Unlink in FlashDuty**，然后在 Flashduty 控制台确认解除。
</Step>
</Steps>
...
<Step title="解除个人账号关联">
如需解除您的 Teams 账号关联，请在同一个人聊天中发送 `unlinkUser {ID}`。在响应卡片中选择 **Unlink in FlashDuty**，然后在 Flashduty 控制台确认解除。
</Step>
</Steps>
...
请在原始 Teams 上下文中使用对应指令：在 Team 频道中使用 `unlinkTeam {ID}`，在群聊中使用 `unlinkChat {ID}`，或在个人聊天中使用 `unlinkUser {ID}`。指令会打开 Flashduty 中对应的解除关联流程。
</Accordion>
</AccordionGroup>
