...
以下 AI SRE 控制项在集成配置页面的 **增强功能** 模块中配置，默认均为开启：

- **自动发起 AI 故障分析**：需先开启作战室才会出现；作战室创建成功后，自动发起一次 AI 故障分析。
- **允许在 IM 中使用 AI SRE**：无需开启作战室即可配置，同时控制该集成的群聊 @ AI SRE 与机器人私聊两个入口；关闭后对应入口不会触发 AI SRE 回复。

企业微信的两种应用模式对 AI SRE 入口的支持不同。**允许在 IM 中使用 AI SRE** 在两种模式下都会显示，集成详情预览页也会展示其状态：

| 应用模式 | 群聊 @ AI SRE | 机器人私聊 |
| :--- | :--- | :--- |
| **企业自建应用** | 支持 | 支持 |
| **第三方应用** | 不支持，需切换为 **企业自建应用** 模式 | 支持 |

第三方应用模式下，该开关的说明文案为「开启后，可在机器人私聊中直接发起对话；群聊 @ AI SRE 仅企业自建应用支持」。

如果账户尚未启用 AI SRE，以上配置暂不生效。

## 四、配置 AISRE

如需在企业微信中使用 AISRE，请在 Flashduty 企业微信集成中确认 **允许在 IM 中使用 AI SRE** 已开启（无需开启作战室），然后额外创建一个智能机器人，并使用 API 模式接入 Flashduty。智能机器人的凭据在 **企业自建应用** 模式的集成配置页面填写。
