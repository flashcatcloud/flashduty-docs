...
以下为企业微信 IM 集成在开启 AISRE（包含基础通知、作战室、AI SRE 对话和 AI 生成复盘）时需要确认的完整配置与授权清单。企业微信自建应用后台不会把这些能力全部展示为统一的 scope 名称，请在应用可见范围、API 接收、可信域名、可信 IP 和接口调用许可中逐项确认。

| 配置或接口能力 | 用途 |
| :--- | :--- |
| 应用可见范围 | 覆盖需要接收通知、完成账号关联、参与 AI SRE 对话或加入作战室的成员 |
| 企业微信接口调用许可 | 第三方应用模式下使用免密登录、消息发送和回调处理的前置授权 |
| 应用消息发送与卡片更新接口 | 发送基础通知并更新交互卡片状态 |
| API 接收 `Token` 和 `EncodingAESKey` | 接收、校验和解密企业微信应用回调 |
| 智能机器人 `Token` 和 `EncodingAESKey` | 接收用户 @ 机器人或单聊提问、返回 AI SRE 流式回复，并解密机器人消息中的图片和文件 |
| 可信域名和前端可信域名 | 支持企业微信内免密登录、账号关联和从通知卡片打开 Flashduty 页面 |
| 可信 IP | 允许 Flashduty 服务调用企业微信接口 |
| 手机号或邮箱换取 `userid` | 用于一键关联用户，以及创建或更新作战室时邀请成员 |
| `openuserid` 转 `userid` | 用于企业微信智能机器人消息的发送者身份识别 |
| 应用群聊 `appchat` 创建、更新、查询和发送接口 | 创建和维护作战室群聊，并向作战室发送 AI SRE 分析消息 |
...
> 配置作战室前，请确认自建应用已满足页面开头列出的 [AISRE 所需配置与授权](#aisre-permissions)。
