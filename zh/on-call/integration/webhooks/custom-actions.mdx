---
title: "自定义操作"
description: "配置故障自定义操作，允许您在故障排查期间，快速调用外部接口，实现故障自愈、信息丰富等任何自定义操作"
keywords: ["自定义操作", "Webhook", "故障自愈", "外部接口", "自动化"]
---
...
4. 配置 管理团队，选择管理该集成的团队，只有团队成员可以编辑此集成配置
5. 配置 协作空间，可以配置多个，但每个协作空间至多添加 **5** 个自定义操作
6. 配置 Endpoint、自定义 Headers
7. **TLS 验证** 默认启用。如果目标服务使用自签名证书或内部 CA 签发的证书，可以关闭 **TLS 验证** 开关，系统将不校验服务端证书的合法性
...
<Warning>
每个协作空间最多关联 5 个自定义操作。如果某个协作空间已达到上限，在添加新的自定义操作时将无法选择该空间。
</Warning>
