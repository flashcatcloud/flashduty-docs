...
keywords: ["Jira", "工单同步", "Webhook", "Issue跟踪", "故障关联"]
---
...
- **严重程度映射**：如果选择的事务类型不支持优先级字段，则无法配置该映射关系。每行的 **Jira 优先级**（结果字段）为必填项；删除了全部映射行，或因事务类型不支持优先级字段导致该区块未渲染时，视为未配置严重程度映射，可以直接保存集成配置。
- **状态映射**：为 FlashDuty 故障的待处理、处理中、已关闭三个处理进度分别选择对应的 Jira 状态，状态列表会根据所选的项目和事务类型从 Jira 中获取。全部留空时使用历史默认映射（预览中显示为「使用默认映射」）；一旦配置了其中任意一项，其余两项也必须选择，缺省时无法保存集成配置。
- **自定义字段映射**：可以选择将故障的故障 ID、故障链接、某些标签或所有标签以及自定义字段内容同步至 Jira 的字段中（仅支持文本类型的字段）。每行的 **Key**（故障字段）与 **结果字段**（Jira 字段）均为必填项，缺省时无法保存集成配置。
...
下表为默认状态映射，您也可以在集成配置的 **状态映射** 区块中自定义覆盖。
