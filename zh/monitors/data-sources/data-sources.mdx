---
title: "数据源管理"
description: "配置和管理 Monitors 的数据源，包括 Prometheus、Elasticsearch、Loki、ClickHouse、MySQL、Oracle、PostgreSQL、SLS 等类型"
keywords: ["数据源", "Prometheus", "Elasticsearch", "Loki", "ClickHouse", "MySQL", "Oracle", "PostgreSQL", "SLS"]
---
...
| 配置项 | 说明 |
|--------|------|
| **名称** | 数据源的唯一标识名称，告警规则通过名称（支持通配符）关联数据源 |
| **备注** | 可选的补充说明 |
...
<Warning>
数据源名称在告警规则中用于关联（支持通配符匹配）。修改数据源名称可能导致已有告警规则失效，请谨慎操作。
</Warning>
...
这些数据源的连接配置与 Prometheus 类似，包括服务地址、认证和 TLS 配置。具体参数请参考创建表单中的说明。
