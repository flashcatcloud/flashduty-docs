---
title: "ElasticSearch"
description: "配置 ElasticSearch 数据源的告警规则，支持 SQL 聚合查询"
keywords: ["ElasticSearch", "日志告警", "SQL查询", "日志监控", "告警规则"]
---
...
无数据告警的**恢复方式**在「之前查到过数据，现在查不到了，就告警」这张检测卡内配置，决定该告警如何结束：

| 恢复方式 | 说明 |
|------|------|
| **数据重新出现后自动结束** | 默认方式。一旦该 `host_name` 重新出现在查询结果中，告警自动结束 |
