...
默认选择逻辑是：**优先使用当前成员可用的在线 BYOC Runner；否则使用云端 Sandbox。** 可用 Runner 包括共享范围（即账户级）的 Runner，以及当前成员所属团队下的团队级 Runner。您也可以在会话输入框的环境选择器中固定使用云端 Sandbox，或固定使用某个具体 Runner。
...
| 范围 | 是 | 共享范围的模板在整个账户内可见；团队范围的模板仅对该团队成员可见和可编辑。 |
...
| 环境变量 | 否 | `.env` 格式（`KEY=value`，每行一条，支持带引号的多行值），最大 32 KB，界面会实时显示已用字节数。平台保留键不能在这里设置，见下方说明。 |
...
<Note>
**平台保留键不能设置**：下列变量由平台注入，写进模板会在保存时被拒绝。

- 精确匹配：`FLASHDUTY_APP_KEY`、`FLASHDUTY_BASE_URL`、`FLASHDUTY_DOCS_URL`、`GH_TOKEN`、`GH_PROMPT_DISABLED`、`PATH`、`HOME`；
- GitLab CLI（`glab`）会读取的认证 / 主机 / TLS 配置项：`GITLAB_TOKEN`、`GITLAB_ACCESS_TOKEN`、`OAUTH_TOKEN`、`GITLAB_HOST`、`GL_HOST`、`GITLAB_URI`、`GITLAB_API_HOST`、`GITLAB_SSH_HOST`、`GITLAB_SUBFOLDER`、`CA_CERT`、`SKIP_TLS_VERIFY`；
- 前缀匹配：`GIT_CONFIG_`、`GLAB_`、`GH_TOKEN_`（最后一个覆盖 `GH_TOKEN_1`、`GH_TOKEN_2` 这类多安装的 GitHub 令牌）。

名单以服务端实际返回的报错为准。其中 `FLASHDUTY_DOCS_URL` 由平台在**每次 bash 执行**时注入，指向产品文档压缩包的下载地址，供内置的 `flashduty-docs` 技能按需下载文档——它不由模板提供，也无法被模板覆盖。
</Note>
...
| 范围 | 是 | 共享范围对整个账户可见；团队范围仅对该团队成员可见和可编辑。范围在创建时固定，之后不能修改；如需迁移，请在目标范围新建 Environment 并重新部署 Runner。见[作用域](#作用域)。 |
...
<Tabs>
  ...
  <Tab title="Docker">
    ...
    命令中的 `--restart=always` 会在宿主机重启后自动拉起容器；如果容器已经在运行，可执行 `docker update --restart=always flashduty-runner` 补上同样的重启策略，无需重建容器。
  </Tab>
  <Tab title="手动 / macOS">
    先安装二进制，再手动启动进程：
...
Runner 可以自己安装它用到的命令行工具，不需要您在主机上预先准备好：

- **手动安装**：`flashduty-runner tool ensure gh`、`glab`、`uv`、`node`。如果 `PATH` 里已经有同名二进制，Runner 直接复用它、不覆盖；否则从上游下载固定版本的归档，校验内置的 SHA-256 后原子安装到 Runner 自己的 tools 目录。
- **自动安装**：启动 stdio MCP 连接器之前，Runner 会先安装该连接器需要的运行时——`uv`（提供 `uvx`）或 `node`（提供 `npx`）——所以主机上没有这两个运行时也能使用这类连接器。

两条路径都需要 Runner 所在主机**能够出网**下载归档。内网 / 离线环境可以设置 `FLASHDUTY_RUNNER_TOOL_MIRROR_URL` 指向私有镜像，镜像需与官方保持同样的 `<name>/<version>/<asset>` 目录结构；该变量一旦设置就只走镜像，不会回退到官方源。

Runner 自己安装的工具在 `PATH` 中排在主机命令之前，因此同名命令优先使用 Runner 安装的版本，主机上的其他命令仍照常可用。
...
权限配置约束的是**命令**；Agent 的**文件读写工具**另有独立的路径语义，与 `bash` 的行为保持一致：

- **相对路径**：限制在 workspace 根目录内，并带符号链接逃逸保护——通过相对路径只能读写 workspace 内的文件。
- **绝对路径**：按设计直接落到宿主机文件系统，与 `bash` 一致。

因此真正划定文件访问边界的，是 Runner 运行的 **OS 用户** 与部署隔离（systemd 的用户与目录保护、Docker 的挂载与容器边界），而不是权限配置文件。
...
1. **会话绑定不回退**（上面两条说的就是它）：一条会话首次发送时锁定的环境不再变化。绑定的 Runner 之后离线、被删除，AI SRE 都不会把这条会话偷偷改派到其他环境，只会如实提示并拒绝发送。
2. **选择级回退会显式告知**：如果您是通过「让 AI SRE 在 Runner 上装好」这类入口带着一个**一次性固定的 Runner** 进入对话，而这个 Runner 在首轮发送前已经不可用（被删除，或已不在您的可选范围内）、属于您未加入的团队，或者无法确认它属于哪个团队，输入框的环境选择会**显式切回「自动环境」**，并弹出一条警告——而不是静默地落到您没选过的环境上。三种文案分别是：

   - 「该 Runner 已不可用，已改为自动环境」
   - 「该 Runner 属于团队 〈团队名〉，你不在其中，已改为自动环境」
   - 「无法确认该 Runner 的团队，已改为自动环境」

   同理，输入框里记住的环境选择如果与当前会话的团队不匹配，首轮发送时不会被带上，由服务端按「自动」决定落到哪里。
</Note>
...
每个 BYOC Environment 都有共享范围与团队范围两档作用域：

| 作用域 | 可见范围 |
|---|---|
| 共享 | 整个账户内所有成员可见、可选择并可用于运行。 |
| 团队级 | 仅该团队成员可见、可编辑、可选择并可用于运行。 |

编辑权限遵循统一规则：

1. 团队级 Environment 仅该团队的成员可以操作，组织管理员也需要先加入该团队；
2. 共享范围 Environment 仅组织管理员可以操作；
3. 不存在“创建者额外权限”，不是创建者也可以按上述规则编辑。

<Note>
账户仍是运行时安全边界，但团队作用域也会参与 Runner 选择：系统自动选择 Runner 或按 ID 固定 Runner 时，只会使用共享范围的 Runner，或当前成员所属团队下的团队级 Runner。这样可以避免成员把会话落到自己无权使用的团队 Runner 上。
</Note>
...
<Accordion title="日志提示 certificate signed by unknown authority" icon="shield-xmark">
...
<Accordion title="会话提示连接超时或无响应" icon="server">
  如果会话固定到了某个离线 Runner，AI SRE 不会偷偷改派到其他环境——这条规则针对的是**已经锁定环境**的会话，首次发送前的选择级回退见[在会话中选择环境](#在会话中选择环境)。请先恢复该 Runner，或新建会话并选择“自动”或“云端 Sandbox”。如果 Runner 在线但任务失败，多半是 Runner 到目标资源这一段的网络或权限问题。
</Accordion>
</AccordionGroup>
