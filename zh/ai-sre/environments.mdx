...
<Tabs>
  <Tab title="Linux (systemd)">
    ...
    脚本会安装二进制、创建 systemd 服务，写入 `/etc/flashduty-runner/env`，并安装 `/etc/tmpfiles.d/flashduty-runner.conf`——它把服务 `PrivateTmp=true` 的私有 /tmp 目录排除在 systemd 基于时间的 `/tmp` 清理之外，避免运行中的服务私有目录被删掉（旧版 systemd 不带这项排除）。同一条命令可用于首次安装和升级；如果已是最新版本，会自动跳过。
  </Tab>
  <Tab title="Docker">
...
- **Linux (systemd) / 手动安装**：安装脚本的 `--uninstall`（保留配置卸载）与 `--purge`（清除配置与数据）；两者都会删除 systemd unit 与 `/etc/tmpfiles.d/flashduty-runner.conf`。
