...
和其他资源（Skill、知识库、MCP、A2A Agent）一样，每个运行环境都带一个两级作用域字段 `team_id`：
...
如果会话**绑定了某个团队**（例如从作战室故障拉起、或在 UI 中显式指定团队），该团队的 Skill、MCP 与知识会在会话内被优先装配，运行环境的算力调度仍以账户为边界。关于团队绑定与按需挂载的完整机制，见 [资源作用域与权限](/zh/ai-sre/customize)。
