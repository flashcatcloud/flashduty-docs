...
<Step title="创建 Environment 记录">
  ...
  | 字段 | 类型 | 是否必填 | 说明 |
  |---|---|---|---|
  | 名称 | 字符串 | 否 | 用于标识此 Environment 的唯一名称，在账户范围内必须唯一，最长 128 字符。留空时，Runner 首次连接并发送心跳后，系统会自动以该机器的**主机名**填入（与 GitHub Actions runner 的自动命名机制一致）。若主机名已被占用，会在末尾追加 Environment ID 的后 4 位以区分 |
