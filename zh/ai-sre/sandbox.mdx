---
title: Sandbox
description: 云端沙箱是 Flashduty 托管的临时执行环境，开箱即用、无需安装。没有在线的自托管 Runner 时，AI SRE 会话默认在云端沙箱里执行；也可在会话中手动指定使用它。
...
它是 AI SRE 的**默认回退环境**：当您的账户下没有在线的自托管 Runner（[BYOC Runner](/zh/ai-sre/environments#byoc-runner)）时，会话会自动在云端沙箱里执行；您也可以在会话里**手动指定**使用它。
