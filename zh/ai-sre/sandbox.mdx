...
云端沙箱在托管服务商的 Serverless 平台上执行，默认位于**香港**（Tencent AGS 香港）；该区域不可用时，创建会依次回退到**广州**（Tencent AGS）与**北京**（阿里云 FC）。

沙箱的闲置处理分为两个阶段，对 Tencent AGS 与阿里云 FC 两个服务商一致（到期均为可恢复的**自动暂停**，而非终止性停止）：

| 阶段 | 触发时机 | 结果 |
|---|---|---|
| **自动暂停** | 最后一次工具调用后空闲约 3 分钟 | 沙箱进入可恢复的暂停状态，工作目录与文件完整保留；下次发送消息时自动唤醒（约 3 秒），继续在原工作目录执行 |
| **回收销毁** | 暂停后持续闲置约 30 分钟（自动化任务触发的会话约 10 分钟） | 沙箱被销毁，文件系统随之重置且**不可恢复**；继续该会话会自动重建一个全新的干净沙箱 |

<Warning>
被回收的沙箱内文件无法找回。请将需要长期留存的产出**保存为 Artifact 或沉淀到知识库**，不要依赖沙箱内的临时文件。
</Warning>
