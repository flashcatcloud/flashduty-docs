...
- 会话开始时，已启用且在当前作用域可见的服务器会出现在 Agent 的 `<available_mcp_servers>` 清单里。
...
<AccordionGroup>
  <Accordion title="启用 / 禁用" icon="toggle-on">
    用列表里的开关切换。只有**已启用**的服务器才会进入会话的 `<available_mcp_servers>` 清单；禁用后 Agent 看不到、也无法调用它。内置服务器**始终启用**，开关不可操作。
  </Accordion>
...
MCP 与其他资源（Skill、知识库、A2A Agent、运行环境）共用同一套**两级作用域**模型，由每行资源上的 `team_id` 字段决定：
...
**运行时可见性**：会话开始时，Agent 的 `<available_mcp_servers>` 清单中只会呈现**账户级** MCP 服务器，以及**当前会话所绑定团队**的服务器。当 Agent 读取某团队的知识（`.flashduty/knowledge/team_<id>/DUTY.md`）后，该团队的 MCP 服务器与 Skill 才会被按需挂载进当前会话。**账户是运行时唯一的安全边界，团队只是归属与编辑的标记。**
