...
<Steps>
  <Step title="打开目录">
    在 MCP 列表页点击 **浏览 Marketplace**，弹出目录对话框，以卡片网格展示所有可用的连接器模板。每张卡片展示连接器名称、供应商、传输方式、认证模式，以及是否需要 BYOC Runner。
  </Step>
  <Step title="查看详情">
    点击任意连接器卡片可查看完整描述、连接参数说明与官方文档链接。已安装的连接器卡片以齿轮图标标识，点击可直接跳转到该连接器的编辑表单。
  </Step>
  <Step title="发起安装对话">
    对尚未安装的连接器，点击卡片上的 **安装** 按钮，系统会打开一个新的 AI SRE 会话并注入该连接器的模板信息。Agent 随即引导您填写端点地址、完成凭证授权，并调用 `mcp_load` 验证连通性——整个安装流程**在对话中完成**，而不是一键写库。
  </Step>
  <Step title="确认安装完成">
...
<Warning>
需要 **BYOC Runner** 的连接器（卡片上有 `requires_runner` 标识，通常为 `stdio` 传输方式）只能在您自己部署了 Runner 的运行环境中使用，云 Sandbox 不支持。安装前请确认您的账户已配置可用的 BYOC Runner，详见 [运行环境（BYOC）](/zh/ai-sre/environments)。
</Warning>
