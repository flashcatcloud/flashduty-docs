...
<AccordionGroup>
  ...
  <Accordion title="阶段 3–7 — 主题逐项采集" icon="layer-group">
    ...
    | 4 | 可观测性栈 | `observability.md`（数据源缺口按「Monitors 优先」两步路由处理，见下方 Note；走 MCP 直连的登记到 `tools.md`） |
    | 5 | 运行手册 | `runbooks/<主题>.md`，一类故障一文件 |
    | 6 | 常见故障 | `common-failures.md` |
    | 7 | 集群访问与运行时探查 | `clusters.md`（k8s）/ 追加到 `tools.md`（MCP） |

    <Note>
      阶段 4 中，每一项数据源缺口都按「Monitors 优先」的两步路由处理：扫描摘要中已有 Monitors 使用证据（存在数据源或告警规则）时，Agent 会给出缺口对应的 `type_ident` 与控制台深链 `/monit/datasource?create=<type_ident>`——打开该深链会进入 Monitors 的添加数据源表单并预选该类型；`type_ident` 取值为 prometheus / loki / victorialogs / tencent_cls / sls / mysql / postgres / oracle / clickhouse / elasticsearch。数据源启用后，内置 `monit-query` 能力自动生效，无需安装任何东西。完全没有 Monitors 证据时，才改为通过 MCP 直连登记你在用的工具（见 [MCP](/zh/ai-sre/mcp)）。`/init` 自身不会创建或修改 Monitors 数据源。
    </Note>
