...
<Steps>
  <Step title="填写实例地址">
    在 GitLab 卡片上点击 **Connect**。地址栏默认预填 **`https://gitlab.com`**；要连接极狐 GitLab 或其他自建实例，把它改成对应地址即可，例如 `https://jihulab.com`（极狐 SaaS）或你的私有化部署地址。
  </Step>
  ...
  <Step title="在弹出的窗口中完成 GitLab 授权">
    向导打开一个弹窗，跳转到 GitLab 的官方授权页；用你的 GitLab 账户登录并确认授权。授权完成后弹窗自动关闭，向导进入下一步。
  </Step>
