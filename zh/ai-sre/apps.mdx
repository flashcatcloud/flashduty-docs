...
<Note>
如果账户授权了多个 GitHub 组织，`git clone` 和 `git push` 可以访问所有已授权仓库。`gh` 的 API 调用（包括 PR 和 Issue 命令）当前只使用最早授权的组织；处理其他组织的仓库时，优先使用 `git` 完成 clone、提交和 push。
</Note>
...
<Steps>
  ...
  <Step title="选择运行环境">
    在发起 OAuth 前，选择连接 GitLab 所使用的运行环境。GitLab.com 或可从公网访问的实例可以使用**云端 Sandbox**；自建 GitLab 位于内网时，选择一台能访问该实例且当前在线的 **BYOC Environment**。后续的 OAuth 回调、仓库选择、机器人配置、令牌轮换和断开连接都会通过这个环境访问 GitLab。
  </Step>
...
<Warning>
**GitLab.com 上的一条限制**：免费版和试用命名空间不能创建**分组访问令牌**。如果你只需访问部分仓库，请在仓库选择器中改选具体**项目**——项目访问令牌在这类命名空间可用；若要一次覆盖整个分组，则需要升级到付费命名空间。试用命名空间同一时间最多保留一个项目访问令牌；遇到 `limit reached` 时，先在 GitLab 的 **Settings → Access tokens** 撤销未使用的令牌，撤销后可能需要等待几分钟才会释放名额。
</Warning>
