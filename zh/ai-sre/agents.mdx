...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| 名称 | string | — | A2A Agent 标识（如 `metrics-analyzer`）。必填。名称在**所属作用域（账户或团队）内唯一**，不同团队可以使用相同名称；不能包含 `@`（该字符被保留用于「名称@团队ID」的消歧限定）；不能与平台内置 Agent 重名（如内置 Subagent `general`、`explore` 及内置 A2A Agent `flashduty-use-guide`），否则该 Agent 永远无法被派发 |
...
| Card URL | string | — | 远端 A2A Agent 的 Agent Card 地址，或仅填写遵循默认 well-known 规则的服务根地址（如 `https://agents.example.com`）。如果 URL 已带路径，平台会按该地址原样读取；如果只填服务根地址，平台按 A2A 约定查找 `/.well-known/agent-card.json`。必填。格式校验只检查 URL 合法（scheme 为 http/https、host 非空），不做主机或 IP 分类；实际的网络可达性与出站限制取决于发起委派的 AI SRE 会话所在执行环境，上面的执行环境字段只限制该 Agent 在哪些环境中可用。保存时，平台还会通过所选执行环境实际抓取该 Agent Card 并做**兼容性校验**（抓取请求通过执行环境出口发起，不经平台服务器，也不携带任何认证信息）：Card 无法解析为合法 Agent Card、缺少 `supportedInterfaces`（0.x 旧格式的卡）、或没有任何一项接口的 `protocolBinding` 为 `JSONRPC` / `HTTP+JSON` 且 `protocolVersion` 大版本为 1 时，保存会被直接拒绝并提示要求远端升级；如果当时没有可用的执行环境、网络不可达或返回非 2xx，平台无法完成校验时会跳过该检查，不阻止保存。远端 Agent 需实现 A2A Protocol 1.x，见下文「[协议版本与 Agent Card 要求](#协议版本与-agent-card-要求)」 |
...
平台作为 A2A 客户端调用远端 Agent，实现的是 **A2A Protocol 1.x**（`JSONRPC` 或 `HTTP+JSON` 协议绑定）。**0.x 及其他大版本协议不受支持**：远端 Agent 的 Agent Card 必须是 1.x 格式（大版本为 1 即可，`1.1` 等次要版本与 `1.0` 客户端兼容），否则保存时会被兼容性校验拒绝，未通过保存前校验的卡片在委派调用时也会失败。

1.x 格式的 Agent Card 通过 `supportedInterfaces` 数组声明接入点，其中至少一项需包含 `url`、`protocolBinding`（`JSONRPC` 或 `HTTP+JSON`）和 `protocolVersion`（大版本为 1，如 `"1.0"`）。一张最小合法卡片如下：
...
**从 0.x 迁移到 1.x**：如果您的 Agent Card 仍是 0.x 旧格式，请按下表调整后再接入：
...
模板只负责预填通用信息；你仍需在表单中选择**范围**（账户或团队）并按需配置认证。A2A Agent 可以按不同范围安装多次，因此 FlashAI 折叠卡片不会因为已有某个 FlashAI Agent 就自动消失。Agent 名称在**所属作用域（账户或团队）内唯一**——不同团队可以使用相同名称；如果需要在同一个作用域内为同一个 FlashAI 域名安装多个 Agent，请在表单里调整名称。
...
<Note>
出于安全考虑，**服务授权**的共享密钥（如 `token`、`api_key`）在保存前会**加密存储**，已保存的敏感字段在读取时会被**掩码**返回。编辑时敏感字段的语义是：保留表单回显的掩码 = **保留原值**；输入新的明文 = 覆盖原值；**把字段清空（或不填写）= 删除该密钥**（这与 MCP 的「留空即保留」语义不同）。此外，切换**服务授权**方式（如从 `bearer` 改为 `api_key`）会丢弃原方式已存的密钥。
</Note>
...
**命名唯一性**：Agent 名称在所属作用域内唯一——账户级名称不能与已有的账户级 Agent 重名，团队级名称不能与同一团队内的其他 Agent 重名；不同团队之间可以使用相同名称。
