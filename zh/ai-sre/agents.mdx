...
<Note>
出于安全考虑，已保存的敏感字段（如 `token`、`api_key`、`client_secret`）在读取时会被**掩码**返回。编辑时把敏感字段留空（或删除该字段）会**清空**已存储的凭证——这是清除凭证的唯一方式；唯一的例外是表单回显的掩码值：保持掩码值不变即保留原凭证。切换授权类型（如从 Bearer Token 改为 API Key）时，旧类型的凭证会被丢弃，需要重新填写。
</Note>
