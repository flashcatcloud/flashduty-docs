...
<Info>
  **公测功能**：AI SRE 已全量开放公测，无需申请，登录控制台即可直接使用；自 2026 年 9 月 16 日（北京时间）起按 credits 计费，此前的使用不收费。功能与界面可能继续调整。
</Info>
...
故障绑定并不只发生在故障路由入口：在**任意会话**的消息里附加一个故障引用（例如用 `@` 选中一个故障，见 [上下文引用](/zh/ai-sre/sessions#附件与上下文引用)）后，从该回合起这个故障同样会绑定到会话，故障时间线回写等故障操作随之生效。一次只绑定一个故障——同一回合里出现两个**不同**的故障会被视为歧义，两个都不绑定；同一个故障重复出现（引用与 IM 锚点指向同一个故障）仍算一次绑定。会话已有的绑定（例如自动化触发上下文带来的）优先，单回合的引用不会改写它。
