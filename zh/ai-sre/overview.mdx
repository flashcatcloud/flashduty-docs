...
<Accordion title="计费规则" icon="gift">
  ...
  体验额度、资源包与钱包余额都用尽时，AI SRE 会拒绝新的对话请求：输入区被禁用，上方常驻一条**不可关闭**的提示（文案由服务端给出，覆盖「尚未开通」「本周期额度用尽」「钱包余额不足」三种情形），并附带 **前往 AI SRE 计费** 入口，跳转到费用中心（`/wallet/plan?product=ai-sre`）。由管理员在那里开通按量付费、购买资源包或充值，状态恢复后即可继续对话。
</Accordion>
