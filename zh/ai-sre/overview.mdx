...
<Accordion title="计费" icon="gift">
  AI SRE 自 2026 年 9 月 16 日（北京时间）起正式收费，按 credits 计量模型用量、沙箱在线时长和联网检索；此前的使用不收取费用。计费从您开通 AI SRE 之时起算，开通前的历史用量不追溯计费。

  开通 AI SRE 即表示授权按量付费：赠送 credits 与已购资源包用完后，按 `1 credit = ¥1` 从账户通用钱包结算；您可以在控制台的 AI SRE 计费页把按量付费设为不限额、设置月度上限或禁止。未开通 AI SRE 的账户不产生费用——用量照常计量，但由 Flashduty 全额承担。
