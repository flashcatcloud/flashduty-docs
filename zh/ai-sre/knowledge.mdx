...
Knowledge Pack 是 AI SRE 资源的一种，遵循统一的两级作用域模型。其他资源（Skill、MCP、A2A Agent、运行环境）的作用域规则与本页一致，详见 [资源作用域与权限](/zh/ai-sre/customize)。
...
<Steps>
  ...
  <Step title="顺着引用读取文件">
    Agent 阅读 `DUTY.md` 后，根据当前故障决定展开哪些 `@引用`，用 `read` 工具读取 `knowledge/<scope>/<文件名>` 取得具体内容。文件按需从存储拉取并落到运行环境的工作目录。
  </Step>
  <Step title="按需挂载其它团队的知识">
    需要跨团队排障时，Agent 读取 `knowledge/team_<id>/DUTY.md`，系统会把该团队的整套知识（`DUTY.md`、运行手册）连同其 Skill、MCP 一并挂载进当前会话，且在本次会话中持久保留。同一个团队在一次会话里至多挂载一次。
  </Step>
</Steps>

<Tip>
  跨团队挂载只在 Agent**显式读取**某个团队的 `knowledge/team_<id>/DUTY.md` 时触发，不会被模糊的文件遍历误触发。挂载一次后，该团队的知识、Skill 与 MCP 在本次会话内一直可用。
</Tip>
...
**运行时可见性**：会话开始时，提示词只展示**账户级**资源加上**当前会话绑定团队**的资源。绑定来源是显式指定的团队，或战情室（war-room）故障对应的团队。其它团队的知识在会话进行中、当 Agent 读取该团队 `DUTY.md` 时才按需挂载。

<Warning>
  账户是运行时唯一的安全边界；团队是「编辑 / 所有权」标签，**不是**「运行时可读」边界。也就是说，账户内任意会话都能读取并挂载本账户其它团队的知识——这是跨团队联合排障的基础。如果某份知识对账户内其它团队也敏感，请评估是否适合纳入知识库。
</Warning>

这套两级作用域模型对账户内所有资源（Skill、MCP、A2A Agent、运行环境）一致适用，完整规则见 [资源作用域与权限](/zh/ai-sre/customize)。
