...
<Note>
AI SRE 运行时内置了几个 Skill，无需安装即可使用。`flashduty` 是其中一个范例：它通过 `fduty` 命令行覆盖整个 Flashduty API，让 Agent 可以排障故障、读取 AI 详情、查询告警、关联变更等。您可以参考它来编写自己的 Skill。另一个内置 Skill 是 `github`，Agent 会从 `<available_skills>` 中自主选用它，让 AI SRE 直接在 GitHub 仓库里工作——探索代码、调查 PR / 提交、按需开 PR 或 Issue；它需要安装 GitHub App（云端）或运行环境主机上的 `gh`（BYOC）。第三个内置 Skill 是 `gitlab`，能力与 `github` 对称：Agent 自主选用它在 GitLab 仓库里探索代码、追溯 MR / Issue、按需开 MR 或 Issue；它需要安装 GitLab App（云端）或运行环境主机上的 `glab`（BYOC）。第四个内置 Skill 是 `alert-governance`，它是内置 Skill 里**唯一**同时暴露在两种方式上的一个：您既可以在输入框直接输入 `/alert-governance` 触发它，Agent 也会在您用自然语言提出治理诉求时**自主选用**它（`init`、`insight` 只能由您输入斜杠命令触发，`flashduty`、`github`、`gitlab` 只供 Agent 自主选用）。它是**严格只读**的：只读取告警处理指标、产出一份治理报告，不会改动任何告警规则、路由或升级策略，也不会创建静默、确认或关闭任何事件。详见 [Apps](/zh/ai-sre/apps)。
</Note>

## 告警治理报告（/alert-governance）

---

`/alert-governance` 是内置的**告警健康度治理** Skill：在任意会话的输入框输入它，或直接用自然语言提出治理诉求，AI SRE 会产出一份单页羊皮纸样式的 HTML 报告，并作为聊天卡片渲染在会话中。与 [`/insight`](/zh/ai-sre/insight) 一样，报告里的数字由程序确定性地统计得出，而不是模型估算。

它回答的是「这一堆告警里，哪些最该动手、动手能省多少」：

- **账户总览**——分析窗口内的通知量、打扰次数、平均确认时长（MTTA）、平均关闭时长（MTTR）、噪声下降比例与夜间事件数；
- **问题告警卡片**——挑出最值得治理的 5–8 个告警，每张卡片给出支撑它入选的具体数字、一段判断（是该收紧等待窗口、该补聚合去重，还是该定责任人 / 调降级别 / 正式退役）、一条**预期收益**（按窗口内的实测数字线性外推，估算治理后大约能少多少通知），以及这项改动**该到哪个控制台页面**去执行；
- **On-call 与负载观察**——谁扛了最多的事件与打扰；
- **下一步建议**——账户整体配置层面的 1–3 条，区别于单个告警的修法。

### 需要知道的几点

- **严格只读。** 它只读指标、只写报告：不修改任何告警规则、路由或升级策略，不创建静默，也不确认或关闭任何事件；报告里每条建议都写明落点，由您自己到控制台执行。
- **默认分析近 30 天，窗口可覆盖。** 想换窗口直接说（如「看最近一周」「看过去 90 天」）即可。
- **范围跟着会话走。** 绑定了团队（如作战室）就分析该团队，未绑定团队则分析您能看到的全部团队；账户始终是安全边界，它只看得到当前凭证有权读取的数据。
- **事件条数有上限：单次分析最多拉取 500 条事件。** 如果某个窗口的事件拉取没能完整覆盖，采集会直接失败并且**不产出报告**——宁可不出，也不给一份基于部分数据的误导性报告。它按需运行、一次一份，没有断点续传，重跑一次即可。
- **数字只来自采集结果。** 采集不到的字段会在报告里明确标为不可获取，而不是补一个看起来合理的估计值。

### 怎么进入

三条路径都会走到同一份报告：在输入框直接输入 `/alert-governance`；用自然语言提出诉求（新会话空状态里的 **治理告警噪音** 卡片预填的就是这类提示）；或在自动化里用 **告警治理** 预设模板定时运行——该模板的任务提示词本身就是 `/alert-governance`，并明确要求「仅生成报告——不要修改任何 Flashduty 配置」。
