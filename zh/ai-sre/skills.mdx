...
<Info>
**团队同名 Skill 与 `@` 限定语法。** Skill 名在同一团队内必须唯一，但不同团队可以各自拥有同名 Skill。当两个团队都启用了同名 Skill 且都在当前会话的可见范围内时，裸 `/<skill-name>` 无法确定指向哪一个，会被拒绝并提示可用的限定形式；对话框 `/` 自动补全菜单会直接以 `name@<teamID>` 的限定形式列出这些条目（例如 `/deploy@5`），可以直接从中选择。此外，当**团队级 Skill 与账户级 Skill 同名时，团队条目会遮蔽账户条目**——补全菜单与可用列表中只显示团队项，裸名解析到团队 Skill。因此 `@` 字符在 Skill 名中被保留，不能使用。
</Info>
...
| 字段 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| `name` | string | 是 | Skill 名，kebab-case 格式（仅小写字母、数字、连字符，如 `my-skill-name`），长度 1–64。会作为 `/<skill-name>` 的触发词。不得与内置 Skill 同名（见下方「内置 Skill 保留名」说明） |
...
上传时系统会自动校验：归档是合法 zip、根目录存在 `SKILL.md`、frontmatter 可解析、`name` 符合 kebab-case 命名、声明的工具有效（内置工具存在、MCP 服务存在）。同一**团队**内**Skill 名不能重复**，重名会被拒绝并提示换名；不同团队可以各自拥有同名 Skill。

<Warning>
**内置 Skill 保留名**：`init`、`flashduty`、`insight`、`github`、`gitlab`、`artifact-design`、`alert-governance` 是运行时内置 Skill 的保留名称，在**任何作用域**（账户级或任意团队级）都不能用作自定义 Skill 的 `name`。上传同名 Skill 会被拒绝并提示 `Skill name '<name>' is reserved for an embedded bootstrap skill`；请改用其他名称后重新上传。
</Warning>
...
Skill 列表以表格展示每个 Skill 的**名称**（含来源模板角标、「有更新」标记与「仅本地 Runner」角标）、**范围**（账户或团队）、**版本**、**启用**开关与**操作**列。仅声明了 `venues: [byoc]` 的 Skill 会带上「**仅本地 Runner**」角标，鼠标悬停显示「仅在绑定本地 Runner 的会话中可用，云沙箱会话中不可用」；Marketplace 的模板卡片上也有同样的角标，方便您在安装前就知道它需要自托管 Runner。列表上方的工具条提供范围筛选（全部 / 账户 / 团队）与搜索框，搜索按名称、描述或作者中的关键词过滤列表。
