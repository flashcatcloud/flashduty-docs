---
title: Skill
description: Skill 是可复用的能力包：一段 SKILL.md 说明加上允许使用的工具，供 AI SRE Agent 在对话中按需调用。从市场一键安装，或上传自定义 Skill。
keywords: ["AI SRE", "Skill", "SKILL.md", "市场", "Marketplace", "Agent", "资源"]
sidebarTitle: Skill
---
...
启用后，Skill 会出现在会话的 `<available_skills>` 清单中。Agent 既可以**自主判断**何时调用某个 Skill，您也可以在对话框里用 `/<skill-name>`（如 `/flashduty`）来**显式触发**它。
...
当市场中的模板发布了更高版本时，对应 Skill 行会出现 **有更新** 标记。是否自动更新取决于该 Skill 是否被本地改动过（`is_modified`）：
...
Skill 也可以**在对话中由 Agent 直接生成或修改**：Agent 在工作区的 `skills/<名称>/` 目录下编辑文件后，可一键保存为 Skill；与已有 Skill 重名时会先提示「替换」确认。在检视面板里点击「在聊天中编辑」即可让 Agent 用内置的 `skill-creator` 协助您改写某个 Skill。
...
<AccordionGroup>
  <Accordion title="启用 / 禁用" icon="toggle-on">
    用列表或检视面板上的开关切换。只有**已启用**的 Skill 才会进入会话的 `<available_skills>` 清单；禁用后 Agent 看不到、也无法调用它。
  </Accordion>
...
Skill 与其他资源（知识库、MCP、A2A Agent、运行环境）共用同一套**两级作用域**模型，由每行资源上的 `team_id` 字段决定：
...
**运行时可见性**：会话开始时，提示词中只会呈现**账户级**Skill，以及**当前会话所绑定团队**的 Skill。当 Agent 读取某团队的知识（`.flashduty/knowledge/team_<id>/DUTY.md`）后，该团队的 Skill 与 MCP 才会被按需挂载进当前会话。**账户是运行时唯一的安全边界，团队只是归属与编辑的标记。**
...
<CardGroup cols={2}>
  ...
  <Card title="A2A Agent" icon="satellite-dish" href="/zh/ai-sre/agents">
    用 A2A Agent 扩展 AI SRE 的协作与分工能力。
  </Card>
