---
title: Skill
description: Skill 是可复用的能力包：一段 SKILL.md 说明加上允许使用的工具，供 AI SRE Agent 在对话中按需调用。从市场安装、上传自定义 Skill，或在对话中由 skill-creator 新建或从网上找现成的来装。
...
| `examples` | object[]（`{zh, en}`） | 否 | 「试试这样问」示例问题。最多 **10** 条，中英文两段都要填；每段都按 `description` 的规则校验（不能含尖括号 `<` / `>`，单段不超过 1024 字符），超限或非法会在**上传时被拒绝** |
...
Skill 市场目录是 **插件 → 概览** 页插件目录的一部分：与 MCP 模板、内置 App 一起按类别列出，Skill 单独归在 **Skill** 类别（在「全部」页签下也会出现，行上带 `Skill` 类型标签）。目录上方的搜索框按名称或描述过滤，页签可只看某一类，类别标题右侧的「查看全部 N」可展开该类别；搜索词与所选页签都记在页面地址里，离开再回来仍在原来的位置。

<Steps>
  <Step title="找到要安装的模板">
    在 **插件 → 概览** 的 **Skill** 类别里找到目标模板，每行展示模板名称与简介。
  </Step>
  <Step title="确认安装">
    未安装的模板行右侧是一个圆形 **+** 按钮，点击 **+** 或点击整行，都会弹出「**安装 Skill「〈模板名〉」**」确认对话框，提示「将安装到账户，账号内所有成员可用。」——市场安装的 Skill 固定为**共享范围**（共享即账户级），不提供归属选择。点 **安装** 才真正调用安装接口，把模板内容复制到您的账户，成为一个普通 Skill 行，并标记其来源模板（列表上以 `来自 Marketplace — 〈模板名〉 v〈版本〉` 的悬停提示标识）。安装成功会提示「「〈模板名〉」安装成功」。
  </Step>
  <Step title="管理已安装项">
    已安装的模板行不再显示 **+**，而是显示 **已启用**。安装后的管理与更新都在 **插件 → Skill** 的 Skill 列表里进行，见下文「管理与检视」与「自动更新与手动更新」。
  </Step>
</Steps>

<Note>
新账户会自动预装一组官方 Marketplace 模板：`browser-automation`（浏览器自动化 CLI，用于操作网站/仪表盘/监控 UI）、`mcp-builder`（指导创建 MCP 服务器）与 `skill-creator`（见下文「在对话中添加」）。这些预装 Skill 与手动安装的 Skill 完全一样，可以在下方「管理与检视」中启用/禁用、卸载或更新到最新版本。

`monit-query` 原本也是其中之一，现在对应模板已从市场下架，能力改为运行时内置 Skill：账户存在已启用的 Monitors 数据源时自动加载，否则不出现；此前安装过该模板的副本会被自动停用。它不可安装、卸载或更新，`monit-query` 名称被保留，您无法自建同名 Skill。
</Note>
...
Skill 列表页右上角的 **在对话中添加** 会新建一条会话，并预填一段需求描述：「我想添加一个 Skill。先问我是自己新建，还是从网上找现成的来装。找现成的就搜几个给我挑，我选好再装。」——即 Agent 会先问您要**自己新建**还是**装现成的**；选现成的先查插件目录/市场，目录里没有再联网搜索候选供您挑选，选好后再安装。提示词只是预填，不会自动发送。该按钮只在您有明确的可写入范围时显示：能授权到账户范围时始终显示，否则需要当前范围筛选恰好只选中一个您所属的团队（或您只属于一个团队）。
...
- **从网上找现成的**：从「在对话中添加」进入并选择装现成的时，Agent 先查插件目录，目录里没有再联网搜索候选；选定后由它完成安装，成为一个普通 Skill 行。
...
- **试试这样问**：若该 Skill 在 `SKILL.md` 的 frontmatter 里声明了 `examples`，检视面板标题区下方会出现一个 **试试这样问** 开关，**默认收起**——示例问题不会直接铺开；点击展开后才列出最多 2 条示例问题，点击其中一条即以该 Skill 加该问题预填一条新会话消息（不会自动发送）。展开区底部提示「点问题会新建对话并填好，确认后再发送」。
