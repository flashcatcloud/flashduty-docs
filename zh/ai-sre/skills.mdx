...
<Note>
**内置 Skill。** AI SRE 运行时自带一组**无需安装**即可使用的 Skill：`flashduty`、`github`、`gitlab`、`init`、`insight`、`alert-governance`、`artifact-design`（随版本演进可能增加）。其中几个还可以用斜杠命令直接触发：

- `flashduty` —— 通过 `fduty` 命令行覆盖整个 Flashduty API，让 Agent 可以排障故障、读取 AI 详情、查询告警、关联变更等。您可以参考它来编写自己的 Skill。
- `github` —— Agent 会从 `<available_skills>` 中自主选用它，让 AI SRE 直接在 GitHub 仓库里工作：探索代码、调查 PR / 提交、按需开 PR 或 Issue；它需要安装 GitHub App（云端）或运行环境主机上的 `gh`（BYOC）。
- `gitlab` —— 能力与 `github` 对称，作用于 GitLab 仓库：探索代码、追溯 MR / Issue、按需开 MR 或 Issue；它需要安装 GitLab App（云端）或运行环境主机上的 `glab`（BYOC）。两者详见 [Apps](/zh/ai-sre/apps)。
- `init` —— 用 `/init` 触发，以「先导入、后确认」的方式带你搭建运维知识库（服务清单、可观测性、runbook 等），每一步写入都需要你逐项确认。详见[初始化（/init）](/zh/ai-sre/init)。
- `insight` —— 用 `/insight` 触发，回看近 30 天会话，产出一份**只读**的月度复盘报告。详见[使用洞察（/insight）](/zh/ai-sre/insight)。
- `alert-governance` —— 用 `/alert-governance` 触发（也可以直接说「帮我治理告警」「哪些告警最差」），统计所选范围内告警的处理指标，挑出最值得治理的检查项，产出一份**只读**的告警健康度 HTML 报告。它只给建议、不改动任何告警规则、路由或升级策略，具体调整仍由你在控制台完成。
- `artifact-design` —— HTML 产物的设计与渲染规范：调色板、字体、布局、明暗主题与「单文件自包含」等硬性约束。Agent 在构建要发布到产物库的 HTML 页面之前会先按它执行；Markdown 产物由控制台按自身样式渲染，不受它约束。

这些 Skill 随运行时镜像提供，属于**只读**内置能力，无法直接编辑；要修正其中某项的行为，请在知识库中写入覆盖内容，或按本页说明编写自己的 Skill。
</Note>
