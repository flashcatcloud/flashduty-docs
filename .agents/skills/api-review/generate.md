...
2. **Extract the input schema.** Walk the `input_type` struct recursively. Honor Go tags:
   ...
   - **Unix-epoch integer time fields** (Go `int64`/`int` whose JSON name is `ts`, `timestamp`, or ends in `_at` / `_time` — e.g. `created_at`, `updated_at`, `start_time`, `end_time`, `last_time`, `assigned_at` — and which carries an epoch value, NOT a duration like `timeout`/`*_ms` interval): emit `type: integer`, `format: int64`, and a **non-empty `description` that contains the word `Unix`, `epoch`, or `timestamp`** — write `"Unix timestamp in seconds"`. This is load-bearing: the downstream go-flashduty SDK generator is description-driven and maps the field to its self-describing `Timestamp` type only when the description matches `(?i)\b(unix|epoch|timestamp)\b`; otherwise the field stays a bare `int64`. For millisecond-epoch fields, the description must additionally contain `milli` — write `"Unix timestamp in milliseconds"` so the field maps to `TimestampMilli`. The generator keys on the **description text only**, never the field name, so a duration without epoch wording (e.g. `timeout`, `interval_ms`) correctly stays `int64`. **Never leave an epoch time field with an empty description** — the v0.3.0 SDK shipped 18 fields stuck as int64 for exactly this reason.
...
- **`operationId`** — convert the registry `name` (`template:read:info`) to **kebab-case** (`template-read-info`). Kebab-case is URL-safe, SEO-friendly, and what Mintlify uses as the URL path segment for each rendered operation. One stable ID per colon-separated token.
  - **Compound resource names MUST keep their word boundaries, derived from the hyphenated `path` — never glued from the provider/registry word.** The registry `name` often fuses a multi-word resource into one token (`statuspage:read:pageList`, provider `status-page`); kebab-ifying that token verbatim yields `statuspage-...`, which **drops the word break**. Instead, take the resource segment from the URL `path`, which is already kebab-case: `/status-page/list` → `status-page-read-page-list` (NOT `statuspage-read-page-list`). The same applies to every compound resource: `status-page`, `war-room`, `alert-event`, `on-call`, `data-source`, `rule-set` — never `statuspage` / `warroom` / `datasource`.
  - **Why it matters:** the downstream go-flashduty SDK derives Go method names by stripping the token prefix shared across all operations in a service/tag group. If one opId's first token disagrees (`statuspage` ≠ the siblings' `status` + `page`), the shared prefix collapses to zero and **every** method in that service gets a stuttering name (`ChangeList` → `StatusPageChangeList`). Keep the resource segment tokenized identically across all ops in a group.
  - **Only fix genuinely fused opIds — do NOT mass-rewrite existing camelCase operationIds** (e.g. `statusPageChangeList`) into kebab. Those already tokenize correctly (`status` + `page`); rewriting them needlessly renames every downstream method. The goal is only that the resource segment of sibling opIds tokenizes consistently. A path segment with no hyphen (`/monit/datasource/...`, `/monit/store/ruleset/...`) is a single word — `datasource`/`ruleset` in the opId is correct, not fused.
