...
A "processing receipt" is the immediate signal the bot gives once it has your message; how it looks on each platform is described under [Messages and Receipts](#messages-and-receipts).
...
<Steps>
  <Step title="Detect the mention">
    The platform distinguishes between **group @mentions** and **direct messages**. Once mentioned, AI SRE deduplicates the event, puts a "processing" receipt on that message (see [Messages and Receipts](#messages-and-receipts)), and determines whether the message contains an imperative instruction.
  </Step>
...
| Platform | Emoji pool (one picked at random) |
| --- | --- |
| Slack | eyes, thinking_face, hourglass_flowing_sand, mag, zap |
...
Mid-turn, the agent can hand over **intermediate progress notes** — each appears as a separate chat message (for example, "5xx traffic confirmed on api-gateway; still waiting on the log query"), so a long investigation shows progress while it runs. A progress note does not count as an answer and never stands in for the final answer; a progress note that arrives after the turn has ended is dropped outright. **The final conclusion is still the formal reply.**

### How Answers Are Delivered

Every IM @mention turn negotiates **reply-only delivery** with the backend: once accepted, the replies the agent hands over through the `reply` tool are delivered **as they are produced** as native messages in the chat, and the end of the turn does not append a duplicate final-answer message. When the negotiation is not accepted (including older backend builds), delivery falls back to the previous behavior — one final answer at the end of the turn. This negotiation is **IM-only**: console / API sessions and war-room auto-diagnosis are unaffected.
