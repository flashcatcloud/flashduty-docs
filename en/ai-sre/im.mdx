...
<Note>
When the account has no AI SRE credits available, this mention never reaches an investigation — the chat gets one fixed reply: "⚠️ AI SRE has no credits available for this account. Please ask an administrator to enable AI SRE, buy a credit package, or raise the usage limit in the console, then try again." Nothing is retried automatically, and sending the message again hits the same admission gate — the account recovers only after an administrator enables AI SRE, buys a credit package, or raises the usage limit in the console.
</Note>
...
Auto-diagnosis can also be refused: when the account has no AI SRE credits available, the run produces no findings and the war room receives a message stating the reason — "AI-SRE could not run: this account has no AI SRE credits available. Please ask an administrator to enable AI SRE, buy a credit package, or raise the usage limit in the console." Unlike the generic failure copy — which, when group replies are enabled, invites you to reply to the message later for a deeper investigation — this message **carries no reply-to-retry invitation**: while the account is blocked, a reply is rejected by the same admission gate, so retrying before an administrator acts accomplishes nothing.
...
- **Console side:** the console conversation header also offers thumbs-up feedback on replies (see [Feedback](/en/ai-sre/sessions#session-feedback)), sharing the same rating system as IM `/feedback`.
