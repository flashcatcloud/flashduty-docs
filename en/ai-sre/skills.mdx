...
<Note>
**Bundled skills.** The AI SRE runtime ships a set of skills that need **no installation**: `flashduty`, `github`, `gitlab`, `init`, `insight`, `alert-governance`, and `artifact-design` (the set may grow across releases). Several of them are also triggerable as slash commands:

- `flashduty` — covers the entire Flashduty API through the `fduty` CLI, letting the agent investigate incidents, read AI insights, query alerts, correlate changes, and more. Use it as a template when writing your own skills.
- `github` — the agent self-selects it from `<available_skills>` to work directly inside a GitHub repository: explore code, investigate PRs and commits, and open a PR or issue on request; it requires the GitHub App (cloud) or the runner host's `gh` (BYOC).
- `gitlab` — symmetric with `github`, acting on GitLab repositories: explore code, trace MRs and issues, and open an MR or issue on request; it requires the GitLab App (cloud) or the runner host's `glab` (BYOC). See [Apps](/en/ai-sre/apps) for both.
- `init` — triggered with `/init`; an import-first walkthrough that builds your operational knowledge base (service catalog, observability, runbooks, and more), where every write needs your confirmation, item by item. See [Setup (/init)](/en/ai-sre/init).
- `insight` — triggered with `/insight`; looks back over the last 30 days of sessions and produces a **read-only** monthly review report. See [Usage Insights (/insight)](/en/ai-sre/insight).
- `alert-governance` — triggered with `/alert-governance` (or plain language such as "help me govern my alerts" or "which alerts are worst"); it aggregates the alert-handling metrics of the selected scope, picks the checks most worth fixing, and returns a **read-only** alert-health HTML report. It only suggests — it changes no alert rule, route, or escalation policy; you make the actual adjustments in the console.
- `artifact-design` — the design and rendering rules for HTML artifacts: palette, typography, layout, light/dark themes, and hard self-contained-file constraints. The agent follows it before building an HTML page destined for the artifact gallery; Markdown artifacts are rendered by the console with its own styles and are unaffected.

These skills ship with the runtime image as **read-only** built-ins you cannot edit directly; to correct the behavior of one, write an override in your knowledge base, or author a skill of your own as described on this page.
</Note>
