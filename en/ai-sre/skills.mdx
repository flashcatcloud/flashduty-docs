---
title: Skills
description: A Skill is a reusable capability bundle — a SKILL.md instruction file plus a declared set of allowed tools — that the AI SRE agent can invoke on demand during a conversation. Install from the Marketplace with one click, or upload your own custom skill.
keywords: ["AI SRE", "Skill", "SKILL.md", "Marketplace", "agent", "resources"]
sidebarTitle: Skills
---
...
Once enabled, the skill appears in the session's `<available_skills>` list. The agent can **autonomously decide** when to invoke a skill, or you can **explicitly trigger** it in the chat box with `/<skill-name>` (e.g., `/flashduty`).
...
When a Marketplace template publishes a newer version, the corresponding skill entry displays a **Update available** badge. Whether the update is applied automatically depends on whether the skill has been modified locally (`is_modified`):
...
Skills can also be **generated or modified directly by the agent during a conversation**: after the agent edits files under the `skills/<name>/` directory in the workspace, you can save them as a skill with one click. If the name conflicts with an existing skill, a "Replace" confirmation prompt appears first. Click "Edit in Chat" in the detail panel to let the agent use the built-in `skill-creator` to help you rewrite a skill.
...
Skills share the same **two-level scope** model with other resources (Knowledge Packs, MCP, A2A agents, environments), determined by the `team_id` field on each resource row:

| `team_id` | Scope | Visibility |
| --- | --- | --- |
| `0` | Account level | Visible to all members account-wide |
