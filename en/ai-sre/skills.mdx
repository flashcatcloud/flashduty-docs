...
| --- | --- | --- | --- |
| `name` | string | Yes | Skill name: must start with a letter and contain only letters (any case), digits, `-`, and `_` (e.g. `my-skill-name`, `code_reviewer`, `IncidentTriage`), 1–64 characters. This becomes the `/<skill-name>` trigger word, and slash commands are parsed under the same naming rule. |
...
<Note>
The AI SRE runtime bundles a few skills that are available without installation. `flashduty` is one such reference: it uses the `fduty` CLI to cover the entire Flashduty API, allowing the agent to investigate incidents, read AI insights, query alerts, correlate changes, and more — use it as a template when writing your own skills. Another bundled skill is `github`, which the agent self-selects from `<available_skills>` to let AI SRE work directly inside a GitHub repository — explore code, investigate PRs and commits, and open a PR or issue on request; it requires the GitHub App (cloud) or the runner host's `gh` (BYOC). A third bundled skill is `gitlab`, symmetric with `github` in capability: the agent self-selects it to work directly inside a GitLab repository — explore code, trace MRs and issues, and open an MR or issue on request; it requires the GitLab App (cloud) or the runner host's `glab` (BYOC). A fourth bundled skill is `alert-governance`, the only built-in exposed on both surfaces at once: you can type `/alert-governance` directly, and the agent also self-selects it when you ask to clean up alert noise in plain language (`init` and `insight` are triggered only by you typing the slash command; `flashduty`, `github` and `gitlab` are agent-selected only). It is strictly read-only: it reads alert-handling metrics and produces a governance report, and it never touches an alert rule, route or escalation policy, never creates a silence, and never acknowledges or closes anything. See [Apps](/en/ai-sre/apps).
</Note>

## Alert governance report (/alert-governance)

---

`/alert-governance` is a bundled **alert health governance** skill: type it into the input box of any session, or simply ask to clean up alert noise in plain language, and AI SRE produces a single-page parchment-style HTML report rendered in the session as a chat card. As with [`/insight`](/en/ai-sre/insight), the numbers in the report are computed deterministically by a program, not estimated by the model.

It answers "of all these alerts, which are most worth fixing, and how much would fixing them save?":

- **Account overview** — notifications, interruptions, mean time to acknowledge (MTTA), mean time to close (MTTR), the noise-reduction percentage, and the overnight incident count for the analysis window;
- **Problem-alert cards** — the top 5–8 alerts worth governing, each with the specific numbers that justify featuring it, a short judgment (tighten the wait window, add aggregation/dedup, or assign an owner / downgrade / formally retire it), an **expected gain** (a linear extrapolation from the measured numbers in the window, estimating how many notifications the change would save), and **which console page** to make the change on;
- **On-call & load observations** — who is carrying the most incidents and interruptions;
- **Next steps** — 1–3 suggestions at the level of how the account is configured overall, distinct from any single alert's fix.

### What to know

- **Strictly read-only.** It reads metrics and writes a report: it changes no alert rule, route or escalation policy, creates no silence, and acknowledges or closes nothing; every suggestion names where to make the change and you apply it yourself in the console.
- **Default window is the last 30 days, and it can be overridden.** Ask for a different window (for example "last week" or "the past 90 days") and the collector re-pulls for that duration.
- **Scope follows the session.** A team-bound session (a war room, say) analyzes that team; an unbound session analyzes every team you can see. The account is always the security perimeter — it only ever reads data your current credentials are allowed to read.
- **There is an incident cap: one run pulls at most 500 incidents.** If the incident pull for a window cannot be completed in full, collection fails and **no report is produced at all** — rather than a misleading report built on a partial window. It runs on demand, once per run, with no resume mechanism; just re-run it.
- **Every number comes from the collected data.** A field the collector could not retrieve is labeled plainly as unavailable in the report, never filled in with a plausible-looking estimate.

### How to reach it

All three paths lead to the same report: type `/alert-governance` in the input box; ask in plain language (the **Reduce alert noise** card in a new session's empty state prefills exactly this kind of prompt); or run it on a schedule from the **Alert Governance** automation preset — that preset's task prompt is literally `/alert-governance` and explicitly requires "Report only — do not modify any Flashduty configuration."
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| Owner | Team / Shared | Yes | Select the scope for this skill: **Shared** (account level — visible to all members account-wide) or a specific **Team** (visible only to members of that team). To upload into a team scope, you must belong to the target team; shared-scope upload is limited to the account owner or admins. See "Scope" below. |
...
<AccordionGroup>
  ...
  <Accordion title="Edit" icon="pen-to-square">
    Click the edit button to update the **description** and **owner** (scope). Marketplace-installed skills are permanently shared scope, so the scope is not editable in the form. To modify skill content, download the zip, edit it, then **re-upload** (this creates a new version and marks the skill as modified).
  </Accordion>
...
| Scope | Visibility |
| --- | --- |
| Shared (account level) | Visible to all members account-wide |
