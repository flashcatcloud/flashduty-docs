...
<Note>
Those are examples, not the whole set: the runtime also bundles `init`, `insight`, `artifact-design`, `alert-governance`, `monit-query` and more, and the set grows between releases. What the agent actually sees in a session is `<available_skills>`; to find out which bundled skills a given session has, just ask the agent.
</Note>
