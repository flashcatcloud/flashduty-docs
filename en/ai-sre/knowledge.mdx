...
**Deletion and retention**: after you delete a knowledge file (or an entire pack), the file first enters a **soft-deleted** state with a roughly **30-day** grace window — the tombstone prevents an in-flight full sync from resurrecting a file you just deleted, and re-uploading the same path within the window revives it in place. Once the window expires, a background garbage-collection sweep permanently removes the file's record and stored content; from that point on the deletion is **irreversible**.
