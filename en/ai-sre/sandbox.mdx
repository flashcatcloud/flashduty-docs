...
Idle handling happens in two stages, identically on both providers (Tencent AGS and Aliyun FC) — in each case the timeout triggers a resumable **auto-pause**, not a terminal stop:

| Stage | When it triggers | Outcome |
|---|---|---|
| **Auto-pause** | About 3 minutes after the last tool call | The sandbox enters a resumable paused state with the working directory and files fully preserved; your next message wakes it automatically (about 3 seconds) and continues in the same working directory |
| **Reclaim and destroy** | About 30 minutes of continued idleness after the pause (about 10 minutes for sessions started by an automation trigger) | The sandbox is destroyed and its filesystem is reset **unrecoverably**; continuing the session rebuilds a fresh, clean sandbox automatically |

<Warning>
Files inside a reclaimed sandbox cannot be recovered. Save anything you want to keep as an **Artifact or in the knowledge base** — do not rely on temporary files inside the sandbox.
</Warning>
