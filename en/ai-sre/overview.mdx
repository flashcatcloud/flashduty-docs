...
<Accordion title="Billing" icon="gift">
  AI SRE starts charging on September 16, 2026 (Beijing time), metered in credits across model usage, sandbox online time and web search; usage before that date is not charged. Billing starts when you enable AI SRE, and usage from before enablement is never back-billed.

  Enabling AI SRE authorizes pay-as-you-go: once gifted credits and purchased credit packages run out, usage settles from the account's general wallet at `1 credit = ¥1`. You can set pay-as-you-go to unlimited, a monthly cap, or disabled on the AI SRE billing page in the console. An account that has not enabled AI SRE incurs no charge — its usage is still metered, but Flashduty bears the full cost.

  See [Credits and Billing](/en/ai-sre/credits) and the [AI SRE credit package purchase agreement](/en/compliance/ai-sre-credit-package-purchase-agreement) for what is metered, unit prices, credit package tiers, deduction order and refunds.
</Accordion>
