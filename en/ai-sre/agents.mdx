...
| --- | --- | --- | --- |
| Name | string | — | A2A agent identifier (e.g., `metrics-analyzer`). Required. The name must be unique **within its scope (account or team)** — different teams may reuse the same name; it must not contain `@` (reserved as the separator in `name@teamID` disambiguation); it must not collide with a built-in agent name (such as the built-in Subagents `general` and `explore`, or the built-in A2A agent `flashduty-use-guide`) — a name that does would make the agent permanently undispatchable |
...
| Card URL | string | — | The Agent Card URL of the remote A2A agent, or just the service origin for agents that follow the default well-known location (for example, `https://agents.example.com`). If the URL includes a path, the platform reads that exact URL; if only an origin is provided, it resolves `/.well-known/agent-card.json` by A2A convention. Required. The format check only validates that the URL is well-formed (scheme is http/https, host is non-empty) — it does not classify hosts or IPs; actual network reachability and egress restrictions depend on the current AI SRE session's execution environment. The Execution Environment field above only limits where this agent is available. On save, the platform also fetches the Agent Card through the selected execution environment and runs a **compatibility check** (the fetch egresses from that environment, not from the platform's servers, and carries no authentication): if the card cannot be parsed as a valid Agent Card, declares no `supportedInterfaces` (a 0.x card), or has no interface entry with `protocolBinding` of `JSONRPC` / `HTTP+JSON` and a `protocolVersion` with major version 1, the save is rejected with an error asking the provider to upgrade. When no execution environment is available, the endpoint is unreachable, or the response is not 2xx, the check is skipped and the save proceeds. The remote agent must implement A2A Protocol 1.x — see "[Protocol Version and Agent Card Requirements](#protocol-version-and-agent-card-requirements)" below |
...
When calling a remote agent as an A2A client, the platform implements **A2A Protocol 1.x** (with the `JSONRPC` or `HTTP+JSON` protocol binding). **Protocol 0.x and other major versions are not supported**: the remote agent's Agent Card must be in the 1.x format (any major version 1 works — e.g., `1.1` is compatible with a `1.0` client), or the save-time compatibility check will reject it; cards that never passed the check will also fail delegated calls.

A 1.x-format Agent Card declares its endpoints via the `supportedInterfaces` array, where at least one entry must contain `url`, `protocolBinding` (`JSONRPC` or `HTTP+JSON`), and `protocolVersion` (major version 1, e.g. `"1.0"`). A minimal valid card looks like this:
...
**Migrating from 0.x to 1.x**: if your Agent Card still uses the 0.x format, adjust it as follows before connecting:

| 0.x format | 1.x format |
| --- | --- |
...
The template only prefills common values. You still choose the **Scope** (Account or Team) and configure authentication in the form. A2A agents can be installed multiple times for different scopes, so the FlashAI panel does not disappear just because one FlashAI agent already exists. Agent names must be unique **within their scope (account or team)** — different teams may reuse the same name; if you need to install the same FlashAI domain more than once in the same scope, adjust the name in the form.
...
<Note>
For security reasons, the **service authorization** shared secrets (such as `token` and `api_key`) are **encrypted before being stored**, and saved sensitive fields are returned **masked** when read. When editing, the sensitive-field semantics are: leaving the echoed mask in place = **keep the stored value**; entering new plaintext = overwrite it; **clearing the field (or omitting it) = delete the secret** — note this differs from MCP, where leaving a field blank means "keep the value." Switching the **service authorization** type (e.g., from `bearer` to `api_key`) also discards the secrets stored under the previous type.
</Note>
...
**Name uniqueness**: an agent name must be unique within its scope — an account-level name cannot duplicate an existing account-level agent, and a team-level name cannot duplicate another agent in the same team; different teams may reuse the same name.
