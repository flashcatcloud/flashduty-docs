---
title: A2A Agents
description: A2A (Agent-to-Agent) lets AI SRE connect to external agent ecosystems — delegating tasks to remote agents as well as exposing an Agent Card so external clients can call Flashduty's AI SRE in reverse, enabling incident / war-room integration and bidirectional event streams.
keywords: ["AI SRE", "A2A", "Agent-to-Agent", "Agent Card", "agent", "delegation", "outbound", "inbound", "resources"]
...
A2A (Agent-to-Agent) is a standard protocol that lets different agents call one another. In AI SRE, the **A2A Agents** resource connects AI SRE to external agent ecosystems in two directions:
...
A2A agents share the same **two-level scope** model as other resources (skills, Knowledge Packs, MCP, environments), determined by the `team_id` field on each row:
