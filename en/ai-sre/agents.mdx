...
<Note>
For security reasons, saved sensitive fields (such as `token`, `api_key`, and `client_secret`) are returned **masked** when read. When editing, leaving a sensitive field blank (or removing the field) now **clears** the stored credential — this is the only way to clear it; the single exception is the masked value echoed back in the form, which keeps the stored secret when left unchanged. Switching the auth type (e.g., from Bearer Token to API Key) discards the previous type's credentials, which must be re-entered.
</Note>
