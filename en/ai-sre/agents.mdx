...
**Execution environment (`environment`)**: `agent_dispatch` also takes an optional `environment` argument that decides **where** a subtask runs:

| Value | Meaning |
| --- | --- |
| Omitted | The current session's environment, sharing the parent's workspace (default) |
| The session's own current environment | Also a no-op: still the parent's workspace, never a second sandbox |
| `cloud` | A fresh cloud sandbox: its own container and filesystem, under the account's default egress policy |
| The `environment_id` of an online BYOC runner | Runs on that self-hosted runner |

- **The selectable targets are stated in the tool description**: alongside its parameters, `agent_dispatch` describes this run's options — the session's current environment (`current`, where an omitted argument lands), `cloud`, and every BYOC runner this session may use that is **online**. The runner list follows the same team-placement rule as Fork: account-scope runners are always listed, a team-scope runner only when the session is bound to that team (same-named runners carry their team name), so **anything advertised is dispatchable**.
- **Re-validated at dispatch time**: the same placement rule plus a liveness probe. An offline, deleted, or foreign-team target returns its reason verbatim to the model, which relays it to you or picks another environment.
- **Subagents only**: passing `environment` for an A2A agent is rejected outright — a remote agent always runs where it lives, with this session initiating the call.
- **Another environment is another machine**: the subtask cannot see any file you wrote in this session; put every input it needs in the dispatch prompt, and expect its output back as text or through a system both sides reach (git, a ticket). The dispatch result echoes where it actually went (`cloud` or `byoc runner <environment_id>`), and only for a real cross-environment dispatch.
- **A connector's "Execution environment" feeds the same menu**: when an MCP connector is bound to environments other than this session's, those environments are presented to the model as dispatch targets — the `tool_search` connector directory lists, per connector, the environments it can run in and which cannot be dispatched to right now, telling the model to send a subagent there with `agent_dispatch(environment=<target>)`. The dispatch reference line and the task panel in a session also label that environment; see [Console](/en/ai-sre/sessions).
