...
Incident binding is not limited to the incident-route entry point: when you attach an incident reference to a message in **any** session (for example by selecting an incident with `@` — see [Context references](/en/ai-sre/sessions#attachments-and-context-references)), that incident also binds to the session from that turn on, and incident operations such as timeline write-back take effect. Exactly one incident binds at a time — two **different** incidents in one turn are treated as ambiguous and neither binds; the same incident appearing twice (a reference plus an IM anchor pointing at it) still counts as one binding. A binding the session already has (for example one carried by an automation trigger context) wins, and a single-turn reference never retargets it.
