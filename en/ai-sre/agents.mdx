...
<Steps>
  ...
  <Step title="Configure FlashAI /config">
    Open `/config` in the FlashAI / Flashcat console (for example, `https://demo.flashcat.cloud/config`), add `a2a_server_base_url`, and set it to the customer's own FlashAI access domain, for example `https://demo.flashcat.cloud`.
  </Step>
...
The template only prefills common values. You still choose the **Scope** (Account or Team) and configure authentication in the form. A2A agents can be installed multiple times for different scopes, so the FlashAI panel does not disappear just because one `flashai` agent already exists.
