...
<Info>
  **Public beta**: AI SRE is now open to all accounts — no application needed. Sign in to the console and start using it; billing starts on September 16, 2026 (Beijing time), and usage before that date is free. Features and the UI may continue to evolve.
</Info>
...
The input box remains active while a turn is running: you can keep typing and send messages, which are queued and executed in order after the current turn completes. Queued messages render as **ordinary user bubbles** appended to the end of the transcript (there is no collapsible queue card above the input box), and a queued bubble carries no per-message action buttons. The only way to withdraw one is to press **↑ in an empty composer**, which pulls your own most recently queued message back into the input — recalling it also withdraws it from the server queue, so clearing the input is how you drop it. While the queue does hold a message of yours, the composer placeholder reads "Press up to edit the last message". If a send fails, the bubble shows "Not sent" with **Retry** / **Delete** actions.

When several **plain messages** from the same person are waiting in the queue as a turn starts, AI SRE folds them into that **single turn** and answers them together — history still shows each message as its own row. Messages from other people, messages sent from IM, question answers, and automation / child-session notifications each keep their own turn.
...
Forking clears temporary state that only belongs to an in-progress run, such as active-turn caches, pending mount state, frontend state that has not been persisted, and current-turn counters. Persisted history, tool calls, and reusable compaction state are retained; team and environment binding are written to the new session based on your choice in the fork dialog, while the **chat model is inherited from the source session** — a fork continues the same conversation, so it is not drawn again; if the source had no model bound, the fork stays unbound and draws one on its first turn. The forked session has its own context, so later messages, compaction, and run results do not write back to the source session.
...
- **`model`**: the model on `session_meta` and `llm_call` lines is the **session's own bound chatmodel** (set on the first turn's weighted pool draw, on any later failover rebind, or inherited directly when the session was forked from another), which can and does disagree with the app's static config default (e.g. config default qwen, the session's actual model deepseek); sessions that never bound one fall back to the app config default, and unregistered apps get an empty value.
