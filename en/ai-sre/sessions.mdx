...
In a blank AI SRE session, four scenario suggestion cards appear above the composer: **Investigate an incident**, **Query data with natural language**, **Reduce alert noise**, and **Automatically analyze new incidents**. Selecting a card only fills the composer; it does not send the prompt. You can edit it before pressing Enter. Open **More** to choose additional scenarios, including **Schedule an automated inspection**.
