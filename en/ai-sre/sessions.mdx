...
### What Happens After a Turn Fails

A turn does not simply stop the moment it fails: the platform looks at why it failed and then decides whether another automatic attempt is worth making, so you do not have to resend anything immediately.

- **Engine-class failures are retried once automatically**: for faults that cannot be determined up front and may not repeat — the model service or an upstream being temporarily unavailable, for example — the platform automatically retries the same turn once (after waiting a few seconds). If the retry succeeds, it carries on and produces the conclusions.
- **Deterministic failures are not retried**: the model's content-safety review rejecting this request, an unusable runtime environment (an offline self-hosted Runner, or a selected environment that has expired or been deleted), and an insufficient account balance — retrying these with identical input produces the identical result, so the platform does not retry and instead hands you the reason it failed. Act on the prompt (rephrase the flagged content, switch to a usable runtime environment, top up your balance) and send again, or use that message's **Retry** action (see [Message actions](#message-actions)).

The automatic retry applies only to turns started from a main session; subagent turns and notification-type messages generated inside the platform (a subagent completion notice, for example) are excluded.
...
<Note>
Editing a historical message is, under the hood, a **rewind** operation: once submitted, the conversation regenerates from that message onward, and any content after that message is replaced. Confirm before submitting. If the turn that produced the target message is still running or stopping, the rewind is rejected (`rewind target invocation is still active`) — wait for that turn to settle before submitting. If the session is busy with another operation (compacting its context, for example), the rewind first waits for that operation to release the session lock, and is only rejected if it can't get it: a compaction holds the lock for up to about **3 minutes** (a pre-compaction memory extraction of up to 2 minutes, plus one summariser call and some headroom), and the rewind usually goes through once that wait ends. No progress is shown while it waits, so the submit looking stuck for a while is expected. Only when the lock is still held after that wait budget do you get "The session is busy with another operation. Please try again in a moment." — that is not a permanent failure; just resubmit. When a rewind takes effect, the background processes and dispatched subagents spawned by the wiped turns are stopped along with it, and their task rows are removed from the task panel.
</Note>
