...
Exception: when the agent has finished its turn but is still waiting on dispatched Subagent / A2A tasks (their task cards are still running), a new message is **not queued** — it starts a new turn immediately. The queue card only covers messages sent while a turn is actually executing.
...
The **event-count trigger** is a safety net: in a session made of many short turns, the token estimate can stay below the threshold for a long time while the number of events (messages, tool calls, and so on) keeps growing. Compaction fires once the count reaches about 500, keeping the un-compacted event history from growing without bound — compaction is the only bound on what the model loads per turn (there is no longer a fixed per-load cap; when the event count passes the alert line, only an alarm signal is reported, and nothing is dropped). If you see compaction happen while the Context percentage is well below the threshold, this is why.
