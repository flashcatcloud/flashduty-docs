...
<Info>
  **Public beta**: AI SRE is now open to all accounts — no application needed. Sign in to the console and start using it. From September 16, 2026 (Beijing time) it is billed in credits; usage before that date is not charged — see [Credits and Billing](/en/ai-sre/credits). Features and the UI may continue to evolve.
</Info>
...
Streaming output **auto-follows** by default: the view stays pinned to the bottom as new content arrives. Scrolling up to read history pauses auto-follow, so new output no longer drags the view back down; a **Scroll to latest** button then appears at the lower right of the conversation area — click it to jump to the newest message and resume auto-following. Scrolling back to within 100 px of the bottom resumes auto-follow the same way.
...
### Account-Level Billing Block

AI SRE usage is billed per account, so "this account cannot run AI SRE right now" is a standing **account state**, not a one-off failure of a particular send. While that state holds, a warning banner stays pinned above the input box: it shows the sentence the server returned (already localized to your console language, and already naming the fix), with a **Go to AI SRE billing** button on the right that opens the AI SRE billing page in a new tab (`/wallet/plan?product=ai-sre`), where you buy a credit package or raise the usage cap.

Three situations share this one banner, each with its own server-side sentence:

| Situation | Server message |
|---|---|
| AI SRE is not enabled for the account | AI SRE is not enabled for this account. Enable it to continue. |
| This period's allowance is used up | This account has used its full AI SRE credits allowance for this period. Buy a credit package or raise the usage limit to continue. |
| The wallet balance is insufficient (pay-as-you-go) | The account balance is insufficient. Top up the wallet to continue using AI SRE on pay-as-you-go; a recent top-up may take a few minutes to take effect. |

A few deliberate design choices in the banner:

- **Not dismissible**: it stays visible for as long as the state holds. Dismissing a block that is still true would only make you send again and hit the same sentence.
- **The Send button is disabled with it**: sending is unavailable across the whole account, and it is not silently greyed out — its hover hint is that same sentence from the banner.
- **Re-checked automatically**: the console proactively reads the account's credits overview to decide whether it is blocked; because the billing page opens in a new tab, "buy credits, switch back to the console" is the main self-healing path — the check re-runs when the tab regains focus, with no page reload needed. A top-up that just completed may itself take a few minutes to take effect (see the third row above).
- **No toast, no transcript entry**: an ordinary send failure raises an error toast and leaves an error status in the conversation; a billing refusal is the deliberate exception and produces neither — feedback lives only in the banner, which neither scrolls away with the conversation nor invites you to retry into a fresh copy of the same refusal.
- **Not shown on-premises**: an on-premises deployment is neither metered nor billed, so there is no allowance to block.

For the billing model, metering and credit-draw order, package validity, and refund rules, see <a href="/en/ai-sre/credits">AI SRE Billing</a>.
...
|---|---|---|
| agent | Subagent / A2A subagent dispatch | Has its own child session — see "Subagents" above |
...
Forking is useful when you want to try another path from the same investigation context. The new session keeps the conversation and tool-call history up to the selected turn, but it never inherits the source environment binding on the server, nor the query-editor binding — neither the source datasource nor the statements it had already validated: a forked session is attached to no editor, so you must pick a datasource again before running queries. Available environments are filtered by the target scope. If you are not a member of the source team, the dialog changes to Personal; if the source environment is deleted, offline, never connected, or outside the target scope, it changes to **Auto**. Later turns from the source session are not included. The forked session includes a "Forked from conversation" divider; click it to return to the source position in the original session.
...
- **No model identity, no cost**: the export **deliberately carries neither the model identity nor any cost figure** — this NDJSON is a customer-downloadable file, and the serving chatmodel key plus the per-call token counts are exactly enough for a customer to reconstruct our procurement pricing from the vendor's public rate card. So the export gives you token counts (`usage`) and nothing else; the usage frames pushed to your browser are redacted the same way. Internal consumers that need the model should read the database (`t_session.chat_model` / `t_event.usage_metadata`) rather than the export.
