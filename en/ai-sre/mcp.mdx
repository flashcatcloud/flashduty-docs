...
<AccordionGroup>
  <Accordion title="Enable / Disable" icon="toggle-on">
    Toggle the switch in the list. Only **enabled** servers appear in a session's `<available_mcp_servers>` list; disabled servers are invisible to agents and cannot be called. Built-in servers are **always enabled** and their toggles cannot be changed.
  </Accordion>
...
<Tip>
To confirm which tools a given MCP server actually exposes in a particular environment, the most direct way is to ask the agent in **Chat** to run `mcp_load` against that server — it will list the tools and their parameter schemas. See [Chat & Sessions](/en/ai-sre/sessions).
</Tip>
...
MCP shares the same **two-level scope** model as other resources (Skills, Knowledge Packs, A2A agents, Environments), determined by the `team_id` field on each row:
...
**Runtime visibility**: At session start, an agent's `<available_mcp_servers>` list includes only **account-level** MCP servers and servers belonging to the **team bound to the current session**. Once the agent reads a team's knowledge (`.flashduty/knowledge/team_<id>/DUTY.md`), that team's MCP servers and Skills are mounted into the session on demand. **The account is the only security boundary at runtime; the team is an ownership and editing tag only.**

<Note>
For the complete scope model (including permission rules and runtime-loading details), see [Resource Scope & Permissions](/en/ai-sre/customize).
</Note>
...
<CardGroup cols={2}>
  <Card title="Resource Scope & Permissions" icon="sliders" href="/en/ai-sre/customize">
    Learn about account-level and team-level scope, edit permissions, and runtime loading behavior.
  </Card>
  ...
  <Card title="A2A Agents" icon="satellite-dish" href="/en/ai-sre/agents">
    A2A agents share the same authentication modes and scope model as MCP.
  </Card>
