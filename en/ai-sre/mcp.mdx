...
<Tabs>
  <Tab title="Service authorization (default)">
    ...
    Also, any value starting with `enc:v1:` is **rejected** on save (`invalid value for key ...: the "enc:v1:" prefix is reserved for encrypted values; please re-enter the plaintext secret`) — the prefix is reserved for values encrypted by the platform and is not accepted as plaintext under any key. Enter plaintext credentials; do not paste back a previously stored encrypted value.
...
<Note>
If a tool call returns `MCP server "xxx" is configured but its stored credentials could not be read ... Ask your administrator to re-enter this server's credentials in the console`, the server is configured but its stored credentials failed to decrypt (e.g., the encryption key changed or the value was corrupted). Only that server is **skipped** — other MCP servers and all sessions keep working. Re-enter the credentials for that server in the console to restore it.
</Note>
