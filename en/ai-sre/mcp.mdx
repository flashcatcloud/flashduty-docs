...
| --- | --- | --- | --- |
| Name | string | Yes | The server name, used as the identifier when agents call it (e.g., `sqlite-explorer` in `mcp:sqlite-explorer/query`). Must start with a letter; may only contain letters, digits, `-`, and `_`; length 1–255; it cannot contain `@` (reserved as the separator for team-qualified addresses). Names are **case-insensitive and unique within a scope** (account or one team); **different teams may own servers with the same name**, in which case the session addresses them with the team-qualified form `name@team_id`, and duplicate-name errors state the scope where the clash occurred. Cannot duplicate a built-in server name. |
...
<Warning>
Credential encryption depends on a deployment-level **credential master key** (for self-hosted / BYOC deployments, `credentialMasterKey` in `etc/srv.yml`). If that key is missing or invalid, saving an env/headers map that contains sensitive keys fails with "unable to save credentials: the deployment's credential key is missing or invalid. Contact your administrator" — ask your deployment administrator to check the credential key configuration. In addition, plaintext values carrying the encrypted-marker prefix under any key are rejected on save, and updating an existing server re-encrypts legacy plaintext credentials into ciphertext.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Edit" icon="pen-to-square">
    Click the edit button (or click the row) to open the form. You can modify the name, transport, description, endpoint/command, authorization mode, and scope. Opening the form is a pure database read and does not connect to the remote server. **After a successful create, or after an edit that changes connectivity-relevant fields (transport, command, arguments, environment variables, URL, headers, timeouts, execution environment, authentication, etc.), the system automatically re-probes the tool list in the background and refreshes the tools cache and AI description** — no manual trigger is needed; changes to metadata only (name, description, scope) do not trigger a re-probe. Manual connectivity verification is done through the "Test" entry. If you do not have edit permission, the form opens in **read-only** mode with an explanation.
  </Accordion>
...
<Note>
**Troubleshooting: a server shows as "configured but unreadable"**: if an enabled MCP server reports "configured but unreadable" or is skipped during a session (instead of "not found"), its stored secrets failed to decrypt — usually because the credential master key changed (rotation or misconfiguration) or the data is corrupted. That server is skipped without affecting other servers in the account. To fix it, re-save the server's credentials (edit and submit once), or check the deployment's `credentialMasterKey` configuration.
</Note>
...
**Name uniqueness**: MCP server name uniqueness is per scope — names must be unique (case-insensitively) within the account scope or within one team; two different teams may each own a same-named server. When a team-level server shares its name with another server visible in the same session, its call address is qualified as `name@team_id`.
