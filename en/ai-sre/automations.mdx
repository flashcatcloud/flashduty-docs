...
| Run now (API) | Call `POST /safari/automation/rule/run` to start one real run manually. The endpoint performs preflight checks first, returns a `run_id` after accepting the run, and returns `session_id` after the session is created. Manual runs are limited to one per rule per minute. |
