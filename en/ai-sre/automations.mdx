...
The form also offers a **Merge similar incidents into one analysis** switch under this trigger, on by default: when it is on, a new incident from this rule is first compared for similarity against the incidents being analyzed now (or analyzed within the last 10 minutes); one scoring at least 0.9 — the same bar intelligent grouping uses — joins that analysis instead of starting a session of its own, and one analysis takes in at most 20 incidents. When it is off, every incident is analyzed on its own. The rule's detail page shows the switch's state (**Enabled** / **Disabled**) in the trigger information.
...
| `oncall_incident_merge_similar` | boolean | Whether **Merge similar incidents into one analysis** is on. Defaults to `true` (a request that omits the field is treated as on). When on, a new incident scoring at least 0.9 joins the analysis in progress instead of creating a session of its own. |
...
When an incident is folded into another analysis, the platform performs no incident merge and each incident's bindings stay unchanged: the Agent receives an `incident_joined` task notification telling it that the new incident joined the analysis in progress because it cleared the similarity bar. The analysis summary comment gains a **concurrent incidents** dimension in turn, listing other incidents created in any channel within 15 minutes either side of this incident's start time, as clues to a common root cause.
...
| Column | Notes |
|---|---|
| Name | The hidden session behind this run. The backend batch-resolves each run's session title (`session_name`, the hidden session's auto-generated name); when it can't be resolved, the cell falls back to the session ID (`session_id`). A run folded into another analysis by similar-incident merging has no session of its own, so the Name column shows `#<incident-no>` plus the incident title instead; the run record also carries `joined_session_id` / `joined_session_name` fields |
...
Click any row to jump to the chat page of the hidden session for that run (`chat?session_id=<sessionID>`), where you can view the full messages, tool calls, and artifacts of that run; a run folded into another analysis by similar-incident merging opens the analysis session it joined instead.

A run that ended abnormally (status `failed`, `blocked`, `abandoned`, or `skipped`) also shows a failure-reason line under the session name in the Name column: when AI SRE is not enabled, or this period's credits or balance are used up, the line says the run did not execute and links to **Go to AI SRE billing**; when the On-call subscription has lapsed or its plan does not include AI SRE, it says AI SRE is paused and links to **Go to On-Call subscription**; the remaining cases use fixed wording — the run did not finish in time and was abandoned, this run was interrupted, or the run ended without a final reply; and a run folded into another analysis by similar-incident merging shows **Joined "analysis name"**.
