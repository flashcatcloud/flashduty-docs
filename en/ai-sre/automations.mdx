...
<Note>
**Cloud sandbox idle reclaim**: when an automation run uses a cloud sandbox, the sandbox is reclaimed after roughly **13 minutes** of idleness (`idle_pause_seconds` 180s + `automation_idle_destroy_seconds` 600s) — noticeably shorter than the ~33 minutes for regular interactive sessions (180s + `idle_destroy_seconds` 1800s). This is deliberate: an automation run is one-shot, and its workspace is almost never revisited once the run finishes. After the sandbox is reclaimed, sending a follow-up message to that run rebuilds a brand-new sandbox (the UI shows "Rebuilding session…"), and any temporary files in the old workspace are lost; content published as an **Artifact** during the run is unaffected and kept long-term. For output you need to keep, have the agent publish it as an Artifact.
</Note>
...
A run with status `failed` also carries a stable, machine-readable `error_code`, plus an `error_message` that is sanitized and safe to show to users:

| `error_code` | Meaning |
|---|---|
| `turn_failed` | The turn failed to execute (a model call, tool call, or runtime error); `error_message` is the sanitized, user-facing error text |
| `no_final_response` | The turn finished normally but the agent produced no final response |
