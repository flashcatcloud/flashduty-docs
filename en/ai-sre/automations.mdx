...
<Steps>
  ...
  <Step title="From a preset template">
    Below, a set of **preset template** cards is listed (delivered by the backend per UI language: `zh-CN` for Chinese, `en-US` for English). Common ones include **Weekly Insights**, **[Alert Governance](/en/ai-sre/skills)**, **On-call Incident Analysis**, and **On-call Load Analysis**. Click any template card to prefill the form with the template's name and task prompt; tweak from there.
  </Step>
</Steps>
...
When a run hits a deterministic failure (content-safety review rejected, runtime environment unusable or expired, insufficient account balance) it gets no extra recovery turn and is recorded as `failed` outright; a retryable engine-class failure (the model service being temporarily unavailable, for example) is retried automatically once, on the same turn.
