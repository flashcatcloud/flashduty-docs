...
### Refused runs: not enough credits

An automation run is a new top-level AI SRE run, so it goes through the same credits admission check as the other entry points: when the account has no credits available, the run does not start — no hidden session is created and no turn is enqueued, and the run history keeps only a `failed` record whose `error_code` is `BalanceNotEnough` and whose `error_message` is the sentence in the table below.

| Why it was refused | Sentence recorded on the run and returned verbatim to the caller |
|---|---|
| AI SRE is not enabled for the account | `AI SRE is not enabled for this account. Enable it to continue.` |
| This period's credits are used up | `This account has used its full AI SRE credits allowance for this period. Buy a credit package or raise the usage limit to continue.` |
| Wallet balance is insufficient (accounts funded by pay-as-you-go alone) | `The account balance is insufficient. Top up the wallet to continue using AI SRE on pay-as-you-go; a recent top-up may take a few minutes to take effect.` |

Admission is decided once, when a new top-level run starts: a run already in flight is never cut off — even if it drains the account before finishing, it completes and its usage is metered as usual; subagent work delegated by an already-admitted turn is a continuation of that same run and is not checked again. A manual run or an HTTP POST trigger returns the sentence along with `BalanceNotEnough` to the caller; a scheduled run has no request context, so the sentence recorded on it is always the Chinese one.

For credits, packages, and usage limits, see <a href="/en/ai-sre/credits">Credits and billing</a>.
