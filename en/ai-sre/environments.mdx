...
| Environment variables | No | `.env` format (`KEY=value`, one per line, multi-line quoted values supported), up to 32 KB, with a live byte counter in the UI. Platform-reserved keys cannot be set here — see below. |
...
<Note>
**Platform-reserved keys cannot be set here.** These names are injected by the platform, and a template that sets one is rejected on save.

- Exact matches: `FLASHDUTY_APP_KEY`, `FLASHDUTY_BASE_URL`, `FLASHDUTY_DOCS_URL`, `GH_TOKEN`, `GH_PROMPT_DISABLED`, `PATH`, `HOME`;
- The auth / host / TLS settings the GitLab CLI (`glab`) reads: `GITLAB_TOKEN`, `GITLAB_ACCESS_TOKEN`, `OAUTH_TOKEN`, `GITLAB_HOST`, `GL_HOST`, `GITLAB_URI`, `GITLAB_API_HOST`, `GITLAB_SSH_HOST`, `GITLAB_SUBFOLDER`, `CA_CERT`, `SKIP_TLS_VERIFY`;
- Prefix matches: `GIT_CONFIG_`, `GLAB_`, and `GH_TOKEN_` (the last covering extra-installation GitHub tokens such as `GH_TOKEN_1`, `GH_TOKEN_2`).

The server's own error is the authority on the list. One of these, `FLASHDUTY_DOCS_URL`, is injected into **every bash execution** and points at the product-docs tarball, so the built-in `flashduty-docs` skill can download the docs on demand — a template neither supplies nor overrides it.
</Note>
...
<Tabs>
  ...
  <Tab title="Docker">
    ...
    The `--restart=always` flag brings the container back up automatically after a host reboot; if the container is already running, run `docker update --restart=always flashduty-runner` to apply the same restart policy without recreating it.
  </Tab>
...
A Runner can install the command-line tools it uses itself — you do not have to prepare them on the host:

- **Manual install**: `flashduty-runner tool ensure gh`, `glab`, `uv`, or `node`. If a binary of the same name is already on `PATH`, the Runner reuses it and installs nothing; otherwise it downloads the pinned upstream archive, verifies the embedded SHA-256, and installs it atomically into the runner-owned tools directory.
- **Automatic install**: before it launches a stdio MCP connector, the Runner installs the runtime that connector needs — `uv` (which provides `uvx`) or `node` (which provides `npx`) — so a host without either runtime can still run those connectors.

Both paths need the Runner host to have **outbound network access** to fetch the archives. For intranet or air-gapped hosts, set `FLASHDUTY_RUNNER_TOOL_MIRROR_URL` to a private mirror with the same `<name>/<version>/<asset>` layout; this explicit override never falls back to the official source.

Tools installed this way lead the `PATH`, so a name that exists both in the tools directory and on the host resolves to the Runner-installed version, while every other host command stays available.
...
<Note>
These are **two different layers** — don't conflate them:

1. **A session binding never falls back** (the two paragraphs above describe this layer): the Environment a session locked to on its first send does not change. If that Runner later goes offline or is deleted, AI SRE never silently moves the session to another environment — it says so and refuses the send.
2. **A selection-level fallback announces itself**: if you arrive via an entry that pins a **one-off Runner** — "Let AI SRE install it on the Runner", for example — and that Runner is no longer usable before the first send (deleted, or outside the set you can pick), belongs to a team you are not in, or its team cannot be confirmed, the composer's environment selector **explicitly switches back to Auto** and shows a warning — it does not silently land you on an environment you never picked. The three messages are:

   - "This Runner is no longer available - switched to Auto."
   - "This Runner belongs to team 〈team〉, which you're not a member of - switched to Auto."
   - "Couldn't confirm which team this Runner belongs to - switched to Auto."

   Along the same lines, a remembered composer selection that does not match the session's team is simply left off the first send, and the server resolves it as Auto.
</Note>
...
<Accordion title="Session times out or does not respond" icon="server">
  If a session is pinned to an offline Runner, AI SRE does not silently move it to another environment — that rule covers sessions **already locked** to an Environment; for the selection-level fallback before the first send, see [Selecting an environment in a session](#selecting-an-environment-in-a-session). Restore that Runner, or start a new session and choose Auto or cloud Sandbox. If the Runner is online but the task fails, the likely issue is network or permissions between the Runner and the target resource.
</Accordion>
</AccordionGroup>
