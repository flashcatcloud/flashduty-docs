...
<Tabs>
  <Tab title="Linux (systemd)">
    ...
    The script installs the binary, creates a systemd service, writes `/etc/flashduty-runner/env`, and installs `/etc/tmpfiles.d/flashduty-runner.conf` — a drop-in that keeps the service's `PrivateTmp=true` private /tmp directories out of systemd's age-based `/tmp` cleanup, so a running service's private directory is never deleted (older systemd ships no such exclusion). The same command works for fresh installs and upgrades; if the latest version is already installed, it skips automatically.
  </Tab>
  <Tab title="Docker">
...
- **Linux (systemd) / manual install**: the install script's `--uninstall` flag (removes the service while preserving config) and `--purge` flag (removes config and data); both also remove the systemd unit and `/etc/tmpfiles.d/flashduty-runner.conf`.
