...
<Note>
If the account authorizes multiple GitHub organizations, `git clone` and `git push` can reach every granted repository. `gh` API calls, including PR and Issue commands, currently use only the oldest authorized organization. For a repository in another organization, use `git` for cloning, committing, and pushing.
</Note>
...
The **GitLab** App connects a GitLab instance — **GitLab.com** or your own **self-managed** instance. Once authorized, AI SRE can read code and investigate changes / MRs, and — when you ask — file an issue or open an MR, in the repositories you authorized. Just like GitHub, **you never paste a personal token**: once Flashduty gets an OAuth grant, it provisions a dedicated bot identity for your account to do the actual work.

### Connecting a GitLab Instance

<Steps>
  <Step title="Start the connection">
    Click **Connect** on the GitLab card, and choose the instance type — **GitLab.com** or **Self-managed**.
  </Step>
  <Step title="Select the execution environment">
    Before starting OAuth, select the Environment that connects to GitLab. Use the **cloud Sandbox** for GitLab.com or a publicly reachable instance; for a self-managed GitLab instance in a private network, select an online **BYOC Environment** that can reach it. OAuth callbacks, repository selection, bot provisioning, token rotation, and disconnecting all use this Environment to reach GitLab.
  </Step>
  <Step title="(Self-managed only) Register an OAuth application on GitLab first">
    The connect wizard shows a **copyable Redirect URI**. Take it to your GitLab instance and create an OAuth application: a group **Owner** does this under **Group Settings → Applications**, or an instance admin under **Admin Area → Applications**. Fill in the Redirect URI the wizard gave you, check the **api** scope, and check **Confidential**. GitLab then issues an **Application ID** and a **Secret** — paste both back into the wizard's register step.

    <Tip>Connecting **GitLab.com** skips this step — Flashduty already has an official OAuth application registered on GitLab.com, so you go straight to authorization.</Tip>

    <Tip>The “Self-managed” path works for **any URL-reachable, API-compatible GitLab instance** — including **JihuLab (GitLab’s China distribution) SaaS at jihulab.com** and its self-managed distribution: enter `https://jihulab.com` (or your private deployment’s URL) as the instance address and register your own OAuth application there.</Tip>
  </Step>
  <Step title="Authorize on GitLab">
    You're taken to GitLab's official authorization page; sign in with your GitLab account and confirm.
  </Step>
  <Step title="Pick the groups and projects to authorize">
    After authorization, AI SRE shows a repository picker listing the **groups where you have the Owner role** and the **projects where you have the Maintainer role**. Select the groups / projects you want AI SRE to access and save — selecting a group covers all of its projects, including ones created later.
  </Step>
</Steps>
...
<Warning>
**One restriction on GitLab.com**: free and trial namespaces cannot create **group access tokens**. If you need only selected repositories, choose specific **projects** in the repository picker — project access tokens work in those namespaces. To cover an entire group, upgrade the namespace to a paid plan. A trial namespace can keep only one project access token at a time; if you see `limit reached`, revoke unused tokens in GitLab under **Settings → Access tokens**, then wait a few minutes for the slot to be released.
</Warning>
