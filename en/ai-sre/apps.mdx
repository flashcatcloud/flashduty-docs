...
<Steps>
  <Step title="Enter the instance address">
    Click **Connect** on the GitLab card. The address field is prefilled with **`https://gitlab.com`** by default; to connect JihuLab or another self-managed instance, replace it with that instance's address — for example `https://jihulab.com` (JihuLab SaaS) or your private deployment's URL.
  </Step>
  ...
  <Step title="Authorize in the popup">
    The wizard opens a popup that loads GitLab's official authorization page; sign in with your GitLab account and confirm. The popup closes automatically once authorization completes, and the wizard advances to the next step.
  </Step>
