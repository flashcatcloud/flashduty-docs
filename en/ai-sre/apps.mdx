...
<Steps>
  ...
  <Step title="Set namespace access">
    Choose **All namespaces** or **Specific namespaces**. All namespaces applies one permission level to current and future namespaces. With specific namespaces, set **Read-only**, **Read + limited modifications**, or **Full access** for each namespace. Leaving the specific-namespace list empty allows the agent to read only basic cluster metadata. Full access for a specific namespace allows every operation on every resource in that namespace while basic cluster metadata remains read-only. Full access for all namespaces grants `cluster-admin`, including access to cluster-scoped resources and cluster RBAC.
  </Step>
...
<Warning>
After changing namespace access or permissions, run the installation command again so the in-cluster RBAC is updated. Before choosing **Read + limited modifications**, confirm that AI SRE is allowed to perform those actions in the namespace. **Full access** can read, create, change, and delete resources; read and change ConfigMaps and Secrets; enter or attach to containers; forward ports; and change namespace RBAC. Use it only when it is necessary and has passed your access review.
</Warning>
