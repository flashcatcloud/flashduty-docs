...
<Update label="2026-09-23" description="🔌 Checkly alert integration added">

### Checkly alert integration added

A new Checkly synthetic-monitoring alert integration is available, so Checkly check state changes can be pushed straight into Flashduty On-call:

- **Status and severity**: failures (`ALERT_FAILURE`, `ALERT_FAILURE_REMAIN`, `ALERT_DEGRADED_FAILURE`) and recovery (`ALERT_RECOVERY`) are Critical; degradations (`ALERT_DEGRADED`, `ALERT_DEGRADED_REMAIN`, `ALERT_FAILURE_DEGRADED`) and SSL certificate expiration (`ALERT_SSL`) are Warning; `NO_ALERT` creates no event. An empty or unknown `alert_type` returns an invalid-parameter error
- **Alert correlation**: the trimmed `check_id` is the Alert Key, so a Check's failures, degradations, ongoing issues, and recovery all update the same alert. `ALERT_SSL` is a standalone one-shot event and uses a new Alert Key even when the payload carries a `check_id`
- **Where to receive it**: configure a shared integration under **Integrations → Alert Events → Checkly**, or add an exclusive integration inside a channel; copy the generated push URL into a Checkly Webhook Alert Channel
- **Labels**: `check`, `source=checkly`, `check_id`, `checkly_alert_type`, `check_name`, `check_type`, `group_name`, `region`, `run_location`, `is_reminder`, `reminder_sequence`, and JSON-encoded `tags`
...
<Update label="2026-09-16" description="💰 AI SRE billing goes live, the plugin catalog consolidates onto Overview">
...
- The subscription card gains a self-service **Disable AI SRE** button (it requires wallet purchase permission): it first asks for confirmation with "Disable AI SRE?" and then places a ¥0 unsubscribe order. **Purchased credit packages and gift credits are retained**, and you can enable the service again at any time. Disabling checks neither the wallet balance nor a valid On-call subscription, so an account in arrears or with a lapsed subscription can still turn it off
- Disabling completes only once the server confirms the service is off: when that confirmation fails the order is not completed (the console shows "Failed to disable AI SRE. Please try again." — retry directly) rather than half-applying. Self-service disabling is offered for **AI SRE only**; cancelling any other product still needs support

See [AI SRE Overview · Activation and billing](/en/ai-sre/overview#activation-and-billing) and the [AI SRE Credit Package Purchase Agreement](/en/compliance/ai-sre-credit-package-purchase-agreement). Enabling and disabling are covered in full under [Activating and closing AI SRE](/en/ai-sre/overview#activating-and-closing-ai-sre).
...
- The catalog no longer lists the **PagerDuty / incident.io / Better Stack / Rootly** MCP templates: all four are incident-management and on-call products themselves, overlapping with the platform's own incident center and on-call capability. Connections you already created from those templates keep working, and nothing needs to be done
...
### Data sources now support team authorization

Data sources gained instance-level authorization: `manage_team_id` (the single managing team) and `readonly_team_ids` (the teams allowed to query and reference it) travel with the data source's create and update requests. Leaving them unset keeps today's behaviour exactly — the whole tenant can query and manage, and existing data sources need no change at all.
...
<Update label="2026-09-15" description="📉 Cloud-log data sources no longer support per-series no-data detection">

### Cloud-log data sources no longer support per-series no-data detection

Tencent Cloud CLS and Alibaba Cloud SLS are cloud-log data sources. They no longer support **per-series** no-data detection ("alert if previously found data is now missing") and keep only **Alert when all queries return no data**:

- When you edit an alert rule, the per-series detection card no longer appears for cloud-log data sources. For these alerts the **recovery mode** is fixed to "recover automatically when any query finds data again" and is shown as read-only text — there is no dropdown
- Existing rules with per-series detection enabled get that check **automatically turned off** when you open the edit page, with a notice; it takes effect on save. Not opening the edit page leaves the rule configuration untouched. If that was the rule's only enabled check, the save is blocked by the "at least one check" validation and you must reconfigure explicitly
- Tencent Cloud CLS already rejected this check through a hard server-side validation; this change aligns the cloud-log data sources on one product rule

We recommend switching these rules to **Alert when all queries return no data**. See [Alibaba Cloud SLS](/en/monitors/alert-rules/sls) and [Tencent Cloud CLS](/en/monitors/alert-rules/tencent-cls).

</Update>

<Update label="2026-09-11" description="🔗 Invite links and QR codes, join restrictions, AI query assistant for alert rules">

### Invite links and invite QR codes

**Members → Invite members** gains **Invite link** and **Invite QR code** tabs, so an organization can be shared directly instead of typing each email address or phone number:

- **One link per account**: generated the first time either tab is opened; the link and the QR code share a single invite, valid for a fixed **42 days**
- **Reset rotates it**: resetting invalidates the old link and QR code immediately, while the default role is unaffected
- **Default role on joining**: choose one or more roles; picking a role that includes Admin asks for confirmation first, so admin rights are not handed to everyone holding the link
- **Landing page**: invitees open the link or scan the code to reach a public landing page and join after signing in — see [Team Management](/en/platform/team-members)

### Join restriction: email domain allowlist

**Organization Information** gains a **Join Restrictions** section that only lets people whose email domain is on the allowlist join the organization:

- An option to allow subdomains lets a listed domain cover its subdomains
- The allowlist is checked only when you **invite members** and when someone joins through an **invite link / QR code**; it plays no part in sign-in or authentication, and members who already joined are unaffected

See [Organization Information](/en/platform/organization-info#join-restrictions).

### Free notification quota reminders

**Organization Information → Notification settings** gains **Free Notification Quota Reminders**, two toggles that are on by default and can be turned off:

- **Remind at 20% left**: fires once per quota period per notification channel when the free quota drops to 20% or below while still above 0
- **Remind when exhausted**: fires once when the free quota first reaches 0
- Only successfully sent voice, SMS, and email notifications on paid accounts count toward the quota; reminder emails go to the primary account and the verified addresses of current Admin members. The [pricing page](/en/platform/pricing) describes the free quotas and reminders as well

### AI query assistant for alert rules

Every query input in alert rules and the entity tree gains a **Write with AI** entry point:

- The assistant reads the current rule's context — data source, query type, the alert thresholds already filled in, and the other query statements — to generate or rewrite the current query
- The statement and the data source's private parameters are written back into the input; writing back only changes the current form and never saves the rule

See [Quickstart](/en/monitors/quickstart/quickstart).

### Incident follow-ups

The incident overview gains a **Follow-ups** card on the right:

- Closing an incident automatically converts unfinished action items into follow-ups and records it on the incident timeline
- The incident list gains an "Unfinished actions / follow-ups" filter (`has_unfinished_work_item`) to narrow the list down to incidents with outstanding work

See [Action Items and Follow-Ups](/en/on-call/incident/work-items).

</Update>

<Update label="2026-09-08" description="📋 Copy silence rules, batch-operation form improvements, and monitor targets retirement">
...
### Monitor targets and monit-agent host diagnostics retirement

- **Monitor targets retired**: The monitor targets feature and its documentation entry have been retired. The target list, column selection, and per-row AI analysis are no longer available.
- **monit-agent host diagnostics retired**: the `fduty monit-agent` command tree and the `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap operations are removed together with the host-tool API; host diagnostics (host-tool / monit-agent) were retired along with them and have no replacement command — use `monit datasource-tools-invoke` (or `monit-query --tool` in the CLI) for database and middleware diagnostics
...
The console now has a unified **task panel** that centralizes agent / shell / monitor background tasks:

- **Task panel**: grouped into "Needs you / Running / Finished", with a badged task button in the header; each session row in the session list shows a 🔭 background-task badge
- **First-class background commands and monitors**: bash `run_in_background` commands and monitor watchers no longer vanish when a turn ends — the detail panel shows run duration, event count, output, deadline, and exit code, and a running task can be stopped directly
- **IM collaboration**: when a turn ends with persistent tasks still running, a "🔭 Monitoring" card is delivered to IM and the conversation stays open; notification turns with no new content are delivered silently
- **Cleanup coupling**: deleting a session or rolling back also terminates the related background tasks

See [Sessions](/en/ai-sre/sessions) and [IM Collaboration](/en/ai-sre/im).

### RUM remote config extension

The **WeChat Mini Program** app type now has a "Remote Config" tab:

- **Configurable items**: only **session sample rate** and **custom config** (the Mini Program SDK has no replay, trace, or other collection channels)
- **Publishing and version history**: publishing adds an optional "change note"; version history shows the operator, and versions produced by a rollback carry a "same content as vN" label marking the version they are equivalent to
- **Activation**: the current console does not offer "condition rules" editing or "immediate activation" (reserved in the protocol) — every save takes effect on the next new session
- **Custom config limits**: key names up to 64 UTF-8 bytes, JSON values nested up to 3 levels

See [App Management](/en/rum/quickstart/app-management).

### Incident list filter bar

The new filter bar is now the default on the incident list:

- **"Assigned to me" toggle** moves to the front of the row, with time / severity / status moved forward and the search box moved to the end
- **"Related to me" filter removed**
- **"Save filter view" temporarily removed**, to be restored in a later release

See [View Incidents](/en/on-call/incident/search-view-incident).

</Update>

<Update label="2026-09-04" description="💳 Payment and top-up documentation, RUM dashboard trend charts">

### Payment center: payment and top-up

The billing behavior of the **Payment** center is now fully documented in [Pricing · Payment and Top-up](/en/platform/pricing#payment-and-top-up):

- **Three top-up channels**: online payment (Alipay / Stripe), corporate transfer, and cloud market, each with its own settlement currency and crediting time
- **The default payment channel follows the interface language**: Alipay on the Chinese UI, Stripe on other languages, both switchable before you submit
- **Stripe credits at a fixed exchange rate**: each USD option shows the exact CNY credit instead of an estimate
- **Narrower trigger for automatic card charging**: the saved card is now charged only when the balance is short **at renewal**, not for outstanding arrears; at most one automatic charge per account every 24 hours, and it stops after 3 consecutive failures
- **Transactions show who initiated the charge**: system-initiated card charges are labeled as an automatic top-up (Auto top-up), separate from Stripe top-ups you start yourself
- **Tighter invoice entry**: the **Invoice** column links only to invoices already issued for a Stripe top-up; all other channels still go through the invoice form
- The top-level navigation item **Subscription management** is renamed to **Plan**

### RUM Native dashboard trend charts

The **Error Type Distribution Trend**, **Version Crash Distribution Trend**, and **System Version Error Trend** charts on the Native dashboard are **stacked bar charts**; the docs previously called them stacked area charts and have been corrected to match the actual chart type (the grouped trend series in these charts are also fixed to render).
...
- Draft states include pending review and published; a swept or unknown draft link shows as expired in the console
...
- Create a War Room, add members, and dissolve it from More Actions; adding members and dissolving remain available after the incident is closed for post-mortems
...
<Update label="2026-07-22" description="💬 @mention notifications in incident comments">

### @mention notifications in incident comments

Mention a member with `@` in incident comments or post-mortem reports, and the mentioned person is notified in real time — key collaboration signals no longer get buried:

- Type `@` in the incident detail comment area or in a post-mortem report to select and mention a member. The notification carries the comment content, an incident summary, and a detail link that jumps straight to the incident
- Notification channels follow the mentioned person's preferences: SMS, email, and all IM apps (DingTalk, Feishu/Lark, WeCom, Slack, Microsoft Teams) are supported, with email as the default
- Adjust how you receive these under **Personal Center → Notification Preferences → Collaboration & follow-up → Collaboration Mention**; the same comment never triggers duplicate notifications
...
AI SRE adds Automations, which run hidden sessions on a schedule, through an API trigger, or from On-call incident events to produce health checks, insights, or post-mortems.
