...
<Update label="2026-09-23" description="🤖 AI SRE queries Monitors data sources directly, similar-incident merge analysis, configurable Jira status mapping, and more">
...
- **DingTalk ack emoji receipt**: after a member acknowledges (acks) an incident in IM, AI SRE confirms with an emoji receipt; the emoji is picked randomly from each platform's emoji pool and **removed once delivered**
...
- **Status mapping** is now configurable: map the Pending / In Progress / Closed states to Jira statuses individually, or leave blank to use the default mapping
- **Field mapping** gains incident attribute sources: **incident ID** and **incident link**

See [Jira Two-Way Sync](/en/on-call/integration/webhooks/jira-sync).

### Work items can be assigned to AI SRE

The assignees array's `type` now supports `person` or `ai_sre`, so a work item can be assigned to AI SRE; the CLI gains an `--assignee-type` filter. See [CLI](/en/developer/cli) and [Action Items and Follow-Ups](/en/on-call/incident/work-items).
