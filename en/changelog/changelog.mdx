...
<Update label="2026-08-27" description="🔐 SSO settings page redesign and Explore enhancements">

### SSO settings page redesign

The SSO settings page has been aligned with the system settings style: configuration now lives directly on the page (no more drawer), organized into sections for connection, field mapping, LDAP sync, and login policy:

- Always-on form: the save bar only appears when there are changes, with a single **Save** and **Cancel** for the whole page — Cancel truly reverts to the server-side saved configuration
- Login policy (create account on login, members can only log in via SSO, disable profile editing for SSO members) is merged into the main form and takes effect on **Save**, instead of applying instantly on toggle
- Field mapping adds a **Stable User ID field** (optional): when set, a member is still recognized as the same person after their email or phone number changes; when left empty, matching falls back to the email and other fields below

### Explore enhancements

Several experience updates to the monitoring Explore workbench:

- **Share links**: the toolbar **Share** button copies the current query link in one click, pinning the time to the exact window the query actually ran — recipients see the results for the same time range
- **Legend filtering**: time series legends can now filter curves by label facets (modeled after Grafana): values within one label key are OR'd, different keys are AND'd, and a key with nothing checked does not participate in filtering
- **Log time distribution**: log queries add a Log Volume chart showing the log count distribution across the entire query window (not just the Top-N rows in the list), with a collapsible header and a show/hide toggle
