...
<Update label="2026-09-10" description="💳 AI SRE billing (credits), automatic action-to-follow-up conversion, and alert list/detail filters">

### AI SRE billing (credits)

AI SRE starts charging on **2026-09-16 (Beijing time)**; usage before that is free. Usage is metered in credits for **model usage, sandbox online time and web search** (sandboxes cost 2 credits per online hour, web search 0.25 credits per call, and models are priced by tier — the current tier is 8 / 32 / 0.16 credits per million input / output / cache-hit tokens):

- **Console entry**: **Payment → AI SRE** shows this period's allowance and pay-as-you-go spend; any member can view it, but **only owner/admin can change the cap**, and the save button follows that permission
- **Three pay-as-you-go states**: `Unlimited`, `Set a monthly cap` (an amount greater than 0), and `Forbid pay-as-you-go (stop running once packages are exhausted)`; once the cap is reached new runs stop, while a run already under way can finish
- **Low-balance SMS reminder**: when this period's usable allowance (including packages about to expire) drops to **20%** of total capacity, one SMS goes to the account's phone number — at most once per billing period, and again after the period resets
- **Accounts that never enabled AI SRE are metered but not charged**: usage is still recorded and metered, but the platform bears the cost and nothing is charged

See [AI SRE billing](/en/ai-sre/credits).

### Actions automatically converted to follow-ups

Closing an incident no longer leaves unfinished actions behind — they become follow-ups in place:

- **In-place conversion**: every unfinished **action** on the incident becomes a **follow-up** on the **same work item** (its type flips, the converter and conversion time are recorded), and it is bound to the post-mortem automatically when one already exists
- **Batch timeline record**: a batch entry is written reading "converted N unfinished actions to follow-ups" (with each action's title attached); the IM notification reads "The incident is closed. N unfinished actions were converted to follow-ups; track them in the incident details."
- **The editable card follows the close**: while the incident is being handled the **Actions** card is editable and the **Follow-ups** card is read-only; after it closes this reverses — the **Follow-ups** card is the editable one
- **Idempotent**: a redelivered close event writes nothing when there is no unfinished action left to convert
- **Incident list filter**: **More Filters** gains **Unfinished actions / follow-ups**, where choosing "Yes" narrows the list to incidents that still carry unfinished actions or follow-ups

See [Actions and follow-ups](/en/on-call/incident/work-items).

### Alert list and alert detail

- **New alert-key filter**: the alert list gains an **Alert Key** (`alert_key`) condition — multi-value free input (separate with Enter), matched exactly against the full value on the server with OR between values
- **New alert-key block on the alert detail**: full-width, monospace and copyable (up to 255 characters); hidden when the alert has no `alert_key`
- **New noise-reduction history panel**: lists the **silence policies** and **inhibition policies** the alert matched, each linking straight to the policy; it appears only when the alert was ever silenced or inhibited, retries on load failure, and shows "No policy history available" when there is none
- **Time condition becomes presets**: **Last 1 day / Last 7 days / Last 30 days** (default **Last 7 days**); the window is pushed down as a relative range, so it slides with the clock on auto-refresh
- **Alert ID validation**: a malformed ID is rejected at the input and warns "Invalid ID format" together with the offending value

See [Alert management](/en/on-call/incident/alert-management).

### Explore: AI-generated queries and datasource query tools

- **Write with AI**: an AI entry point now sits at the top right of the Explore query editor's input; it opens the query assistant panel, which generates a query from the current datasource, query type and time range and writes it back into the editor (the execution window and datasource-specific args are converted along with it)
- **10 unified query tools**: `prometheus.query`, `mysql.query`, `postgres.query`, `oracle.query`, `clickhouse.query`, `elasticsearch.query`, `loki.query`, `victorialogs.query`, `sls.query`, `tencent_cls.query`, sharing the datasource tool-invocation entry with the diagnostic tools
- **Execution semantics**: Prometheus and Loki accept `instant` / `range` (Prometheus `instant` accepts only `to_ms`); the SQL types, SLS and CLS accept only `window`; VictoriaLogs requires `from_ms`, and `limit` plus `direction` (`latest` / `earliest`) apply only under `window`; SLS requires `project` + `logstore` (with optional `powersql`), Tencent Cloud CLS requires `region` + `topic_id` + `syntax` (`cql` / `lucene`); an explicit `null` field is rejected while unknown additive fields are tolerated

See [Explore](/en/monitors/explore/explore).

### CLI and SDK coverage

- **CLI coverage refresh**: the OpenAPI now holds **337 operations**, and the CLI generates commands for **334** of them; the rest are hand-written (such as `session export`)
- **New RUM remote-configuration command family**: `flashduty rum application-remote-config-get` / `application-remote-config-history-list` / `application-remote-config-preview` / `application-remote-config-update` / `application-remote-config-history-revert`
- **Go SDK**: new request-parameter types for the datasource `query` tools (`PrometheusQueryParams`, `MySQLQueryParams`, `SLSQueryParams`, `TencentCLSQueryParams` and more, with optional fields as pointers so "omitted" and "sent explicitly" stay distinguishable, plus a `NewDatasourceQueryInvokeRequest` constructor) and the AlertRules V2 methods (`ReadInfoV2` / `WriteCreateV2` / `WriteUpdateV2`)

See [Command-line tool (CLI)](/en/developer/cli).
...
- **monit-agent host diagnostics retired**: the `fduty monit-agent` command tree and the `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap operations are removed together with the host-tool API; host diagnostics and database/middleware diagnostics both go through `monit-query <datasource-id> --tool '<name>' --params ...`, with `monit datasource-tools-invoke` as the spec-mirror entry for the same API (neither `monit-query diagnose` nor `monit-query data` exists any more)
...
Mention a member with `@` in incident comments or postmortem reports, and the mentioned person is notified in real time — key collaboration signals no longer get buried:

- Type `@` in the incident detail comment area or in a postmortem report to select and mention a member. The notification carries the comment content, an incident summary, and a detail link that jumps straight to the incident
