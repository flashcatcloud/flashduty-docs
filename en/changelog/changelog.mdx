...
<Update label="2026-09-09" description="💳 AI SRE billing starts, unlinking a war room whose IM integration is gone, and AI SRE queued-message coalescing">

### AI SRE starts charging

- **Start date**: AI SRE starts charging on **2026-09-16 (Beijing time)**; usage before that date is free of charge
- **Metering**: credits meter model usage, sandbox online time, and web search
- **Allowance and credit packages**: allowance and credit-package details will be announced separately; read the [AI SRE credits package purchase agreement](/en/compliance/ai-sre-credit-package-purchase-agreement) before buying

### Unlink a war room whose IM integration is no longer available

- **Still listed**: when the IM integration behind a war room is disabled or deleted, the war room stays on the incident detail page with the group name it was created with
- **Admins can unlink**: account owners and administrators can use **Unlink** on the incident detail page in the desktop console; **the original IM group chat is not dissolved**
- **Operations withheld**: adding members and dissolving the war room are not offered until the association is unlinked
- **Recorded**: unlinking is written to the incident timeline as "Requested war room unlink"

See [War Room](/en/on-call/advanced/war-room).

### AI SRE queued-message coalescing

- **Queuing**: messages you send while the current turn is running are queued instead of being blocked by the composer
- **Consecutive messages folded**: several plain messages the same person sends back to back in the queue are folded into one turn for the model; the history still shows each message separately
- **↑ to recall**: with an empty composer, press ↑ to pull your most recently queued message back into the input for editing (recalling withdraws it from the server-side queue)
- **Simpler interaction**: queued messages no longer carry "send now" or "cancel" bubble buttons — ↑ is the only recall affordance
...
- **monit-agent host diagnostics retired**: the `fduty monit-agent` command tree and the `monit targets` / `monit tools-catalog` / `monit tools-invoke` / servicemap operations are removed together with the host-tool API; diagnostics uniformly use the named tools of `monit datasource-tools-invoke`
