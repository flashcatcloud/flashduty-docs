...
<Update label="2026-08-31" description="☁️ New Huawei Cloud AOM alert integration">

### Huawei Cloud AOM alert integration

Alert ingestion adds a **Huawei Cloud AOM** integration:

- Select "Huawei Cloud AOM" in the Integration Center to generate a dedicated webhook push URL; AOM alarms are pushed to Flashduty through an SMN topic HTTPS subscription, with noise reduction and routing applied automatically
- The alarm severity mapping can be adjusted in the integration's custom severity mapping (customSeverityMapping) setting

See [Huawei Cloud AOM Integration](/en/on-call/integration/alert-integration/alert-sources/huawei-aom).
