...
<Steps>
  ...
  <Step title="Dismiss War Room">
    ...
    <Tabs>
      ...
      <Tab title="Mobile App">
        On the incident details page, tap the **⋯** button in the bottom action bar to open **More Actions**, then choose **Add stakeholders to war room** or **Dissolve war room** (requires confirmation). Both actions remain available after the incident is closed, which is handy for post-mortems.
      </Tab>
    </Tabs>
  </Step>
</Steps>
...
<Warning>
If the IM integration used by the war room is later disabled or deleted, **Add stakeholders to war room** and **Dissolve war room** are hidden in both the console and the mobile app. In that case, ask an account owner or account admin to open the **View war room** dialog on the incident details page and use **Confirm unlink**: it removes only the link between the incident and the original group and does **not** dissolve the group chat, and the timeline records it as "Requested war room unlink". Before creating a new room, make sure the original group has been handled.
</Warning>
...
- **Add stakeholders to war room / Dissolve war room**: Also in **More Actions**. Both remain available after the incident is closed (for review), and dissolving requires confirmation; they do, however, require the war room's IM integration to still be available. Once that integration is disabled or deleted, the two entries disappear from **More Actions** and an administrator must unlink the war room from the incident details page in the console; the **War room** entry card on the overview page stays, and tapping it shows "The integration is disabled or deleted. Ask an administrator to unlink the war room from the incident details on desktop. The original group chat will not be dissolved."
...
War rooms integrate deeply with **AI SRE**: when a war room opens, AI SRE automatically runs a first round of diagnosis and posts its findings back to the group; you can also **@ AI SRE** in the chat to follow up or start a new investigation. See the [AI SRE introduction](/en/ai-sre) and [IM platforms](/en/ai-sre/im).
...
<CardGroup cols={2}>
  ...
  <Card title="Incident review" icon="file-lines">
    Generate structured post-mortems from the incident timeline and investigation evidence
  </Card>
</CardGroup>
