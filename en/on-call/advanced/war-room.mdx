...
Key incident operations (acknowledgment, closure, snooze, comments, etc.) are synced to the war room group as messages in real time. Among them, five [action item and follow-up](/en/on-call/incident/work-items) event types are included in the war room's default reply event set and are posted to the group as plain-text messages when they occur:
...
| `i_wi_auto_converted` | `Incident closed. 3 unfinished action(s) became follow-ups; track them on the incident page.` |

- In messages for follow-up operations, `Action` is replaced with `Follow-up`
- `i_wi_auto_converted` fires on incident close: one reply per close, and only when at least one unfinished action item was actually converted; the message names no operator and reports only how many items were converted
