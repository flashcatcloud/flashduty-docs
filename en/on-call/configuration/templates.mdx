...
**Create war room** is a Feishu/Lark-only switch (backed by the `feishu_app_war_room_enabled` field, **off** by default; save the template to apply the change):

- **On**: when the incident is not closed, has no war room yet, and has responders, the Feishu/Lark incident card shows a dedicated **👥 Create War Room** button at the bottom; clicking it starts group creation
