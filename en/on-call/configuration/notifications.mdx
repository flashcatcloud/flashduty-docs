...
#### Cross-team Webhook URLs

When configuring a bot Webhook URL for group notification in an escalation rule, besides addresses already used by **this channel and your team**, you can switch to **Other teams (alerts will be sent to their group chats)** and pick a Webhook URL from another team's channel:

- Once selected, alert notifications are delivered to **that team's group chat**, which is useful for cross-team incident collaboration
- When you select an address from another team, a confirmation dialog tells you which team's channel it comes from; the address **alias automatically gets a source prefix** (editable) and is labeled **Cross-team** in the list for easy identification
- If you select addresses that have never been used by this channel or your team, an additional confirmation dialog lists those addresses and explains that alerts will be sent to the corresponding teams' group chats; the selection only takes effect after you confirm
