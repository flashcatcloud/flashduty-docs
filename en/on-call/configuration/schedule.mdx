...
<!-- REVIEW: Auto-generated stub from coverage-audit (f006). Schedule handoff mechanics and overlap handling lack documentation. -->
## Handoff Mechanics and Overlap Handling

### Handoff Time Configuration

<!-- REVIEW: Feature overview — Add detailed handoff time configuration instructions -->

Rotation handoff is the core mechanism of schedules. The system automatically transfers on-call responsibility from the current person to the next at the handoff time.

| Configuration | Description |
| :--- | :--- |
| **Handoff Time** | The handoff moment when the rotation cycle expires, e.g., every Monday at 10:00 |
| **Timezone Handling** | Handoff time is calculated based on the schedule's configured timezone |

<!-- REVIEW: Verify and add the following:
- Where the schedule timezone is configured and its default value
- Handoff time calculation rules for cross-timezone teams
- Impact of daylight saving time changes on handoff times
-->

### Overlapping Coverage Windows

<!-- REVIEW: Add the following:
- Priority rules when multiple layers are active during the same time period
- How overrides interact with permanent rules when they overlap
- How responders are notified during overlap windows
-->
<!-- END REVIEW -->
