...
<Steps>
  ...
  <Step title="Set Shift Notifications">
    ...
    - **Group Chat Channels**: Send notifications to group chats, supporting the following platforms:
      - Feishu app, DingTalk app, Slack app (select chat groups via integrated IM apps; disabled IM apps stay in the app dropdown, labelled "(Disabled)", and cannot be selected as a notification target — re-enable the app in Integrations first; the picker automatically excludes platform-created chats such as war rooms, and stale targets render as **Chat unavailable** and can be removed)
