...
The system detects that the alert carries a specific label (e.g., `layer_person_reset_0_emails=bob@corp.com` or `layer_person_append_0_emails=bob@corp.com`) and automatically replaces or appends notification targets in level 1 of the escalation rule.
</Step>

<Step title="Dispatch based on the adjusted rule">
...
<Warning>
Dynamic dispatch does not work independently — it depends on an existing escalation rule in the channel. You need to configure an escalation rule in advance as a "template". Dynamic labels only replace or append notification targets (responders, teams, or chat bots) within the rule; other settings (notification methods, timeout, escalation levels, etc.) remain unchanged.
...
<Note>
This guide uses the `reset` replacement mode as an example. To keep the original targets in the template escalation rule while adding responders, teams, or group bots, use the `append` mode. For the complete parameter reference, see [Dynamic dispatch](/en/on-call/advanced/dynamic-notifications).
</Note>
...
Configure an escalation rule in the target channel. The notification targets in this rule can be set to any value (e.g., a default team) — it serves only as a "template". During actual dispatch, the dynamic labels will replace or append the notification targets.
