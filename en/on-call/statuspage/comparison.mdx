...
<Tabs>
  <Tab title="Incident Management">
    ...
    | **Maintenance Events** | ✅ Schedule start/end times with auto-progression and manual override; **maintenance excluded from uptime calculation**, ideal for separating outages from planned maintenance | ✅ Supports Scheduled Maintenance: start time and duration, automatic status progression, and subscriber reminders |
  ...
  <Tab title="Subscription Management">
    ...
    | **Bulk Import/Export** | ✅ Admins can bulk import and export subscribers (import requires Pro; export requires Standard or above) | ✅ CSV import and JSON/CSV export |
    ...
    <Tip>
    Flashduty offers finer-grained subscription management: full, component, and incident-level subscriptions all included.
    </Tip>
  </Tab>
  ...
  <Tab title="Display & Customization">
    ...
    | **Widgets** | ✅ Status badge + event banner embeddable in any website, plus a public JSON snapshot API (public status pages only) | ✅ Status embed supported |
  </Tab>

  <Tab title="Notification Channels">
    | Method | Flashduty | Atlassian Statuspage |
    | --- | --- | --- |
    | **Email** | ✅ Supported (public status pages only) | ✅ Supported |
    | **SMS** | ❌ Not supported | ✅ Supported, but with limited triggers -- intermediate updates do not send SMS |
    | **Webhook** | ❌ Not supported | ✅ Supported |
    | **Feishu/Lark** | ✅ Supported (internal status pages only) | ❌ Not supported |
    | **Dingtalk** | ✅ Supported (internal status pages only) | ❌ Not supported |
    | **WeCom** | ✅ Supported (internal status pages only) | ❌ Not supported |
    | **Slack** | ✅ Supported (internal status pages only) | ✅ Supported |
    ...
    <Tip>
    Flashduty offers notification channels better suited for enterprise collaboration. Internal status pages natively support Feishu/Lark, Dingtalk, WeCom, and Slack notifications, while public status pages support email notifications.
    </Tip>
  </Tab>
</Tabs>
...
Flashduty CLI supports one-click migration of components, sections, incident history, and email subscribers from Atlassian Statuspage (migration requires the Pro plan), with `history.rss` and `history.atom` link format compatibility so existing RSS/Atom subscribers need no changes.

<Steps>
  <Step title="Migrate structure and history">
    Use `flashduty status-page migrate-structure` to automatically import components, sections, historical incidents, and notification templates -- this step does not notify subscribers
  </Step>
  ...
  <Step title="Migrate email subscribers">
    Use `flashduty status-page migrate-email-subscribers` to import subscribers -- imported subscribers are immediately active
  </Step>
...
<Tip>
For detailed command flags and a complete migration walkthrough, see the migration section in [Get started with Flashduty Status Page](/en/on-call/statuspage/get-started).
</Tip>
...
<CardGroup cols={3}>
  <Card title="More Comprehensive Features" icon="layer-group">
    Incident subscriptions, component display controls, maintenance excluded from uptime calculation, native IM notifications, and other **differentiated capabilities**
  </Card>
