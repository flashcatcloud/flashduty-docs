...
Flashduty provides comprehensive migration tools to help you smoothly switch from Atlassian Statuspage with no manual rebuilding required.

<Steps>
  <Step title="Export data">
    Use the Flashduty CLI to connect to your Atlassian Statuspage account and automatically export components, historical incidents, and email subscribers
  </Step>
  <Step title="One-click import">
    Import the exported data into Flashduty Status Page via CLI, preserving the complete component structure and incident history
  </Step>
  <Step title="Switch domain">
    Point your custom domain's CNAME to Flashduty. Flashduty is compatible with `history.rss` and `history.atom` link formats, so existing RSS/Atom subscribers need no changes
  </Step>
  <Step title="Verify and go live">
    Confirm that status page content, component statuses, and subscriber notifications are all working correctly, then go live
  </Step>
</Steps>
