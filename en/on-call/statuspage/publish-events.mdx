...
## Publish from an AI SRE draft

Besides filling in the console form manually, you can let **AI SRE** draft the event content first and then review and publish it yourself. AI SRE's built-in `statuspage_draft` tool can draft three kinds of content: a new incident, a new maintenance, or a progress update appended to an event that is already open.

The flow is "**AI drafts → a human reviews → a human publishes**":

1. Describe what you want to publish in an AI SRE session; the Agent calls `statuspage_draft` to validate the draft and save it server-side.
2. A review card appears in the chat, previewing the full draft (title, status, message, affected components).
3. Click the **Review** button on the card to open the publish form described on this page (the new-event form or the add-update form), with the draft content prefilled automatically.
4. Check every field in the form and click Publish yourself — **the Agent never publishes**; the publish action always belongs to a person.

Drafts follow these server-side rules:

| Rule | Description |
| --- | --- |
| Retention | Retained for **30 days** after creation, published or not, then deleted automatically |
| Publish count | Each draft can be published **exactly once**, preventing duplicate events |
| Field validation | Title 1–255 characters, message required, maintenance end time later than start time, components and statuses validated per event type — the same rules as the manual publish form |

<Card title="Status page drafts" icon="file-pen" href="/en/ai-sre/statuspage-draft">
  Learn the full AI SRE status page draft workflow, review card states, and draft rules.
</Card>
