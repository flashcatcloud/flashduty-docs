...
4. Configure Channels—you can configure multiple, but each channel can have at most **5** Custom Actions
5. Configure Endpoint and custom Headers
6. **TLS Verification** is enabled by default. If the target service uses a self-signed certificate or a certificate issued by an internal CA, you can disable the **TLS Verification** toggle so the system will not validate the server certificate
