...
You can use <code>{"${}"}</code> syntax to dynamically extract values from parameters to construct the request URL. For example, parameter values can be automatically filled from incident labels, without manual input:

```text
https://cmdb.com/vm?sn=${labels.sn}
```

In the above example, <code>{"${labels.sn}"}</code> means the `sn` label value from the incident will be dynamically injected into the URL. If the incident data contains `sn=VM123456`, the final request URL will be:
...
You can use <code>{"${}"}</code> syntax to dynamically extract values from parameters to construct the request URL. For example, parameter values can be automatically filled from custom fields, without manual input:

```text
https://cmdb.com/vm?sn=${fields.sn}
```

In the above example, <code>{"${fields.sn}"}</code> means the `sn` custom field value from the incident will be dynamically injected into the URL. If the custom field data contains `sn=VM123456`, the final request URL will be:
