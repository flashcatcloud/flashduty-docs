...
Configure the following AI SRE controls in the integration configuration page's **Enhanced Features** module. They are enabled by default:
...
- **Allow AI SRE conversations in IM**: Available without enabling War Room. It controls both entry points of this integration — @ AI SRE in group chats and direct chats with the bot. When disabled, the corresponding entry point does not trigger an AI SRE reply.

The two WeCom app modes support different AI SRE entry points. **Allow AI SRE conversations in IM** is shown in both modes, and the integration detail preview also shows its status:

| App mode | @ AI SRE in group chats | Direct chat with the bot |
| :--- | :--- | :--- |
| **Custom App Integration** | Supported | Supported |
| **Third-party App** | Not supported; switch to **Custom App Integration** mode | Supported |

In Third-party App mode, the switch is described as "When enabled, people can start a direct conversation with the bot. @ AI SRE in group chats is available only for self-built enterprise apps."

These controls do not take effect until AI SRE is enabled for the account.

## 4. Configure AISRE

To use AISRE with WeCom, confirm that **Allow AI SRE conversations in IM** is enabled in the Flashduty WeCom integration (War Room is not required), then create an additional Smart Robot in WeCom and connect it to Flashduty in API mode. The Smart Robot credentials are filled in on the integration configuration page of **Custom App Integration** mode.
