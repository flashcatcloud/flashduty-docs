...
## AI SRE Required Configuration and Authorization

The following list includes all configuration and authorization items required when the WeCom IM integration uses AI SRE, including basic notifications, War Room, AI SRE conversations, and AI-generated post-mortem reports. The WeCom custom app console does not expose all of these capabilities as one unified scope list, so confirm them across app visibility, API receive settings, trusted domains, trusted IPs, and API call authorization.

| Configuration or API capability | Purpose |
| :--- | :--- |
| App visibility | Include members who need to receive notifications, link accounts, use AI SRE conversations, or join War Rooms |
| WeCom API call authorization | Required for third-party app passwordless login, message sending, and callback handling |
| App message sending and card update APIs | Send basic notifications and update interactive card status |
| API receive `Token` and `EncodingAESKey` | Receive, verify, and decrypt WeCom app callbacks |
| Smart Bot `Token` and `EncodingAESKey` | Receive user mentions or direct-chat questions, return AI SRE streaming replies, and decrypt images and files in bot messages |
| Trusted domain and frontend trusted domain | Support WeCom passwordless login, account linking, and opening Flashduty pages from notification cards |
| Trusted IP | Allow Flashduty services to call WeCom APIs |
| Phone number or email to `userid` lookup | Used for one-click user linking and inviting members when creating or updating War Rooms |
| `openuserid` to `userid` conversion | Used to identify senders from WeCom Smart Bot messages |
| App chat `appchat` create, update, get, and send APIs | Create and maintain War Room chats and send AI SRE analysis messages to War Rooms |
...
> Before configuring War Room, confirm that the custom app satisfies the [AI SRE required configuration and authorization](#aisre-permissions) listed at the beginning of this page.
