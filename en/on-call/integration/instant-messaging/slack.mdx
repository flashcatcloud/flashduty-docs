...
When you enable War Room or use AI SRE, the Slack app needs the following permissions. Flashduty checks part of the permission set when you enable War Room. If the page shows missing permissions, click **Re-authorize**.
...
| `channels:manage` | Create and manage War Room channels |
| `groups:write` or equivalent private-channel management permission | Create, invite members to, or archive private War Room channels |
| `channels:read` / `groups:read` | Read channel metadata and War Room details |
...
<Note>
If you use the official Flashduty Slack app, you only need to complete authorization or re-authorization from the Flashduty page. Review the permission list item by item only when you use a self-managed Slack app or need an internal app permission review.
</Note>
