...
| `im:message` | Get and send direct and group messages |
...
| `im:resource` or `im:resource:upload` | Upload and download images, files, and other message resources |
...
<Note>
If the Feishu/Lark permission page asks you to configure contact field permissions, grant read access to basic user information so Flashduty can display sender names and match users.
</Note>
