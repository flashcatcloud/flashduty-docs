...
<Note>
The number of mapping tables and mapping table rows is governed by account quota: the default quota is 20 mapping tables with 10,000 rows per table; some accounts have a higher quota (50 mapping tables with 50,000 rows per table) or no limit. Refer to the quota displayed on the page. Mapping services (APIs) remain capped at 50 per account. Creation or upload fails with a limit-exceeded error once the quota is reached; the mapping table list and the data details page show current usage (e.g. `12 / 20`, or "Unlimited" when no limit applies).
</Note>
