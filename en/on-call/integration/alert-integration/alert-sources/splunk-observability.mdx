...
3. Click **New Integration**, then click **Next** on the **Summary** page
4. On the **Configure connection** page, fill in:
   ...
   - **Method**: select `POST`
   - **Shared secret**: Flashduty authenticates the request by the `integration_key` in the push URL and does not verify the `X-SFX-Signature` header, so enter whatever the form requires
   - **Headers**: leave empty
...
- **Labels**:
  ...
  - every dimension in `dimensions`. Characters such as `.` and `-` in a key become `_`, so `k8s.pod.name` becomes `k8s_pod_name`. Keys that start with a digit are dropped
