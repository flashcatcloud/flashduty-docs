...
2. Select **Configuration** → **Integrations** → **Private integration**, then click **Add an integration** to enter the add integration page.
...
| Password | No | Connection password, stored encrypted and never shown in plaintext after saving. Provide it only when the selected database account requires password authentication. |
...
A "Dedicated Integration" (created under **Configuration → Integrations → Private integration** in a specific channel) is automatically bound to that channel and does not require a separate default route.
