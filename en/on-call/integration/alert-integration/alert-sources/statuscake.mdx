...
description: "Send StatusCake uptime test down and up notifications to Flashduty On-call through a contact group webhook."
keywords: ["alert integration", "StatusCake", "Uptime", "Contact Group", "webhook", "TestID"]
---

Use a StatusCake contact group webhook to send uptime test down (`Down`) and recovery (`Up`) notifications to Flashduty On-call. Each StatusCake test maps to one Flashduty alert: the alert triggers when the test goes down, repeated down notifications merge into the same alert, and the alert recovers when the test is up again.
...
<Steps>
<Step title="Create a contact group">

1. Sign in to StatusCake, go to **Alerting → Contact Groups**, and click **New Contact Group**
...
Edit each uptime test you want to connect, select the `Flashduty` contact group under **Contact Groups**, and save. One contact group can serve many tests.
...
No. When Repeat Alerts is enabled on the contact group, every notification sent while the test stays down carries the same `TestID` and merges into the same alert.

</Accordion>

<Accordion title="Can SSL, PageSpeed, or domain checks be connected?">

This integration is built for the `Down` / `Up` notifications of uptime tests, including Heartbeat tests. Other check types that also post through a contact group are accepted only when the request carries a `TestID` and `Status` is `Down` or `Up`; otherwise Flashduty returns a parameter error.

</Accordion>

<Accordion title="The contact group test succeeds, but real alerts do not arrive">

The contact group test only proves that the URL is reachable. Check that the test is attached to the contact group, that the test is not paused, and that the outage lasts long enough to meet the test's confirmation setting.
...
- **The alert does not recover**: Make sure the test is still attached to the same contact group when it recovers; the up notification must carry the same `TestID` as the down notification
