...
6. Leave **Webhook Headers Template** and **Webhook Body Template** empty. Flashduty parses the default Atlas request body, and a custom template drops the fields it needs
7. Click **Save**
...
The Atlas webhook body does not include the severity configured in the alert setting, so the severity is fixed:

| Alert type | Flashduty severity |
| :--- | :--- |
| Alerts that recover (`alert.open` and so on) | Warning |
| Informational alerts (`alert.inform`, such as Primary Elected) | Info |

A recovery event keeps the original severity. To set severity by alert type, use an [alert processing pipeline](/en/on-call/integration/alert-integration/alert-pipelines) on the `event_type` or `metric_name` label.
...
- **Every alert is Warning**: the Atlas webhook does not carry severity. See the severity section above
