...
Flashduty stores the Monitor ID, Monitor type, detection method, `TriggerType`, trigger time range, condition and value, `AlertGroup`, query, and Sumo Logic links. `ResultsJson` is not sent by default, avoiding large and high-cardinality query results in labels.
