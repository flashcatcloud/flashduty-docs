...
<!-- REVIEW: Auto-generated stub from coverage-audit (f002). Integration-level silence rules exist in code but lack step-by-step documentation. -->
## Alert Silence

Pipeline supports configuring silence rules at the integration layer, with the same functionality as channel-level silence rules, but taking effect earlier. Suitable for scenarios requiring unified suppression of specific alerts at the data source.

<!-- REVIEW: Feature overview — Add explanation of differences between integration-level and channel-level silence, and typical use cases -->

### Integration Layer vs Channel Layer

| Comparison | Integration Layer (Pipeline) Silence | Channel Layer Silence |
| :--- | :--- | :--- |
| Effective Layer | Integration layer | Channel layer |
| Use Case | **Global** silence logic, e.g., suppressing all alerts during planned maintenance | Silence rules for specific channels |

<!-- REVIEW: Verify the comparison above is accurate and add more differentiators -->

### Configuration

<!-- REVIEW: The following is a configuration step skeleton — verify and add screenshots and field descriptions -->

<Steps>
<Step title="Go to integration alert processing tab">
Go to **Integration Center** => Select created integration => **Alert Processing** tab
</Step>
<Step title="Add silence rule">
Click add rule, select "Alert Silence" action type
</Step>
<Step title="Configure silence conditions and time">
Set matching conditions (severity, title, labels, etc.) and silence time window
</Step>
</Steps>

### Usage

<!-- REVIEW: Add operation steps including prerequisites and notes -->
<!-- END REVIEW -->
