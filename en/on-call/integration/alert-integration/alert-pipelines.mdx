...
<Note>
**An empty rule list disables alert processing**: Processing rules can be saved as an empty list. After you delete all processing rules and save, the integration no longer performs any alert processing, matching the integration's initial state. To re-enable processing, click **Add processing rule** on the edit page; after you delete the last rule, the edit page shows the **No pipeline rules** empty state with an add button.
</Note>
