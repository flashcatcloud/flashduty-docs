...
<!-- REVIEW: Auto-generated stub from coverage-audit (f005). Storm alerts and war room events in incident timeline lack documentation. -->
### Alert Storm and War Room Events

The following special event types may appear in the incident timeline:

<!-- REVIEW: Feature overview — Add how alert storm warnings and war room events are displayed in the timeline -->

| Event Type | Description |
| :--- | :--- |
| **Alert Storm Warning** | When merged alert count reaches the storm threshold, a storm warning event is recorded in the timeline |
| **War Room Created** | When an incident is escalated and a war room is created, the war room lifecycle event is recorded in the timeline |
| **War Room Closed** | After the war room ends, a closure event with duration is recorded in the timeline |

<!-- REVIEW: Verify the event types above are complete, and add:
- Where the alert storm warning threshold is configured and its default value
- Complete lifecycle event list from war room creation to closure
- Example screenshots of these events in the timeline
-->
<!-- END REVIEW -->
