...
An action item can be converted into a **follow-up**, which then enters the post-mortem process. Follow-ups share the same work-item model as action items (same fields and statuses); the difference is that follow-ups belong to a post-mortem: when you convert or create a follow-up and the incident already has a post-mortem report, the system automatically links it to that report, and it is then maintained in the report's **To-dos** block.
...
When you decide during handling that an action item needs continued follow-through, you can convert it into a post-mortem follow-up:
...
- If the incident already has a post-mortem report at that point, the converted follow-up is **automatically linked** to that report
- If no post-mortem exists yet, the follow-up stays unlinked; once a post-mortem is created, all unlinked follow-ups under the incident can be linked (bound) to it in bulk

Once linked, follow-ups can be edited, completed, or deleted in the report's **To-dos** block, and related operations are recorded on the incident timeline (for example, "follow-up linked to post-mortem"). For writing and publishing post-mortems, see [Create and Edit Post-Mortem Reports](/en/on-call/post-mortem/create-edit-post-mortem).
...
| Post-Mortem follow-ups | Require edit permission on the corresponding post-mortem report |
