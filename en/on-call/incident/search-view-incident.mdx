...
| 5 | More Filters | Supports rich dimension filtering, including responders, channels, creator, acknowledger, closer, teams, snooze status, ever converged due to flapping, rare incidents, labels, custom fields, and **Unfinished actions / follow-ups** (the only option is **Yes**, which returns only incidents that still have unfinished action items or follow-ups). The **Channels** and **Teams** filters are only visible under the global Incident Management entry, and Teams narrows incidents by the team that owns each channel |
...
- Set **Unfinished actions / follow-ups** to **Yes** to surface incidents that still have open work to wrap up
...
| **Post-Mortem** | A built-in post-mortem editor that lets you create, edit, AI-generate, and publish a post-mortem report without leaving the details page | Subject to subscription tier; always visible when enabled |
...
| `i_wi_bound` | Linked follow-up to the post-mortem |
| `i_wi_auto_converted` | Converted N unfinished actions to follow-ups: title, title… (a batch conversion on incident close; one record summarizes all items) |
| `i_wi_deleted` | Deleted action / Deleted follow-up |

### AI SRE Action Records

Actions that AI SRE performed on behalf of a member — for example when an [automation rule](/en/ai-sre/automations) finishes a run and writes its conclusion back to the incident — are recorded on the timeline under a **joint signature**: the byline reads `{{name}} and AI SRE`, and both names carry equal weight because AI SRE acts with that member's identity and permissions. Such records are flagged `via: ai_sre` on the backend.

- **Signature hover card**: hovering the **AI SRE** part of the byline opens a hover card that states "Acting as `{{name}}` with their permissions" and offers a **Continue conversation** button, which opens that session in the AI SRE panel
- **Always-visible entry**: every AI-performed action row also carries a persistent **Continue conversation** entry with a ↗ icon, so you can jump straight into the session without hovering; one incident can contain several AI sessions, so the entry sits on each row rather than being a single incident-level entry

Both entries require the AI SRE module to be enabled and the current member to hold the `AiSreChatVisit` permission. When the session is a personal session (visible only to its creator) or has expired, the always-visible entry is not rendered and the hover card shows "This session is visible only to its creator or has expired" instead; when the row has no associated session, the hover card shows "This action is not linked to a session".
...
If the IM integration used by the war room is later disabled or deleted, **Invite Followers to War Room** and **Close War Room** both disappear (the original chat can no longer be operated on). The **View War Room** dialog then reads "The original integration is disabled or deleted. An administrator can unlink this war room without dissolving the group chat. Before creating another room, make sure the original group has been handled." and, for account owners/admins with incident update permission, offers a **Confirm unlink** button. That action only removes the link between the incident and the original group — it does **not** dissolve the chat — and the timeline records it as "Requested war room unlink".

In the Flashduty App (mobile), war room behavior matches the console: the incident overview page shows a **War room** entry card (IM integrations with deep-link support open the chat directly; otherwise a dialog shows the group name to search for); **Create war room** lives in **More Actions** at the bottom, hidden when the incident already has a war room or is closed, and disabled with a toast when the incident has no responders; **Add stakeholders to war room** and **Dissolve war room** remain available after the incident is closed, but they do depend on the war room's IM integration being available — once it is disabled or deleted, both entries disappear from **More Actions**, while the overview entry card stays and shows "The integration is disabled or deleted. Ask an administrator to unlink the war room from the incident details on desktop. The original group chat will not be dissolved." on tap.
