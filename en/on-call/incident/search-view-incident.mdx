...
### Batch Send to AI SRE

After selecting multiple incidents in the incident list, you can send the selection to the AI SRE panel as an "Incident list (N)" context for batch analysis:

- The context includes each incident's ID, number, severity, progress, and title. When the selection is large enough to exceed the context size budget, some incident titles are omitted and the AI fetches details by incident ID on demand
- When all selected incidents belong to the same team, the AI SRE session opens in that team's scope; selections spanning multiple teams or without team ownership fall back to personal scope
...
| **Timeline** | The complete lifecycle record of the incident — triggering, assignment (reassigning responders is recorded here as "Reassigned responders"), notifications, acknowledgments, snoozes, closure, comments, and action-item and follow-up operations such as creation, updates, and completion — with a persistent Markdown comment box at the top | Always visible |
...
The timeline also includes the following display details:

- **Cross-day date separators**: A date separator row (showing the date and weekday) is inserted between records from different days; each record's timestamp on the left shows only HH:mm
- **Custom field change comparison**: Each custom field update is shown as a "Before → After" comparison, making it easy to trace field changes
- **Convergence record entry**: Convergence (flapping) records provide a **View convergence settings** link that jumps to the alert convergence configuration of the owning channel
- **Overview preview**: The "Recent Timeline" section on the incident overview page shows only the most recent records; click **View all** to open the full Timeline tab. On the Timeline tab, the comment input box is always visible so you can comment at any time
- **External reporting**: For incidents reported through external channels, the trigger record on the timeline shows the reporter's email address instead of an internal member's avatar and name
