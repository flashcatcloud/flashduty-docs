...
| 5 | More Filters | Supports rich dimension filtering, including responders, channels, creator, acknowledger, closer, teams, snooze status, ever converged due to flapping, rare incidents, unfinished action items / follow-ups, labels, and custom fields. The **Channels** and **Teams** filters are only visible under the global Incident Management entry, and Teams narrows incidents by the team that owns each channel |
...
- Use **Unfinished action items / follow-ups** to find incidents with work still outstanding: this filter offers only one selectable value, **Has**, and leaving it unselected means **All** (the condition is not applied); with **Has** selected, the list keeps only incidents that still own at least one unfinished action item or follow-up, that is, a work item whose status is neither **Completed** nor **Won't do**. For how action items and follow-ups are created, transitioned, and handled at closure, see [Action Items and Follow-Ups](/en/on-call/incident/work-items)
...
| `i_wi_auto_converted` | On incident closure, converted unfinished action items to follow-ups in bulk, listing each item's title on the record (titles separated by "、") |
| `i_wi_bound` | Linked follow-up to post-incident review |
| `i_wi_deleted` | Deleted action / Deleted follow-up |

`i_wi_auto_converted` is a **batch** record, at most one per incident closure: if unfinished action items remain at closure, the system converts them all into follow-ups after the close completes (no record is written when the converted count is 0), so it records the converted item count and title list rather than a change to one particular action item; a single conversion triggered manually by a user is still recorded as `i_wi_converted`. For how action items and follow-ups change after closure, see [Action Items and Follow-Ups - Closed Incidents](/en/on-call/incident/work-items#closed-incidents).
