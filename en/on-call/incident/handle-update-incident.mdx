...
## Post Comments and Comment Types

When posting a comment on an incident's timeline, you can associate a **comment type** (name + color label) with it to distinguish the nature of the comment (e.g., "Root Cause Analysis", "Progress Update"); timeline entries display the selected type as a colored label. Comment types are **account-level** configuration and apply to all incidents under the current account. See [Search and View Incidents](/en/on-call/incident/search-view-incident) for the full description and management operations.

In the Flashduty App (mobile), the comment type selector behaves as follows:

- When writing a comment, types appear as pills above the comment box; tap to select and tap again to deselect. Up to 3 types are shown inline, the rest collapse into a **More** panel, and the selected type always stays visible in the inline row
- Members with the incident comment type management permission (`incidentCommentType:manage`) can **create** a new type right in the **More** panel: name up to **20** characters, color picked from 30 presets (6 common presets + 24 palette colors); the newly created type is selected automatically
- Management actions such as renaming, recoloring, deleting, and reordering must be done in the console. If the account has no comment types yet and you do not have the management permission, the type selector is not shown in the App
