...
<!-- REVIEW: Auto-generated stub from coverage-audit (f003). ML-based intelligent grouping exists but lacks detailed documentation. -->
## Intelligent Grouping Details

Intelligent grouping uses a machine learning model to analyze semantic similarity between alerts, automatically merging related alerts into the same incident without manual rule configuration.

<!-- REVIEW: Feature overview — Add core differences between intelligent grouping and rule-based grouping to help users choose the right mode -->

### Intelligent Grouping vs Rule-based Grouping

| Comparison | Intelligent Grouping | Rule-based Grouping |
| :--- | :--- | :--- |
| Matching Method | ML-based semantic similarity scoring | Exact matching by specified dimensions |
| Configuration Complexity | Low, only need to select calculation fields | High, need to define grouping dimensions and conditions |
| Use Case | Variable alert content, hard to predefine rules | Fixed alert structure, need precise control |

<!-- REVIEW: Verify the comparison above is complete, add more dimensions (e.g., accuracy, performance impact) -->

### Similarity Scoring Mechanism

<!-- REVIEW: Add the following content:
- How similarity scores are calculated and threshold descriptions
- What factors affect scoring (title, description, label weights)
- How to tune for better grouping accuracy
-->

### Tuning Recommendations

<!-- REVIEW: Add intelligent grouping tuning methods, including:
- How to choose the best calculation fields
- Troubleshooting when grouping results are unsatisfactory
- Criteria for switching from intelligent grouping to rule-based grouping
-->
<!-- END REVIEW -->
