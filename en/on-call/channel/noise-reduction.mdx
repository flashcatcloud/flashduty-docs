...
**Auto-delete on expiration** (`is_auto_delete`): one-time silence rules can opt into this switch. When enabled, the rule is automatically removed by the system 24 hours after its end time (the cleanup window starts from the later of the end time and the rule's last update time, so editing a rule after it expires recalculates the window instead of deleting it immediately), matching the cleanup behavior of [Quick Silence](#quick-silence). When disabled (the default), the rule stays in the list after expiration but is no longer active — you can keep it for reuse or delete it manually.
...
Multiple silence rules are matched in creation order, with no priority setting. To change the matching order, delete the rules and recreate them in the desired order.
...
Multiple inhibit rules are matched in creation order, with no priority setting. To change the matching order, delete the rules and recreate them in the desired order.
