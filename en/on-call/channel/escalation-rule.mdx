...
### Copy an Escalation Rule

Each escalation rule card shows a **Copy** icon on hover. Clicking it opens the copy dialog (the same component used for copying silence rules), which contains:

| Field | Description |
| :--- | :--- |
| **Rule name** | Required; pre-filled with "Copy " plus the original name, **39 characters max** (the backend limit is 40) — overlong input is truncated in the field |
| **Channel** | Choose **This channel** or **Another channel**. Copying to another channel requires picking the target from a dropdown that excludes the source rule's channel; non-admin users only see channels they manage |
| **Description** | Optional; pre-filled with the original description, **500 characters max** — overlong input is truncated in the field |

- The copy reuses the source rule's full configuration (trigger conditions, notification targets, notification methods, delay window, notification template, and escalation levels), but the **priority (list matching order) is not copied** — reposition the new rule in the list after copying
- Copying to the current channel makes the new rule appear in the list immediately; copying to another channel shows the **Copy succeeded** result dialog with the target channel, rule name, description, and creator. Click **Go to view** to open the target channel and locate the new rule
...
- **On**: Only incidents matching the filter conditions hit this rule; unmatched incidents continue matching subsequent rules. You can add multiple condition groups — conditions within a group are ANDed, groups are ORed; each condition supports **Match** / **Not Match** operators, with available keys:
  - **Integration Source (First Alert)**: Match by the integration that produced the incident's first alert; options are the integrations available to the current channel (both shared and dedicated)
  - **Severity**: Multi-select Critical / Warning / Info
  - **Title**, **Description**: Support exact, wildcard, regex, CIDR, and numeric comparison matching modes
  - **Labels**: Match by alert label key-value pairs (e.g., `service=payment`); keys can be loaded dynamically from recent alert data or entered manually
...
<Tabs>
  <Tab title="Direct Message (Reach Individuals)">
    Point-to-point messaging to specific assignees, supports phone calls, SMS, email, app push, IM direct messages (Feishu/Lark, Dingtalk, WeCom, Slack, Microsoft Teams).
  ...
  <Tab title="Group Chat (Reach Groups)">
    ...
    - **IM App Groups**: Supports Feishu/Lark, Dingtalk, WeCom, Slack, and Microsoft Teams groups (WeCom is only available for existing configurations and cannot be selected when creating new rules), requires completing [IM Integration](/en/on-call/integration/instant-messaging/lark) first. Disabled IM apps stay in the app dropdown, labelled "(Disabled)", and cannot be selected as a notification target — re-enable the app in Integrations first. The group-chat picker automatically excludes platform-created chats such as war rooms (placeholder: "Select … group chat (war rooms excluded)"); stale targets render as **Chat unavailable** and can be removed
