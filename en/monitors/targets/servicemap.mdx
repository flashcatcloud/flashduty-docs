...
- **Double-click any node** to enter focus mode: the canvas keeps only the local dependency graph expanded upstream and downstream from that node. Entering focus mode resets the upstream hop count to 0 and the downstream hop count to 1. Upstream and downstream hop counts can then be adjusted independently from 0 to 3. Upstream edges represent callers that depend on you; downstream edges represent what you depend on.
...
| | Type | The runtime carrier type: `bare_process` or `container` |
...
`Type` and `Entity Identity` answer different questions:

| Entity type | Meaning |
|---|---|
| `bare_process` | A workload made of processes running directly on the host. Multiple process instances may be folded into one entity. |
| `container` | A workload running in a Docker, containerd, or other CRI container. Kubernetes placement does not change its entity type. |

`identity.kind` explains why instances were folded into the same entity. Process identities are `process_rule`, `systemd_unit`, `cgroup`, `executable`, or the lifecycle-scoped `process_instance` fallback. Container identities are `container_rule`, `orchestrator_workload`, `compose_service`, `systemd_unit`, `container_name_image`, or the lifecycle-scoped `container_instance` fallback. `identity.confidence` is `high`, `medium`, or `low`; it describes aggregation confidence, not network endpoint resolution confidence.

Pods, Kubernetes workloads, and Services are placement and enrichment context for an entity rather than additional ServiceMap entity types. Diamond endpoint nodes shown on the canvas are also temporary presentation nodes, not entities.
