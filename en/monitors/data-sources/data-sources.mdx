...
**Datasource tools** in the table above means the [data source tools invoke](/en/api-reference/monitors/data-sources/monit-datasource-tools-invoke) API (`POST /monit/datasource/tools/invoke`): deterministic read-only calls named `<type>.<tool>`, covering both diagnostic tools such as `mysql.overview` and `redis_node.slowlog` and query tools such as `prometheus.query` and `sls.query` (query tools run on the same execution path as the Query Workbench). That API is gated by `enabled` alone: calls are rejected when `enabled=false`, while `alerting_enabled=false` does not block them.
...
| **Timeout (milliseconds)** | Connection timeout | `3000` |
...
| **Timeout (milliseconds)** | Connection timeout | `5000` |
| **TLS** | Whether TLS is enabled | Disabled |
| **CA certificate** | PEM CA certificates or an `${env:NAME}` reference | - |
