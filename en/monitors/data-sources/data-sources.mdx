...
- **Name**: The identifier for the data source. A row with team authorization configured shows team tags after the name: one green tag for the managing team and one gray tag per query-allowed team, each labelling its role on hover ("Managing team" / "Query-allowed team"); when an authorized team has been deleted, its tag reads "Deleted team". Rows with no authorization configured carry no tags
...
The data source page also supports a `?create=<type_ident>` deep link (used by the AI SRE "add in Monitors" entry to jump here with a type): on landing it opens the add drawer with that type preselected. Three checks must all pass — the type is in the console's supported type list, the current user holds the create data source permission (`MonitDatasourceCreate`), and the account plan meets the type's version requirement (Aliyun SLS and Tencent CLS require the Standard plan or above). If any check fails the parameter is treated as absent; only a version rejection adds a message — "xxx data source requires the Standard plan or above; your current plan cannot add it." (with xxx being the type name). The parameter is removed from the address bar once handled, so a refresh does not reopen the drawer.
...
- **Query and reference**: instant queries, data exploration, dashboard panel queries, datasource tool calls, referencing the data source in an alert rule, and the data source list in the rule runtime details drawer (see [Rule runtime details](/en/monitors/quickstart/quickstart#rule-runtime-details)) all require `readonly`; without it the request is refused explicitly, without exposing data source details. The rule runtime details list filters silently: a data source without `readonly` is dropped entirely, so a restricted viewer may see fewer data sources than the rule actually binds.
...
The authorization fields travel with the data source create and update requests (`POST /monit/datasource/create`, `POST /monit/datasource/update`): you can set them while creating a data source, and changing them on an existing data source is a management operation that requires `manage` on that data source. Omitting an authorization field in an update keeps the current value; passing an explicit `null` clears it.

In the console create/edit drawer the two fields are the **Managing team** (`manage_team_id`) and **Query-allowed teams** (`readonly_team_ids`) team selectors, settable both on create and on edit. The form runs the same validation as the backend before submitting: query-allowed teams without a managing team are blocked immediately ("A managing team is required when query-allowed teams are set", matching the backend's HTTP 400). Managing team candidates depend on your role: the owner account and the Admin role can pick any team, while other users can only pick teams they belong to — selecting a team you are not in would immediately lose you the manage right on that data source.
...
| **Managing team** | Optional, maps to `manage_team_id`. Once set, only the creator, the owner account, the Admin role and members of that team can edit, delete, enable or disable this data source; leave empty to let everyone manage it. The owner account and the Admin role can pick any team; other users can only pick their own teams |
| **Query-allowed teams** | Optional, maps to `readonly_team_ids`. Restricts which teams can query and reference this data source; leave empty to let everyone query it. Setting query-allowed teams requires selecting a managing team, otherwise the save is blocked |
...
- **View**: as long as you hold the data source edit permission (`MonitDatasourceUpdate`), the row button opens the drawer even when you lack `manage` on that data source (`my_perm=readonly`) — it opens as a read-only view: the drawer title reads "View data source" and the row button tooltip reads "View", the whole form is disabled, the data source type cannot be switched, and no connection test is available. The read-only view does not call the data source detail endpoint (which is behind the `manage` check): basic fields come from the list row and the connection configuration renders in the original form layout with all 19 string connection and credential fields showing `******` (switches, numbers and enums stay empty). The save button is disabled, and the drawer's submit path has its own separate hard guard.
- **Edit**: with manage permission the drawer loads the data source detail (which returns connection credentials); modify and save.
- **Delete**: Click the **Delete** button in the data source list and confirm to delete the data source.

Edit and delete both go through the data source `manage` check first: you must be the creator of the data source, the owner account or an Admin, or a member of its managing team; otherwise the request is rejected (`only the managing team can manage this datasource`). See the "Data source permissions (team authorization)" section for the decision rules.
