...
Tencent CLS rules support **query time offset**: set an offset (in seconds) in the rule's execution configuration to shift the entire query window backwards, which helps when log ingestion or indexing introduces delays. For example, with an offset of 60, each evaluation queries the window that ended 60 seconds ago, ensuring data has finished writing before it participates in detection.
