...
Recovery query allows you to define a separate PromQL for determining whether an alert should recover, rather than relying on changes in the original alert query results.

Recovery query statements support embedded variables in `${label_name}` format. The engine automatically replaces variables with corresponding label values from the current alert event during execution. This enables recovery evaluation to be precise to specific alert objects.

```promql
up{instance="${instance}", job="${job}"} == 1
```

In the example above, if the alert event labels are `instance="host-1"` and `job="node"`, the engine actually executes `up{instance="host-1", job="node"} == 1`. Recovery is determined when data is found.

<Note>
Recovery query is applicable to threshold evaluation mode and data exists mode. In no data mode, recovery logic is based on data reappearing and does not require a separate recovery query.
</Note>

### Multi-Query Correlation

In threshold evaluation mode, a single alert rule can configure multiple query statements (A, B, C...). The engine automatically correlates results from different queries based on label sets.

The core principle of correlation is **label set alignment**: only when multiple queries return data with exactly the same label sets can they be correlated to the same context for threshold calculation (e.g., `$A > 90 and $B < 50`).

If different queries return inconsistent label sets, use PromQL aggregation operations to explicitly control returned labels:

```promql
# Query A: ensure only instance and job labels are returned
sum by (instance, job) (rate(cpu_usage_total[5m]))

# Query B: ensure the same label set is returned
avg by (instance, job) (mem_usage_percent)
```

<Warning>
If Query A returns a label set containing `disk="/"` while Query B does not have that label, they cannot be correlated. Ensure all queries participating in threshold calculation return consistent label sets.
</Warning>
