...
2. **Field Mapping**:
  ...
  - If Label fields is empty, every returned field except the value fields becomes a label. See [Query Result Field Mapping](/en/monitors/alert-rules/query-result-fields) for the complete behavior.
...
2. **Field Mapping**:
  ...
  - To identify alerts with only stable fields and keep other columns as troubleshooting context, explicitly select Label fields. See [Query Result Field Mapping](/en/monitors/alert-rules/query-result-fields).
...
|------|------|
| **End automatically when data reappears** | Default. Once that `host_name` reappears in query results, the alert ends automatically |
