...
A field **name** must begin with an English letter and may contain only English letters, digits, and underscores (for example, `runbook_url`). The console validates this rule, so a name that does not match it cannot be saved. Names beginning with `$` are reserved for fields produced by queries and cannot be used as custom field names.

Examples:

| Field Name | Field Value | Rendered Result (assuming `service=payment-api`, `$severity=Critical`) |
| --- | --- | --- |
| `runbook` | `https://a.com/runbook/${service}` | `https://a.com/runbook/payment-api` |
| `title` | `[{{ $severity }}] {{ index $labels "service" }} alert` | `[Critical] payment-api alert` |
...
| `.TitleRule` | `string` | The title rule configured on the edge, from monit-edge's `alerter.titleRule` startup flag (deployment-level, not a per-rule setting). When the value contains `{{`, it is rendered as a **Go template** with exactly the variables documented on this page; otherwise labels are substituted using the legacy placeholder forms — `${key}`, `[key]`, and `$key` all work for the same label key (for example, `$check / $data_source_name`). When it is empty, the title falls back to `check / data_source_name`, with ` - resource` appended when the event carries a `resource` label. |
| `.RenderedTitle` | `string` | The rendered title. When the rule defines a custom field named `title`, that field's rendered value wins; otherwise, when `.TitleRule` is a Go template, its rendered value is used. If rendering fails or the result still contains `{{`, this field is empty and the title falls back to the default form — an unrendered template is never used as the title. |
