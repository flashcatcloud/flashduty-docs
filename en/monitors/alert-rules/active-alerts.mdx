...
In the alert rules list, the **Status** column shows the rule's runtime status (click the status label to open the runtime details drawer), and the **Active alerts** column shows how many active alerts the rule currently has (bell icon + number); clicking that number switches you to the "Active Alerts" tab filtered by that rule (URL parameter `active_alert_rule_id`).
...
Every row in the list provides a **View details** entry (the eye icon at the end of the row) that opens the active alert detail drawer, organized into the following sections:
