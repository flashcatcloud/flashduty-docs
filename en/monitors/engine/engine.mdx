...
<Note>
The Linux install and upgrade commands fetch the install script from `https://static.flashcat.cloud/monitedge/install.sh`. The script is hosted on a public CDN (Aliyun OSS), requires no `Referer` header, and is refreshed with every stable release; the script URL in the generated command is fixed, and there is no parameter to point it at a different script source.

Offline or intranet deployments must mirror `install.sh` together with the `releases/download/<vX.Y.Z>/` artifacts and `checksums.txt` it references, and replace the script URL in the command. The artifact download URL inside the script is not parameterized, so a mirror path that differs from the public one requires adapting the script itself.
</Note>
...
| `alerter.updateMirror` | `https://static.flashcat.cloud/monitedge` | Mirror root URL for automatic upgrade artifacts. The engine downloads the artifact and checksum file from `<mirror root>/releases/download/<vX.Y.Z>/`; offline or private-network environments must point this at an internal mirror |
