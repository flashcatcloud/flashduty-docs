...
Every query statement input box has a ✨ button at its top-right (labelled **Write with AI**; it fades in when you hover or focus the box, and only appears once a **data source type** is selected). Clicking it opens the **Query assistant** panel: describe what you want to query in natural language and the assistant writes the statement and applies it. To generate the statement, the assistant reads the current rule's context: rule name, evaluation mode (threshold / data exists / no data), configured thresholds, the other query statements (trigger, related, recovery), attached labels, detection frequency, and the rule's data source selection. Applying only writes the query statement and the data source private parameters — it **never switches the data source**, because the data source belongs to the whole rule and changing it resets all queries. The trigger query, the recovery query (one input in threshold mode and one in data-exists mode), related queries, and the statement input inside the **query preview** dialog all carry this entry.

<Note>
The query assistant is a console AI assistant capability: it requires AI SRE to be enabled for the account and the current user to have AI SRE access permission.
</Note>
