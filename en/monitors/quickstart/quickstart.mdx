...
After configuration is complete, the rule appears in the alert rules list of its folder. The list shows the following columns by default:

| Column | Description |
|--------|-------------|
| **Rule** | Rule name. The icon before the name shows the **data source type** — hover it to see the type name. The name is shown on at most two lines and truncated beyond that; hover to see the full name. With edit permission, click the name to open the edit page |
| **Status** | The rule's runtime status. Click the status label to open the runtime details drawer; see [Rule runtime details](#rule-runtime-details) for what each state means |
| **Active alerts** | How many active alerts the rule currently has (bell icon + number). Click it to open this rule's active alerts |
| **Enable** | The rule's enable switch |
| **Debug log** | The rule's debug log switch (requires monitedge v0.21.0 or later) |
| **Updated at** | When the rule was last updated, in `YYYY-MM-DD HH:mm:ss` format |
| **Actions** | Row action menu (`⋮`): Runtime details, Clone, Move, Version history, Delete |

The leftmost column is the selection column, used to select rules for batch operations. The **Enable** and **Debug log** columns are switches; with rule edit permission you can toggle them directly in the list.
...
Clicking the number in the **Active alerts** column switches the page to the **Active Alerts** tab filtered by that rule (URL parameter `active_alert_rule_id`), letting you directly view the active alerts currently produced by the rule. Historical alert events can be viewed in On-call. See [Active alerts](/en/monitors/alert-rules/active-alerts) for details.

### Custom display columns

Click the **column settings** button (gear icon) on the right of the toolbar to open the column picker:

- **Show/hide columns**: click the checkbox before a column name to toggle its visibility
- **Reorder columns**: drag a column name to change the order of the columns
- **Pinned columns**: the **Rule** and **Actions** columns are always shown and cannot be hidden

Column visibility and order are stored per user in the browser, and still apply after you refresh the page or come back later.

### Pagination

The list shows 10 rows per page by default; use the page-size selector to switch to 10 / 20 / 50 / 100 rows per page.
...
The alert rules list supports multi-select for batch operations, improving rule management efficiency. Use the checkboxes on the left of the list to select rules; the checkbox in the header only selects the rules on the **current page**, not the whole folder. You can keep selecting rules on other pages after paging, and selections are cleared when you switch folders.
...
After selecting multiple rules, click **Edit** in the batch action bar (the panel opens with the title **Batch update**) to uniformly modify the following 10 fields:
