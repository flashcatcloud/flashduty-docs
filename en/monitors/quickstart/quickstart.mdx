...
After configuration is complete, you can review each rule's health in the alert rules list. The list's **Status** column shows the rule's runtime state badge (rather than whether any alert is firing); clicking the status badge opens the [Rule runtime details](#rule-runtime-details) drawer, where you can inspect execution results and troubleshoot.
...
The **Status** column badges sort from least to most severe, with these values:

| Status badge | Description |
|---------|------|
| `Running normally` (normal) | The rule's most recent execution succeeded and it is healthy |
| `Waiting for runtime status` (waiting) | The rule has not executed yet or the engine has not reported status; this is neutral, not an error |
| `Config pending` (config_pending) | The rule configuration has been saved centrally, but the Edge responsible for it has not yet reported the new version |
| `No data source matched` (no_datasource) | The rule matches no data source; check the rule's data source binding |
| `Status stale` (stale) | The engine has not reported this rule's execution status for a long time; check the Edge cluster's health |
| `Execution abnormal` (abnormal) | The rule's most recent execution failed (e.g., a query error); click the badge to see the specific error |
| `Edge offline` (offline) | The Edge engine instance responsible for this rule is offline; check the alert engine cluster |
| `Rule disabled` (disabled) | The rule has been disabled and is not scheduled |

When a rule is bound to multiple data sources, the **Status** column aggregates and shows the most severe state. If the field is missing (for example, an older Edge version does not report it), it is displayed as `Waiting for runtime status`.

To see whether a rule currently has firing alerts, look at the list's **Active Alerts** column: it shows the rule's current active alert count, and clicking the number switches the page to the **Active Alerts** tab filtered by that rule (URL parameter `active_alert_rule_id`). Historical alert events can be viewed in On-call. See [Active alerts](/en/monitors/alert-rules/active-alerts) for details.

## Rule runtime details

Every row in the alert rules list provides runtime introspection. Either clicking the runtime state badge in the list's **Status** column or clicking **Runtime Details** in the row action menu (`⋮` at the end of the row) opens the rule runtime details drawer, which helps you judge the rule's execution health and troubleshoot quickly.
...
- **State badge**: `Running normally` (normal), `Execution abnormal` (abnormal), `Waiting for first run` (waiting), `Status stale` (stale). A `Config pending` badge is also shown when the central config is saved but the Edge has not reported the new version yet; a `Rule disabled` badge appears when the rule is disabled. The per-data-source state is a subset of the list **Status** column values (normal / waiting / config_pending / no_datasource / stale / abnormal / offline / disabled — see [View Results](#view-results)), with the same wording and colors in both places
