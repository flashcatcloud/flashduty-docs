...
The samples view and table view paginate in the browser instead of rendering all rows at once: the samples view shows 20 rows per page; the table view defaults to 100 rows per page with options of 100 / 200 / 500. The paginator is hidden when the results fit in a single page.
...
When a query fails, the result area shows a hint matched to the failure reason (such as query timeout, result row limit exceeded, or Edge version too low). Among these, a data-source-level authorization denial (HTTP 403, error code `AccessDenied`) shows a dedicated hint — "You don't have access to this data source. Please contact the team responsible for the data source or an administrator." — instead of the generic "Query failed, please try again later." For team-based data source authorization rules, see [Data source permissions (team authorization)](/en/monitors/data-sources/data-sources#data-source-permissions-team-authorization).
...
The query editor input has a rainbow **Write with AI** button in its top-right corner that fades in on hover or focus (select a data source first). Clicking it opens the **Query Assistant** panel, which can see the current data source, query type, private parameters, and time window, plus how the query will actually run (instant / range / range+instant / window; Prometheus's default **Graph + Raw** mode runs one range query and one instant query, which is `range+instant`), and generates or rewrites the query statement accordingly.

- The result is applied automatically and as a whole: statement, private parameters, and query type are written back together. If the data source chosen by the assistant differs from the current one, the workbench switches to it first, clears the current results, and remembers it as the default data source for your next visit.
- When the result is delivered, the query runs once automatically and its results appear below. The **Apply** button on the result card re-applies that version without running it; press Enter or click **Run query** to execute it manually afterwards.
- Applying overwrites the statement currently in the editor and merges the assistant’s private parameters into the existing ones (parameters it does not return keep their current values), so review the result before continuing or sharing.
