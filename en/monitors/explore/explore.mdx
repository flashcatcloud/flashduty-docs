...
A ✨ button sits at the top-right inside the query statement box (labelled **Write with AI**; it fades in when you hover or focus the box). Clicking it opens the **Query assistant** panel. Describe what you want to query in natural language and the assistant writes the statement for you.

When generating a statement, the assistant reads the following from the editor:

| Context | Description |
|---------|-------------|
| Data source | The selected data source's type, ID, and name, used to probe the real metric names, label names, and fields |
| Query type | The current query type plus every query type value the data source allows |
| Private parameters | The data source private parameters currently configured, such as SLS project/logstore or CLS log topic |
| Time window | The selected time range and the result pane's resolution, used to pick a suitable range function and step |

This information is read live on every message you send: switch data sources or change query options while the panel is open and the next statement is generated against the new values.

The panel's home screen offers a few suggested questions for the Query Workbench (for example, "Show the error count for the last 15 minutes, grouped by service"); click one to send it.

Generated results appear in the conversation as a card and are **applied to the editor automatically**. What gets applied is the whole query — data source, statement, private parameters, and query type land together, the same semantics as replaying query history; if the data source differs from the editor's current one, it switches data sources first. In the Query Workbench the automatic apply also runs the query once, so results appear directly in the result area below. The **Apply** button on the card writes that version of the statement back to the editor at any time (it is greyed out when the editor already matches the generated statement).

<Note>
The query assistant is a console AI assistant capability: it requires AI SRE to be enabled for the account and the current user to have AI SRE access permission. Without it, the panel cannot read the data source's real structure to generate statements and offers a "Book a demo / free trial" entry instead.
</Note>
