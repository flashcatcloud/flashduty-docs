...
Agreement version: `ai-sre-credit-package-v2`
...
Gifted credits, if granted, are unrelated to this purchase and are granted and calculated under separate rules Flashduty publishes or communicates separately. They are usable only within the billing period they were granted for, do not carry over between periods, and cannot be transferred or refunded. Any unused gifted credits are zeroed out when the AI SRE resource is deleted (de-provisioned), while purchased-package balances and validity are unaffected; closing AI SRE yourself in the console (**Payment** → **Plan** → the **AI SRE** tab → **Disable AI SRE**) is not that event, and it does not zero gift credits — purchased packages and gift credits are both retained and are usable again after you re-enable the service.

Enabling AI SRE does not authorize PAYG. PAYG is off by default and is used only after an account admin turns it on from the AI SRE billing page; once on, PAYG is settled from your account's general wallet at `1 credit = ¥1`. The billing period follows the license period of the account's On-call subscription (consecutive periods counted from the subscription, not calendar months), and the next period starts automatically when one ends.

An account admin can put PAYG into one of three states on the AI SRE billing page:

- **PAYG off** (default): PAYG is never used, and runs stop once gifted credits and packages are exhausted;
- **Monthly cap**: PAYG spends at most the number of credits you set in each billing period;
- **Unlimited**: PAYG continues with no ceiling once gifted credits and packages are exhausted.
...
PAYG is settled from the account's general wallet. If the wallet is exhausted or the account falls into arrears, any new AI SRE run that would have to be funded by PAYG is refused. Gifted credits and purchased packages are prepaid and are unaffected by arrears — while the account still has usable gifted or package credits, AI SRE runs can still be started, and a package's balance and validity are unchanged. Once the account is topped up, PAYG resumes immediately, with nothing to re-enable.

Arrears treat enabling and closing asymmetrically. **Closing AI SRE is unaffected by arrears**: closing costs nothing, so it checks neither the wallet balance nor a valid On-call subscription — an account in arrears or with a lapsed subscription can still close the service and can re-enable it at any time. **(Re-)enabling AI SRE, by contrast, requires a usable wallet balance**: enabling is refused while the wallet balance is negative, the console showing "Your wallet balance is negative, so AI SRE cannot be enabled yet. Please top up first." together with a top-up entry; you can enable AI SRE once you have topped up.
