...
- **Service providers**: cloud hosting, communications, analytics, and AI model providers that process data on our behalf under confidentiality obligations.
...
## 8. Information processing in AI-assisted features

Some features of the Service (such as the incident "AI Summary") use generative AI to help produce text content. For these features:

- **Information processed**: only when you actively trigger the feature, we process the incident metadata directly related to that generation (such as alert titles, timestamps, related service names, and handling notes). We do not use user profiling or perform personalized generation.
- **Redaction and review**: before submission to the model, the system extracts only allowlisted fields and automatically redacts sensitive information such as personal names, contact details, internal IP addresses, and credentials; both the input and the generated output go through content-safety review.
- **Third-party model service**: the redacted, necessary information is submitted to a third-party large-model service platform deployed in mainland China (Alibaba Cloud Model Studio) solely to complete that generation. We have a service agreement with the provider, and it processes the information only for that purpose.
- **No training on your data**: we do not use your personal information or business data to train or fine-tune AI models.
- **Content labeling**: AI-generated content carries a prominent notice (such as "This content is AI-generated and for reference only; please verify carefully") so you can identify and assess it.
- **Complaints and feedback**: if you have concerns about AI-generated content, you can report it via the in-product feedback entry or the methods listed in the "Contact us" section of this policy, and we will review and act promptly.
