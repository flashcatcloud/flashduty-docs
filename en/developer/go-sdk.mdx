...
| Option | Description |
|--------|------|
| `WithBaseURL(raw string)` | Override the API base URL (default `https://api.flashcat.cloud`). A path prefix is preserved, so `https://gateway.example.com/api` sends requests to `/api/<endpoint>` (the path is normalized to end with `/`, so endpoint paths are appended rather than replacing the last segment). Use it to point at your own gateway for private deployments; an invalid URL errors at the `NewClient` stage |
...
<Info>
**Private deployment**: point the client at your own Flashduty gateway address with `WithBaseURL` — everything else stays exactly the same. A gateway address with a path prefix (such as `https://gateway.example.com/api`) works too: the SDK keeps the prefix and normalizes the path to end with `/`, so requests go to `/api/<endpoint>`.
</Info>
