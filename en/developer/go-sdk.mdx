...
| Option | Description |
|--------|------|
| `WithBaseURL(raw string)` | Override the API base URL (default `https://api.flashcat.cloud`). Use it to point at your own gateway for private deployments; the address may carry a path prefix (with `https://gateway.example.com/api`, requests go to `/api/<endpoint>`, with or without a trailing `/`); an invalid URL errors at the `NewClient` stage |
...
| `client.Schedules` | Schedules; `ByPerson` looks up a member's current and next on-call intervals (`MemberOncallInterval`, with `Timestamp` start/end) and every enabled schedule they belong to |
...
`client.Members.MemberNotify` (`POST /member/notify`, `memberNotify`) sends an email to account members on behalf of the caller, and is callable only with a credential minted for an AI SRE session — any other credential, a plain `app_key` included, is rejected with `AccessDenied` (check it with `IsAccessDenied` / `ErrorCodeOf`). Request fields: `subject` (required, 1–200 characters) and `html` (required, the entire email body the recipients see; the server sanitizes it, stripping tags such as `script`, `style`, and `iframe` plus every `on*` event handler, keeping images only when `src` is `https` and links only for `http`, `https`, and `mailto`), while `person_ids` is optional (at most 20, no duplicates; omitted or empty sends to the caller only). Call it once with `dry_run` set to `true` before sending: nothing is queued, the response's `html` is the exact email recipients would get — so you can confirm the sanitizer kept everything the body depends on — and neither the hourly limit nor the per-turn duplicate check is consumed. Delivery is asynchronous, so `accepted` only means the email was queued; the response returns one entry per recipient in `recipients[]` (`person_id` plus `status`), where `status` is `accepted` or `skipped`, and a skipped entry carries a `reason` of `not_member`, `no_email`, `email_disabled`, `duplicate`, `rate_limited` (at most 20 emails per recipient per hour), or `send_failed`.
...
To call a `<type>.query` tool, build the request with `NewDatasourceQueryInvokeRequest(datasourceID, tool, params)` and pass the matching typed struct for that tool: `PrometheusQueryParams`, `MySQLQueryParams`, `PostgresQueryParams`, `OracleQueryParams`, `ClickHouseQueryParams`, `ElasticsearchQueryParams`, `LokiQueryParams`, `VictoriaLogsQueryParams`, `SLSQueryParams`, or `TencentCLSQueryParams`. All of them pair `expr` with an `execution` (`DatasourceQueryExecution`: `kind` is `instant`, `range`, or `window`, and `from_ms` / `to_ms` are Unix-millisecond `*int64` values; `range` additionally requires `max_data_points` and accepts `min_step_seconds` to raise the computed step). Optional fields are pointers — a nil pointer omits the key and lets the server apply its default. Query-tool `params` may not be empty: `NewDatasourceQueryInvokeRequest(..., nil)` returns an error, unlike the general "omitted means `{}`" rule. The execution mode is constrained by datasource type: Prometheus and Loki use `instant` or `range` (`instant` carries only `to_ms`); the SQL types (MySQL, Postgres, Oracle, ClickHouse, and the SQL-flavored Elasticsearch) plus SLS and Tencent CLS use `window`; VictoriaLogs uses `window` for raw logs and `instant` / `range` for stats queries. `limit` and `direction` on Loki, VictoriaLogs, SLS, and Tencent CLS bound raw-log retrieval only, and `SLSQueryParams` also requires `project` / `logstore` while `TencentCLSQueryParams` requires `region` / `topic_id` / `syntax`.
...
`client.DataSources.ReadPrometheusLabelValues(ctx, dataSourceID, labelName)` (`GET /monit/prometheus/api/v1/label/{label_name}/values`, `monit-prometheus-read-label-values`) lists every value of one label on a Prometheus-compatible data source through the monitoring proxy: the label name goes in the URL path (the SDK escapes it), and the target data source is selected by the `X-DSID` request header — the SDK sets it from the `dataSourceID` argument (a `0` ID or an empty `labelName` errors immediately), with the ID taken from `/monit/datasource/list` and required to be a Prometheus-compatible data source of the current account. It is the only method whose success body is not the Flashduty `{request_id, data}` envelope: the response decodes straight into `PrometheusLabelValuesResponse` (`data` carries the label-value strings when `status` is `success`; `errorType` and `error` are present when it is `error`). Failures before the data source is reached (`X-DSID` missing or invalid, data source absent, or not a Prometheus type) come back as `text/plain`, while the data source's own failures use its own JSON error shape; the SDK treats both alike and carries the raw body as-is on `*ErrorResponse.Message`. Supported deployments require monit-edge v0.35.0 or later.
...
Time fields in responses are no longer bare integers but self-describing `Timestamp` (Unix seconds) or `TimestampMilli` (milliseconds) types (types shared by requests and responses are the exception — see the warning below). They serialize to RFC3339 strings in the local time zone, so JSON, logs, and LLM-facing output are directly readable; the raw epoch is still one method call away.
...
<Note>
For **generated endpoints**, if you receive a non-2xx response with a non-JSON body, the SDK returns a plain `error`, not `*ErrorResponse`. This indicates that a gateway, load balancer, proxy, or other intermediary produced the response, usually because the request exceeded an intermediary timeout. Retry the request or split a long-running batch into smaller batches, and do not assume that `errors.As(err, &apiErr)` matches this error.

The hand-written `client.DataSources.ReadPrometheusLabelValues` method is the exception: it parses no envelope and instead puts a non-2xx response's raw body verbatim into `*ErrorResponse.Message`, treating the platform's `text/plain` 4xx/5xx bodies and the data source's own JSON error shape alike (429 is still promoted to `*RateLimitError`). `errors.As(err, &apiErr)` therefore does match there — `Code` is usually empty, so read `Message` and `Response.StatusCode` to identify the cause.
</Note>
