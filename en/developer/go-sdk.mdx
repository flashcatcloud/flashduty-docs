...
`<type>.query` query tools have dedicated request-parameter types on the Go side; build the request with `flashduty.NewDatasourceQueryInvokeRequest(datasourceID, tool, params)`, which marshals the params struct into `DatasourceToolInvokeRequest.Params` and returns an error when `params` is nil — query tools must not omit their params.

Optional fields on these types are always pointers: leave one nil and the key is omitted from the request entirely, so the server applies its default; an explicit `null` is rejected by the server, so there is no way to send one. To send a zero value explicitly (for example `powersql` false), take a pointer with `flashduty.Bool` / `flashduty.Int64` / `flashduty.String`.

| Tool | Params type | Type-specific fields |
|------|-------------|----------------------|
| `prometheus.query` | `flashduty.PrometheusQueryParams` | none |
| `mysql.query` | `flashduty.MySQLQueryParams` | none |
| `postgres.query` | `flashduty.PostgresQueryParams` | none |
| `oracle.query` | `flashduty.OracleQueryParams` | none |
| `clickhouse.query` | `flashduty.ClickHouseQueryParams` | none |
| `elasticsearch.query` | `flashduty.ElasticsearchQueryParams` | none |
| `loki.query` | `flashduty.LokiQueryParams` | `Limit`, `Direction` |
| `victorialogs.query` | `flashduty.VictoriaLogsQueryParams` | `Limit`, `Direction` |
| `sls.query` | `flashduty.SLSQueryParams` | `Project`, `Logstore`, `PowerSQL`, `Limit`, `Direction` |
| `tencent_cls.query` | `flashduty.TencentCLSQueryParams` | `Region`, `TopicID`, `Syntax`, `Limit`, `Direction` |

Every params type consists of `Expr` plus `Execution` (`flashduty.DatasourceQueryExecution`). The `Expr` dialect depends on the datasource type: PromQL for `prometheus`, LogQL for `loki`, LogsQL for `victorialogs`, a single read-only SQL statement for `mysql` / `postgres` / `oracle` / `clickhouse`, a single SQL statement for `elasticsearch` (ES DSL queries are not supported), and each platform's own search syntax for `sls` / `tencent_cls` (CLS's `Syntax` is `cql` or `lucene`).

```go
req, err := flashduty.NewDatasourceQueryInvokeRequest(
	12345, "prometheus.query",
	&flashduty.PrometheusQueryParams{
		Expr: `sum(rate(http_requests_total[5m]))`,
		Execution: flashduty.DatasourceQueryExecution{
			Kind: "instant",
			ToMS: flashduty.Int64(1779514631000),
		},
	},
)
if err != nil {
...
res, _, err := client.DataSources.ToolsInvoke(ctx, req)
if err != nil {
	return err
}

// res.Data is the tool-specific JSON; res.Summary is optional.
fmt.Println(string(res.Data))
```

`DatasourceQueryExecution.Kind` selects the evaluation mode:

| `kind` | Meaning | Time fields |
|--------|---------|-------------|
| `instant` | evaluate once at a single point in time | `ToMS` is the evaluation timestamp; `FromMS` is optional for Loki only (`instant` keeps the full time context so `$__auto` ranges resolve) and is rejected by Prometheus |
| `range` | evaluate a stepped series over `[FromMS, ToMS]` | `FromMS` and `ToMS` are required; `MaxDataPoints` is required and bounds the returned points (the server computes the step); `MinStepSeconds` is an optional positive lower bound for the computed step |
| `window` | evaluate one bounded window | `FromMS` and `ToMS` are required |

The accepted `kind` values differ by type: `prometheus` and `loki` accept `instant` / `range`; `mysql`, `postgres`, `oracle`, `clickhouse`, `elasticsearch`, `sls`, and `tencent_cls` accept `window` only; `victorialogs` uses `window` for raw logs (where `Limit` / `Direction` are allowed) and `instant` / `range` for stats queries, which require `FromMS` and reject `Limit` / `Direction`. `Limit` and `Direction` (`latest` / `earliest`) only bound raw-log results, not SQL rows or scanned data: 1–1000 for `loki`, `victorialogs`, and `tencent_cls`, and 1–100 for `sls`. `step_seconds` is a reserved field the SDK does not model — do not send it.

These types are hand-written and do not take part in code generation: `params` travels as raw JSON, so they are not reachable from any requestBody and the generator would treat them as response-side types (epoch millis would become `TimestampMilli` marshaling to RFC 3339 strings, and optional fields would lose `omitempty`).

`client.Applications` (RUM applications) adds a remote-config method group for reading and writing a RUM application's remote configuration: `RemoteConfigReadGet` (the live remote configuration and the version it is stored under), `RemoteConfigReadHistoryList` (the published configuration versions), `RemoteConfigReadPreview` (evaluate a draft configuration against a client context without publishing it), `RemoteConfigWriteUpdate` (publish a complete new version), and `RemoteConfigWriteHistoryRevert` (republish an earlier version's content as a new version).

`client.Schedules.ByPerson` (`POST /schedule/by-person`) returns a member's current and next on-call shifts plus every enabled schedule they participate in.
...
The alert-rule read and write methods on `client.AlertRules` are now the V2 ones: `ReadInfoV2` for detail, `WriteCreateV2` to create, and `WriteUpdateV2` to update — all take and return `*flashduty.AlertRuleV2` (which carries the v2 recovery and ending modes). `ReadList` still returns basic rule information only; call `ReadInfoV2` when you need the full configuration. The earlier non-V2 `ReadInfo` / `WriteCreate` / `WriteUpdate` no longer exist in the SDK, and `ReadCounterNode` / `ReadDstypes` were dropped as well, so rename accordingly when you upgrade.

Also note that optional array fields on request types are presence-based: leave the slice nil and the key is omitted, so the server keeps the stored value; pass an empty slice and `[]` is sent, clearing the field. `AlertRuleV2.InvestigationTargets` (at most 20 entries) is currently the only such field, so clearing the drill-down entries means passing an explicitly empty slice.
