...
flashduty incident comment <id> [<id2>...] # Append a comment to incident timelines (up to 100 incidents per call)
flashduty incident feed <id>             # View incident activity (paginated timeline)
```
...
`incident comment` appends the same timeline comment to one or more incidents (up to 100 per call). The comment text comes from `--comment-file` (required); a value of `-` reads from stdin, and the content is not shell-parsed — backticks, `$(...)`, and quotes are delivered to the API verbatim. Leading and trailing whitespace is trimmed before sending; after trimming the text must be non-empty and at most 1024 characters — the cap is counted in characters (Unicode code points), not bytes, so multi-byte text such as Chinese is not penalized; use `wc -m`, not `wc -c`, to measure. Add `--mute-reply` so the comment does not trigger Webhook reply behavior. After writing, the CLI re-reads each incident's timeline and confirms an entry exactly matching the sent text exists; if any incident cannot be confirmed, the whole command exits non-zero.

```bash
flashduty incident comment inc_123 --comment-file ./comment.txt
printf '%s' 'Rollback started' | flashduty incident comment inc_123 --comment-file -
```

`incident feed` shows the paginated activity (timeline entries) of a single incident; `--limit` defaults to 20 with a maximum of 100, and `--page` selects the page. Entry `type` values: `a_new` (created), `a_update` (updated by a new event), `a_merge` (merged), `a_comm` (comment), `a_m_silence` / `a_m_inhibit` (masked by a silence / inhibit rule), `a_close` (historical data only; new alerts no longer produce it — the close action is carried at the incident layer).

The assignment target `assigned_to` of `incident create` and `incident assign` (passed via `--data`) supports a `notify` object that overrides the notification channels for this assignment: `personal_channels` specifies the channels (e.g. `voice`, `sms`, `email`), `template_id` specifies the notification template ID (a MongoDB ObjectID), and `follow_preference` controls the channel source — when `false`, `personal_channels` is forced; when `true` or omitted, each responder's personal notification preferences are followed.

```bash
flashduty incident assign --data '{"incident_id":"inc_123","assigned_to":{"person_ids":[123],"type":"assign","notify":{"follow_preference":false,"personal_channels":["sms"]}}}'
```
...
In the generated `monit` rule commands (`rule-create` / `rule-update` / `rule-export`, etc.), the no-data check configuration `rule_configs.check_nodata` supports empty-result alerting: when `alert_on_empty_result` (boolean) is enabled, an alert fires when every query returns an empty result; `alert_on_empty_result_severity` sets that alert's severity (case-sensitive; one of `Critical`, `Warning`, `Info`; only takes effect when `alert_on_empty_result` is enabled).
...
### insight — Incident insight analytics

```bash
flashduty insight top-alerts [flags]      # Top alert sources grouped by a label dimension (--label check|resource, required)
flashduty insight incidents [flags]       # List incidents with performance metrics (--limit defaults to 20, max 100; --page for paging)
flashduty insight incident-export [flags] # Export the filtered incident analytics list as CSV
```

Both `top-alerts` (`--limit` is the Top K, default 10) and `incidents` support `--since` (default `7d`) / `--until` (default `now`).

`incident-export` writes the filtered incident list as CSV to stdout (redirect to a file to save it). The CSV header and formatted values use the request locale (falling back in turn to the member's, then the account's locale); `--start-time` / `--end-time` are Unix seconds. Additional filter flags: `--channel-ids`, `--incident-ids`, `--severities`, `--team-ids`, `--responder-ids`, `--query`, `--export-fields`, `--time-zone`, and more.

```bash
flashduty insight incident-export --start-time 1712000000 --end-time 1712604800 > incidents.csv
```

<Warning>
The export endpoint returns the CSV in one shot and caps the row count server-side, so an overly wide time window may truncate the results. After writing, `incident-export` cross-checks the total incident count under the same filters as an integrity check: when rows are missing, the partial CSV is still written, stderr prints `rows=N` (the number of data rows actually written), and the command exits non-zero noting how many were written versus expected — narrow the time window and retry in that case.
</Warning>
...
flashduty alert timeline <alert_id> # View an alert's timeline (--limit defaults to 20, max 100; --page for paging)
...
**Timestamp field convention**: in structured output (`json` / `jsonl` / `toon`), timestamp fields (such as `created_at`, `updated_at`, `start_time`, `end_time`, `ack_time`, `close_time`, `last_time`, `assigned_at`, etc.) are rendered as RFC3339 strings in the CLI process's **local timezone** — neither the Unix integers on the API wire nor UTC; unset timestamps render as `null` rather than `0`. Treat them as strings when parsing times with `jq`; if you need the raw epoch values, call the API directly.
...
With structured output (`json` / `toon`), a list command whose current page is not the last page prints a one-line pagination hint to **stderr** (`note: showing N of M total results (page X); raise --limit or use --page for the rest`), reminding you to raise `--limit` or page with `--page` for the remaining data; stdout stays clean, so piping into `jq` and similar tools is unaffected. Be careful not to mistake a partial result at the default page size for the full set.
...
When a list projection (`incident list`, `incident similar`, `alert-event list`) exceeds its limit, the CLI keeps the selected field names, shortens long strings, and marks them with `...`, while printing a one-line note to **stderr** naming the truncated fields and warning that matching or filtering on those fields will miss results — narrow `--fields` or `--limit` to get untruncated values. If a list still exceeds the limit and cannot be safely shortened, the command fails and names the largest field, asking you to reduce the fields or result count.
