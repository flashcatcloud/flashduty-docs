...
When a list projection (`incident list`, `incident similar`, `alert-event list`, `channel escalate-rule-list`) exceeds its 16 KiB limit, the CLI handles it in three tiers, with every note going to stderr and stdout left untouched:
