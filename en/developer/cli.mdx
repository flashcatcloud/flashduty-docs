...
| `--output-format` | Output format: `jsonl` (default, one session object per line, pipe straight to `jq`), `json` (full envelope, **not** subject to the 16 KiB list output bound; use `--limit` / `--since` yourself to fetch fewer rows — see "Output formats"), `toon` (compact) | `jsonl` |
...
Beyond the curated commands above, the CLI now provides **full coverage** of the Flashduty OpenAPI through a spec-driven code generator. The current OpenAPI contains **337 API operations**, and the CLI generates resource-organized commands for **334** of them; the remaining three — the streaming export `session-read-export` and the multipart uploads `mapping-data-write-upload` and `skill-write-upload` — are provided by hand-written commands (`session export`, `enrichment mapping-data-upload`, `safari skill-upload`). In addition to the On-call domain (incident, incident-trigger-subscription, change, channel, field, status-page, template, and more), it also covers:
...
Generated commands (the ones covering the OpenAPI through the code generator, such as `safari a2a-agent-list`, `alert event-list <alert-id>`, `insight incident-list`) apply a 16 KiB bound to **list-shaped** responses in `json`/`toon` mode. The reduction has three tiers; when the response is a **list envelope**, the CLI also writes a machine-readable marker into the payload:
...
**A reduced list envelope is rebuilt with the reduction written into the payload.** A list envelope is a response whose row array is named `items`, `docs`, or `list` and whose siblings are all scalars (`total`, `has_next_page`, `search_after_ctx`, …) — `safari a2a-agent-list`, `alert event-list <alert-id>`, and `insight incident-list` are examples. Once a reduction happens, the CLI rebuilds that envelope and adds fields to the payload, so a script sees the reduction without parsing stderr:
...
**Bare top-level arrays** (such as the generated `monit datasource-list`, or the curated `incident list` and `insight incidents`) have nowhere to carry the marker: a generated command's list-shaped output can be a bare array too, and it is reduced to the same 16 KiB bound, but the reduction is announced on stderr only — a script has to read stderr to see it.

A list response whose row array is not named `items` / `docs` / `list` (or that has a non-scalar sibling) is not recognized as a list envelope — the generated `safari session-list` and the curated `session list` both put their rows under `sessions[]`, next to `suggest_init` and `total`. Such responses are neither reduced nor given `truncated` / `emitted_rows`; the complete output prints untouched.
