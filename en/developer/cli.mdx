...
| `--args KEY=VALUE` | Additional arguments, repeatable (values must be strings). In raw mode (`loki` / `victorialogs`), `<ds-type>.start` and `<ds-type>.end` accept a relative duration (`15m`), `now`, a date/RFC3339 timestamp, or unix seconds/milliseconds; the CLI normalizes them to the unix-seconds form the datasource requires. `prometheus.time` / `loki.time` / `victorialogs.time` pin a query to an instant — see "Pin a query to an instant" below |

**Pin a query to an instant**: `--args` accepts three absolute-time arguments named after their datasource type. Each takes non-negative unix seconds and pins an instant query to that moment:

| Argument | Description |
|------|-------------|
| `prometheus.time` | The instant of a Prometheus instant query; only valid with `--ds-type prometheus` |
| `loki.time` | The instant of a Loki stats query; only valid with `--ds-type loki` when `loki.type` is empty or `stats` |
| `victorialogs.time` | The instant of a VictoriaLogs stats query; only valid with `--ds-type victorialogs` when `victorialogs.type` is empty or `stats` |

- Each argument only works for its own datasource type: passing `prometheus.time` with `--ds-type loki` fails with `prometheus.time is only supported for prometheus queries`.
- `loki` / `victorialogs` raw mode does not accept `.time`; keep using `<ds-type>.start` / `<ds-type>.end` for the window, otherwise the query fails with `... is only supported in stats mode; use <ds-type>.start/end for raw queries`.
- The value must be digits only, non-negative unix seconds: a sign, a fraction, or a relative duration such as `15m` is rejected (`... must be non-negative Unix integer seconds`), and the CLI does not normalize the format; the upper bound is `253402300799` (UTC 9999-12-31 23:59:59), above which you get `... is outside the supported Unix seconds range`.
- Future instants are rejected: WebAPI validates against its own clock and returns `<ds-type>.time must not be in the future`.
- When `.time` is present it is the query instant and any `--delay-seconds` value is ignored.
- If the target Edge is older than v0.74.0, WebAPI can only degrade it to a `--delay-seconds` offset (the result then includes transport time and the clock offset between the two sides) and accepts only instants within the last 86400 seconds; beyond that you get `target edge only supports absolute query time within the last 86400 seconds; upgrade to v0.74.0 or later`.
...
Beyond the curated commands above, the CLI now provides **full coverage** of the Flashduty OpenAPI through a spec-driven code generator. The current OpenAPI contains **333 API operations**, and the CLI generates resource-organized commands for **330** of them; the remaining operations (e.g. `session-read-export`) are provided by hand-written commands (`session export` / `safari session-export`). Both numbers track the version of the OpenAPI spec the CLI is generated from. In addition to the On-call domain (incident, incident-trigger-subscription, change, channel, field, status-page, template, and more), it also covers:

- **AI SRE (`safari`)**: a2a-agents, artifacts, automations, knowledge, mcp-servers, sessions, skills, and more
- **Alerting & noise reduction**: alert, alert-event, enrichment (alert-rules), route
