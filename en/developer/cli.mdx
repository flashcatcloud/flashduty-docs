...
Assignees can include the account's AI SRE: `work-item-create` accepts an `assignees` array via `--data` (at most 20 entries, each `{type, id?}` where `type` is `person` or `ai_sre`; an `ai_sre` entry omits `id`), handing the item directly to AI SRE for execution; `--assignee-ids` remains as the equivalent legacy alias (treated as all `person`) and is mutually exclusive with `assignees`. `work-item-assignees-reset` accepts the same `assignees` array (via `--data`) as a full replacement; an empty array clears all assignees. `work-item-list` gains `--assignee-type` (`person`/`ai_sre`) to filter by assignee kind. In responses, `assignee_ids` now contains only person assignees (AI SRE excluded), with new fields `assignees`, `agent_session_id`, and `agent_session_venue` (the executing AI SRE session and its venue, `web`/`im`).
...
`member list` supports `--query` (free-text search by name or email), `--role-id`, `--member-id` (return only the member with this ID), `--page`, `--limit`, `--orderby`, `--asc`.
...
| `--data` | Add `alerting` (notification), `tracing` (trace-link), and `repositories` (linked GitHub source repositories, ordered, at most 10 with the first as primary; each entry requires `repo` in `owner/name` form plus `subdir` for the app's directory inside the repo, empty saved as the repo root; on update the list is replaced wholesale, and an empty array removes all links) objects |
...
#### Remote configuration (application-remote-config-*)

Remote configuration holds per-application collection and privacy parameters; once published, an SDK picks it up on its next new session. For what each setting means and how publishing and activation work in the console, see [Application Management · Remote Configuration](/en/rum/quickstart/app-management#remote-configuration):

```bash
flashduty rum application-remote-config-get <application-id>             # Read the live configuration and its version
flashduty rum application-remote-config-history-list <application-id>    # List published history versions
flashduty rum application-remote-config-preview <application-id> [flags] # Preview the outcome for a simulated client, without publishing
flashduty rum application-remote-config-history-revert <application-id> --version <v>  # Revert to a history version
flashduty rum application-remote-config-update <application-id> [flags]  # Publish a complete new configuration
```

The complete configuration (a `config` object with `enabled`, `activation`, `default`, `rules`, `custom`, and `refresh_on_foreground`) goes in via `--data`; `--data` may also carry the whole request body, and positional arguments and typed flags override fields of the same name in it.

| Command | Key arguments |
|---------|---------------|
| `application-remote-config-get` | None (`<application-id>` is positional); returns the live configuration plus `version` and `updated_at`, where a `version` of 0 means the application has never been configured |
| `application-remote-config-history-list` | `--page` (0-based page index), `--limit` (default 20, max 100), `--orderby` (`updated_at`, the default, or `version`), `--asc` |
| `application-remote-config-preview` | `--env`, `--app-version`, and `--sdk` describe the simulated client; the draft `config` goes in via `--data` and, when omitted, the live configuration is previewed instead. Returns the index of the rule that decided the result (`hit_rule_index`, `-1` when only the default applied) and the `values` finally delivered |
| `application-remote-config-history-revert` | `--version` (required, the history version to republish, min 1); `--reason` (optional note, up to 255 characters — the console fills in `rolled back to vN` when it is empty) |
| `application-remote-config-update` | `config` in `--data` (required, a full replacement rather than a patch); `--reason` (optional note on why, up to 255 characters, stored verbatim) |

Both a publish and a revert allocate a new version number (a revert never reuses the old one). A change lands on new sessions: with `activation` set to `next_session` (the default), running sessions are untouched, while `immediate` makes a client end its current session as soon as the change arrives so a new session starts under the new configuration; publishing `enabled: false` turns configuration delivery off, and clients fall back to their SDK initialization settings.
...
Query tools and diagnostic tools sit behind different Edge version gates: query tools submit through the Explore execution path and require **all** currently online routable Edge sessions in the cluster to support the Explore query protocol **v0.68.0**, while diagnostic tools submit through the base invoke protocol and require **v0.71.0** (see `monit datasource-tools-invoke` below). When the versions do not qualify, the command fails with an error code: `edge_upgrade_required` (Edge too old), `mixed_edge_versions` (mixed versions in the cluster, with only some sessions qualifying), `edge_version_unknown` (an Edge session reports a version the platform cannot recognize), or `no_active_edge` (no routable online Edge). On any of these, do not rotate Edges or fall back to the legacy entry point.
...
- Requires **all** currently online routable Edge sessions in the cluster to support the v0.71.0 base invoke protocol (individual tools may require a newer implementation); query tools (`<type>.query`) go through the Explore execution path instead and carry an additional floor of the Explore query protocol v0.68.0 — a cluster that only supports older versions fails the query tool too, with no fallback.
...
Errors are returned as-is; common codes: `edge_upgrade_required` (Edge too old), `mixed_edge_versions` (mixed Edge versions in the cluster), `edge_version_unknown` (an Edge session reports an unrecognized version; returned on the query-tool path), `no_active_edge` (no routable online Edge), `tool_not_supported` (tool unavailable), `invalid_request` (fix the parameters), `source_too_large` / `result_too_large` (narrow the request). On Edge version issues, do not rotate Edges or fall back to the legacy diagnose.
...
```bash
flashduty monit prometheus-api-v1-label-{label_name}-values <label_name> --data-source-id <datasource-id>
flashduty monit prometheus-api-v1-label-{label_name}-values job --data-source-id 12345   # List all values of the label job
```

| Argument | Description |
|----------|-------------|
| `<label_name>` | Positional argument (required, exactly one), such as `job` or `instance` |
| `--data-source-id` | Datasource ID (required, minimum 1), from `monit datasource-list`; must be a Prometheus-compatible datasource owned by the current account |

The response body is the **datasource's native Prometheus payload**, not the Flashduty `{request_id, error, data}` envelope: on success it is `{"status":"success","data":["api","db","worker"]}`. When the datasource itself rejects a query (an unknown label name, say), the command fails with a non-zero exit code and the error text is whatever the datasource (or the Monitors proxy) returned verbatim — Prometheus's own error looks like `{"status":"error","errorType":"bad_data","error":"unknown label name"}` and is unrelated to Flashduty's error-code scheme.

<Note>
This command only reads the value set of one label; to fetch time series from an expression or run richer PromQL queries, use `monit-query <datasource-id> --tool 'prometheus.query'` as described above.
</Note>
...
Generated commands (the ones covering the OpenAPI through the code generator, such as `safari session-list`, `monit datasource-list`, `safari a2a-agent-list`) apply a 16 KiB bound to **list-shaped** responses in `json`/`toon` mode. The reduction has three tiers, plus a machine-readable marker:
...
**A reduced list envelope is rebuilt with the reduction written into the payload.** A list envelope is a response whose row array is named `items`, `docs`, or `list` and whose siblings are all scalars (`total`, `has_next_page`, `search_after_ctx`, …) — `monit datasource-list` and `safari session-list` are examples. Once a reduction happens, the CLI rebuilds that envelope and adds fields to the payload, so a script sees the reduction without parsing stderr:

- **Rows withheld**: the payload carries `truncated: true` together with `emitted_rows: N` (N is the number of rows actually emitted). The envelope's `total`, `has_next_page`, and `search_after_ctx` still echo the page as the server returned it, so they **cannot** tell you whether you got everything — a page cut to 7 of 100 rows keeps the original pagination fields. To collect all rows, re-request with a `--limit` no larger than the rows you actually received, then follow the returned cursor.
- **Only long values clipped** (no rows withheld): the payload carries `truncated: true` alone, and stderr names the clipped fields. Re-requesting cannot restore them: narrow the filter, lower the rows per page, or switch to a curated command that supports field projections.

**Bare top-level arrays** (such as the curated `incident list` and `insight incidents`) have nowhere to carry the marker, so a reduction is announced on stderr only.

**Detail-shaped single objects** (e.g. `safari session-get`) are never bounded or shortened — a shortened id or status would pass for a real value, so these responses print untouched. Generated commands mostly expose no flag that narrows their output (the occasional `--fields` is a request-body write selector, not an output projection); for complete JSON lower `--limit` and page through with `--page` (e.g. `flashduty safari session-list --limit 20 --page 2`), or narrow the filter.
