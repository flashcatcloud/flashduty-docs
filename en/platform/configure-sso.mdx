...
## Configuration Entry and Saving

---

**Configuration entry**: **Platform Management → Single Sign-On**. After turning on the **Enable Single Sign-On** toggle, the page displays the following configuration sections inline:

| Section | Description |
| --- | --- |
| Protocol & Connection | Select the protocol (SAML 2.0 / OIDC / CAS / LDAP; LDAP is available only in private deployments) and fill in the corresponding connection parameters; LDAP also provides a **Connection Test** button |
| Member Matching | Configure the stable user ID field (`user_id`) and the email, username, and phone field mappings, which determine how identities returned by the identity provider map to Flashduty members |
| Sync Configuration | Shown only for the LDAP protocol; syncs roles and teams based on LDAP Groups |
| Login Policy | Three toggles: Create Account on Sign In, SSO-only login, and Disable editing for SSO members |
| Flashcat Service Provider Info | Shown after the configuration is saved; the addresses and parameters you need to enter at the identity provider — just copy them over |

After enabling and saving the configuration, a **Member Login URL** is also displayed under **Enable Single Sign-On** (not applicable to the LDAP protocol). Distribute this URL to your members so they can initiate single sign-on directly through it.

<Note>
All changes on this page (including the login policy toggles) take effect only after you click the **Save** button that slides out at the bottom of the page; click **Cancel** to discard your changes and restore the saved configuration. If you leave the page with unsaved changes, you will be prompted with "You have unsaved changes. Are you sure you want to leave?".
</Note>
...
**Configuration path**: Platform Management → Single Sign-On → Enable "Enable Single Sign-On" → Select the SAML2.0 protocol in the "Protocol & Connection" section
...
| Create Account on Sign In | Disabled by default; when enabled, new users get an account automatically on sign-in. When disabled, members must be invited before they can sign in |
| SSO-only login (`force_sso`) | Disabled by default. When on, every member of this account can sign in only via SSO; password and verification-code sign-in are rejected. See [SSO-only login](#sso-only-login) below |
...
**Configuration path**: Platform Management → Single Sign-On → Enable "Enable Single Sign-On" → Select the OIDC protocol in the "Protocol & Connection" section
...
**Configuration path**: Platform Management → Single Sign-On → Enable "Enable Single Sign-On" → Select the CAS protocol in the "Protocol & Connection" section
...
**Configuration path**: Platform Management → Single Sign-On → Enable "Enable Single Sign-On" → Select the LDAP protocol in the "Protocol & Connection" section
...
After configuring the LDAP connection information, you can click the **Connection Test** button below the LDAP parameters in the "Protocol & Connection" section to verify that Flashduty can successfully connect to your LDAP server. The system will attempt to establish a connection using the currently entered LDAP URL, BIND DN, and password, and return a success or failure result.
...
**Configuration path**: Platform Management → Single Sign-On → Select the LDAP protocol → "Sync Configuration" section
...
**Configuration path**: Platform Management → Single Sign-On → "Login Policy" section → **Members can only sign in via SSO**

| Item | Behavior |
| --- | --- |
| Field name | `force_sso` |
| Default | **Off** — the toggle is off by default the first time SSO is configured |
...
When you enable single sign-on, members automatically created through their first SSO login are marked as **external members**. You can enable the **Disable editing for SSO members** option in the "Login Policy" section (disabled by default). When enabled, these external members become read-only in Flashduty — you cannot modify their roles or delete them in Flashduty. All member management must be done through the identity provider.
