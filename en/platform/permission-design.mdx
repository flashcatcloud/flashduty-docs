...
<Note>
**Channel-exclusive integrations vs. shared integrations**: an integration created under a channel (its `ref_id` looks like `c_<channel_id>`) is a **channel-exclusive integration**; every other integration is a **shared integration**. The two follow different permission rules.
...
- Only an integration whose `ref_id` looks like `c_<channel_id>` counts as channel-exclusive: a shared integration opened from the integration detail with a `channel_id` query parameter is still judged as a shared integration.
</Note>
