...
In the member list, a warning icon appears next to unverified email addresses or phone numbers. Unverified contact methods cannot receive alert notifications. Please remind members to complete verification promptly. The list also shows each member's **Time Zone**: members without a personal time zone follow the organization time zone — see [Organization Information](/en/platform/organization-info) and [Personal Settings](/en/on-call/configuration/personal-settings) for details. When a member's **name is empty (or whitespace only)**, the name shown in the list and the avatar initial fall back in the order **email prefix** (the part before `@`) → **phone number**: a member auto-created through SSO, or invited through the API without a name, is displayed as the email prefix. This fallback is display-only — it is **never written to the member profile**, and as soon as a name is set on the profile the list and avatar show that name.
...
<Tabs>
  <Tab title="Invite link / QR code">
    ...
    - **Copying the QR code requires HTTPS access**: browsers only allow images to be written to the clipboard in a secure context (HTTPS). When the console is reached over http or an intranet address, **Copy QR code** falls back to downloading `invite-qrcode.png` with a notice that the environment does not support copying images, and the two buttons swap emphasis — **Save QR code** becomes the primary action
    - A second confirmation appears only when the built-in **Account.Admin** role is newly added to the default roles (selecting it together with other roles prompts as well) — everyone who holds the link gets that role. **Custom roles never trigger the confirmation**, even one carrying admin-level permissions such as member management, so check the permission points yourself before granting it. Removing Account.Admin, or changing a selection that already contains it, does not prompt; cancelling the confirmation reverts to the previously configured default roles
    ...
    - A previously deleted member who opens the link again and signs in with the email or phone still on their row is **restored under the same member ID** and rejoins the organization (no new member is created); the row **keeps the roles it already holds**, and the organization's configured default roles apply only to a row that has none
    - New members (the organization holds no row for them) get only the configured default roles and are **not added to any team**
...
<Warning>
  - A deleted member cannot be restored from the console, so confirm the member no longer needs access to the organization before deleting
  - After a member is deleted, their historical data will not be deleted
  - Deletion is a **soft delete**: the member's row stays in the organization, so a deleted member can come back on their own — if they open the organization's **invite link** or **invite QR code** again and sign in with the email or phone still on that row, they are **restored under the same member ID** (no new member is created) and the row **keeps the roles it already holds**; the invite's configured default roles apply only to a row that has none
</Warning>
