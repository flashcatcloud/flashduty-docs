---
title: "RUM Explorer Overview"
sidebarTitle: "Feature Overview"
...
<Tabs>
  ...
  <Tab title="View Details">
    ...
    For Electron sessions, the event list marks main-process events with a "Main Process" badge (unbadged rows are renderer events), and the event detail panel adds a virtual `process` attribute (`main` / `renderer`) in the Other attribute group to tell the two runtimes apart.

    For Resource events, the detail panel provides a dedicated **Resource** attribute group showing key resource-loading attributes:

    | Attribute | Description |
    | --- | --- |
    | `resource_protocol` | Protocol of the resource request (e.g. `h2`, `http/1.1`) |
    | `resource_delivery_type` | How the resource was delivered (e.g. from `cache` or over the network) |
    | `resource_render_blocking_status` | Whether the resource blocks rendering (`blocking` / `non-blocking`) |
    | `resource_size` / `resource_transfer_size` | Decoded resource size / network transfer size |
    | `resource_provider_name` / `resource_provider_type` | Name and type of the resource provider (e.g. third-party CDN, font services) |
    | `client_time` | Client-side collection time |
...
| **Electron** | Renderer views show Web metrics; main-process views have no page performance metrics and the panel shows an explanation instead |
