...
The Explorer supports browsing and analyzing RUM data by the following event types:

| Event Type | Description |
|------------|-------------|
| **Sessions** | {/* REVIEW: User sessions, containing the complete user visit lifecycle */} |
| **Views** | {/* REVIEW: Page views, recording page loads and performance metrics */} |
| **Resources** | {/* REVIEW: Resource loading, recording JS, CSS, image, and other resource loading */} |
| **Actions** | {/* REVIEW: User actions, recording interactions such as clicks and scrolls */} |
| **Errors** | {/* REVIEW: Error events, recording JavaScript exceptions and network errors */} |
| **Long Tasks** | {/* REVIEW: Long tasks, recording tasks that block the main thread for over 50ms */} |

## Faceted Filter Panel

{/* REVIEW: Feature overview — The faceted filter panel on the left side of the Explorer, supporting quick data filtering by multiple dimensions */}

The Explorer provides a Faceted Filter Panel on the left side, organized by the following groups:

| Group | Description |
|-------|-------------|
| **BASIC** | {/* REVIEW: Basic attributes such as application, environment, version, etc. */} |
| **SESSION** | {/* REVIEW: Session attributes such as session ID, user information, etc. */} |
| **VIEW** | {/* REVIEW: Page view attributes such as page URL, load time, etc. */} |
| **RESOURCE** | {/* REVIEW: Resource attributes such as resource type, loading duration, etc. */} |
| **ACTION** | {/* REVIEW: User action attributes such as action type, target element, etc. */} |
| **ERROR** | {/* REVIEW: Error attributes such as error type, error message, etc. */} |
| **LONG TASK** | {/* REVIEW: Long task attributes such as task duration, etc. */} |

The filter panel supports both multi-select filtering and range filtering modes, helping you quickly narrow down the data scope.

## Event Histogram

{/* REVIEW: Please add a screenshot and description of the event histogram */}

The top of the Explorer displays an Event Histogram that visually shows the distribution of events within the selected time range. You can use the histogram to quickly identify peak event periods.
{/* END REVIEW */}
