...
A single upload can therefore only match one fixed installation location, which does not cover real distribution scenarios. If you need stack resolution in distributed builds, **you can load renderer pages over a custom protocol** (register `app://` with `protocol.handle()` and call `loadURL('app://index.html')`), which keeps stack paths stable across machines.

This recommendation exists **only to stabilize source map paths**. The bridge itself works fine under `file://` and needs no change to how pages are loaded.
</Warning>
