...
`init()` returns `false` when validation fails. The SDK prints the reason in the main-process console and does not start collection.
...
Use paired APIs to record critical workflows such as checkout, synchronization, or file upload:
...
startOperation('workspace_sync');

try {
  await syncWorkspace();
  succeedOperation('workspace_sync');
} catch (error) {
  failOperation('workspace_sync', 'error');
}
```

Use `operationKey` to distinguish concurrent operations with the same name. These APIs are available only in the main-process SDK and are in preview; their signatures may change before general availability.
