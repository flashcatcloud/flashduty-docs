...
| Network requests | `http`, `https`, `fetch`; `net.fetch` needs Electron 37 or later | `fetch`, XHR, and static resources |
...
<Note>
Electron's own `net` module is traced only on **Electron 37 and later**. On Electron 30 to 36, requests made with `net.request` / `net.fetch` produce no resource events; `http`, `https`, and the global `fetch` are unaffected and are collected on every supported version.
</Note>
