...
3. Select version rule:
   - **Recommended**: Select **Up to Next Major Version** and enter the latest version number (e.g., `0.6.0`)
...
```ruby Podfile
pod 'FlashcatCore', '~> 0.6.0'    # Core SDK (required)
pod 'FlashcatRUM', '~> 0.6.0'     # RUM feature (required)
pod 'FlashcatTrace', '~> 0.6.0'   # Trace feature (required for Trace feature or manual spans)

# Optional modules
pod 'FlashcatWebViewTracking', '~> 0.6.0'   # WebView tracking
pod 'FlashcatCrashReporting', '~> 0.6.0'    # Crash reporting
```
