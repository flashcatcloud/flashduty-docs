...
<Info>
Flashcat iOS RUM SDK automatically generates events with relevant metrics and attributes. Each RUM event includes default attributes (such as `view.url`, `device.type`, `geo.country`) as well as event-specific metrics and attributes (such as `view.time_spent`, `resource.method`).
</Info>
...
Flashcat iOS RUM SDK automatically attaches default attributes to all events, helping you understand user devices, network status, and app context.
