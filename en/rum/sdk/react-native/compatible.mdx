...
<Tip>
JS source maps are matched by **service + version + bundle file name** and do not depend on the Metro debug ID plugin. Keep the SDK's `serviceName` / version consistent with the upload parameters, and make the upload part of every release build.

Native crashes additionally require `nativeCrashReportEnabled` to be set to `true` (off by default). Both kinds of symbol files are covered in <a href="/en/rum/sdk/react-native/advanced-config">Advanced configuration</a>.
</Tip>
