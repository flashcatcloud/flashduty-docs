...
description: "Configure sampling, tracking consent, event filtering, distributed tracing, WebView tracking, and source map upload for the React Native RUM SDK"
keywords: ["RUM", "React Native SDK", "advanced configuration", "sampling", "tracking consent", "source map"]
---
...
For on-premises deployments, override the RUM, Logs, and Trace intake endpoints separately through `customEndpoints`:
...
<Note>
Every JS code change produces a new bundle, so **source map upload must be part of every release build**; otherwise stack traces for that version stay minified. Native crash stacks are reported by the native SDK and are symbolicated the same way as with the <a href="/en/rum/sdk/android/advanced-config">Android</a> / <a href="/en/rum/sdk/ios/advanced-config">iOS</a> SDKs.
</Note>
