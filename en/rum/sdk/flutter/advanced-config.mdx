...
<Note>
For private deployments, prefix the command with the `FLASHCAT_SOURCEMAP_INTAKE_URL` environment variable to upload symbol files to a custom intake (protocol + domain, no path), e.g. `FLASHCAT_SOURCEMAP_INTAKE_URL=https://rum.example.com`. When unset, uploads go to Flashcat SaaS.
</Note>
