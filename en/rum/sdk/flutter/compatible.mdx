...
| Automatic resources | Supported (requires companion package) | Through `enableHttpTracking()` from `flashcat_tracking_http_client`; 0.1.0 has a known issue, see the limits below |
...
| Known resource-collection issue | In `flashcat_tracking_http_client` 0.1.0, a request whose response body is consumed with `drain()` or `asFuture()` produces no resource event — both replace the `onDone` / `onError` handlers the SDK registers. Use `toList()`, `transform()`, or a `listen()` with your own `onDone` to work around it |
