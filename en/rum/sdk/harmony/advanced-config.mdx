...
The failure is silent: `FLASHCAT_UPLOAD` reads as unset and the task quietly does nothing, or a stale `FLASHCAT_VERSION` uploads the symbols under the wrong version number. Values written directly into `hvigorfile.ts` are not affected.
</Warning>
