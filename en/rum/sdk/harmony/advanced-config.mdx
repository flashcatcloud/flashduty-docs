...
<Note>
Keys in `extraInfo` are expanded into the `usr` object. For example, `extraInfo.role` becomes `usr.role`. Do not reuse standard field names such as `id`, `name`, or `email` inside `extraInfo`.
</Note>
