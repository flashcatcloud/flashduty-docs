...
<Note>
Keys in `extraInfo` are expanded into the `usr` object. For example, `extraInfo.plan` becomes `usr.plan`. Do not reuse standard field names such as `id`, `name`, or `email` inside `extraInfo`.
</Note>
