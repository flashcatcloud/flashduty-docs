...
Link Integration lets you associate RUM events with external systems, such as distributed tracing platforms, log search, object storage that stores crash log packages, or internal troubleshooting systems. After you configure links, RUM generates jump links from the event type and event context. The related links appear in two places: **Explorer · Event Details** (the **Related Links** button in the header opens the link list) and **Error Tracking · Error Details** (link cards below the error sample).
