...
## Geo data and IP privacy controls

<!-- REVIEW: Add overview — explain geo data collection controls, privacy compliance use cases (e.g. GDPR) -->
...
|-------|------|---------|-------------|
| `no_geo` | boolean | `false` | <!-- REVIEW: Disable geo-location data collection; when enabled, geo_country, geo_city etc. are no longer collected --> |
| `no_ip` | boolean | `false` | <!-- REVIEW: Disable client IP collection; when enabled, user IP addresses are no longer recorded --> |

<!-- REVIEW: Please add the following:
- Detailed explanation of geo fields (geo_country, geo_city, geo_region, etc.)
- Relationship between no_geo and no_ip (does disabling IP automatically disable geo?)
- Privacy compliance recommendations (recommended settings for GDPR scenarios)
- Default state explanation (collection enabled by default)
-->

### Usage

<!-- REVIEW: Add step-by-step instructions, including toggle locations in app creation and app configuration -->
<!-- END REVIEW -->
