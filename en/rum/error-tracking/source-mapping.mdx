...
The source code management page provides different upload panels and metadata fields depending on the application type (Web, iOS, Android):

<Tabs>
  <Tab title="Web (Browser)">
    The Web platform uses standard JavaScript SourceMap files (`.map`), uploaded via Flashduty CLI.

    {/* REVIEW: Please add a screenshot and description of the Web upload interface */}
  </Tab>
  <Tab title="iOS">
    The iOS platform requires uploading dSYM files with the following metadata:

    | Field | Description |
    |-------|-------------|
    | **UUID** | {/* REVIEW: UUID of the dSYM file, used to match crash stacks */} |

    {/* REVIEW: Please add iOS upload panel screenshot and operation steps */}
  </Tab>
  <Tab title="Android">
    The Android platform requires uploading ProGuard mapping or R8 mapping files with the following metadata:

    | Field | Description |
    |-------|-------------|
    | **Build ID** | {/* REVIEW: Build identifier */} |
    | **Variant** | {/* REVIEW: Build variant (e.g., release, debug) */} |
    | **Version Code** | {/* REVIEW: Application version code */} |

    {/* REVIEW: Please add Android upload panel screenshot and operation steps */}
  </Tab>
</Tabs>

### Source Map List Filtering

{/* REVIEW: Please supplement the source map list filtering and sorting feature description */}

On the source code management page, you can filter and sort uploaded source map files by the following criteria:

- Filter by service name
- Filter by version number
- Filter by build ID (Android)
- Filter by build variant (Android)
- Sort by upload time
{/* END REVIEW */}
