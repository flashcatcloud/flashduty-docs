...
RUM Error Tracking can use `sourcemap` files, Android mapping files, and iOS dSYM files to restore error stacks and show original source code locations in error details for precise debugging.
...
<Steps>
  ...
  <Step title="Associate Symbol Files">
    When file paths, line numbers, or address information in the error stack match uploaded `sourcemap` files, Android mapping files, or iOS dSYM files, Flashduty automatically maps minified, obfuscated, or compiled error locations back to the original source code.
