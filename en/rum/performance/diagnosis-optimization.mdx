...
<Steps>
  <Step title="Metric Selection">
    ...
    1. Click on a metric card to select the metric to analyze
    ...
    The metric cards adapt to the application's platform, so they always show the metrics that platform actually reports:

    | Platform | Metric cards |
    |----------|--------------|
    | **Browser (Web) / Electron (renderer)** | `LCP`, `FCP`, `INP`, `CLS` |
    | **WeChat Mini Program** | `first_render` (First Render), `lcp`, `fcp`, `onload_to_onshow`, `onshow_to_onready` |

    <Note>
    The platform is determined by the application's own type, so no manual switching is required. `first_render`, `onload_to_onshow`, and `onshow_to_onready` come from the WeChat Mini Program SDK: `first_render` is the time to first render, while `onload_to_onshow` and `onshow_to_onready` correspond to the two segments of the mini program page lifecycle and help you troubleshoot slow cold starts. After you switch applications, the metric cards refresh to the new platform's metric set, and the tree map (metric view) metric dropdown likewise lists only the metrics available for the current application platform.
    </Note>
  </Step>
...
<Tabs>
  <Tab title="Optimize Resource Loading">
    ...
    | AVG DURATION | Average loading duration, with a color bar positioned at real time offsets showing each loading phase: redirect, Service Worker, queueing, DNS resolution, connection establishment (including TLS), time to first byte, and download |

    <Tip>
    Hover over the duration color bar to view the specific timing values for each phase. When one phase accounts for 50% or more of the duration, the card explains the dominant phase and suggests optimizations (TLS is nested inside connection establishment and never becomes the dominant phase alone), helping you determine whether slow loading is caused by network connection, server response, or resource size. If [Link Integration - Built-in Tracing](/en/rum/quickstart/app-management#link-integration) is configured and the resource carries a trace_id, you can drill down to the corresponding trace. When the browser does not expose phase timings (commonly caused by cross-origin responses missing the `Timing-Allow-Origin` header), the bar area shows an "phase timings unavailable" hint.
    </Tip>
  </Tab>
