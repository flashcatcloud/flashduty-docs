...
<Tip>
In the configured jump link, RUM automatically replaces `${trace_id}` with the actual trace_id from the resource event. Built-in Tracing appears only when a resource event contains `trace_id`.
</Tip>
