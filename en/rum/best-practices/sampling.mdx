...
flashcatRum.init({
  ...
  beforeSend: (event) => {
    ...
    return bucketOf(event.session.id) < DETAIL_SAMPLE_RATE;
  },
});
```
...
- **Session Replay is a separate, independent draw.** `sessionReplaySampleRate` is not aligned with the bucket above, so the two are not guaranteed to land on the same sessions. If you need replay and full data present together, make the decision once in your own code (see [business-defined custom sampling](#best-practice-2-business-defined-custom-sampling)).
