...
A common requirement is "cut data volume to 20%, but never miss an error". Setting `sessionSampleRate` to 20 will not do it: sampling works per session, and a session that loses the draw reports nothing at all — errors included (rule 1). You would lose roughly four fifths of your errors along with the volume.
...
- **Session Replay needs one explicit alignment step.** This recipe sets `sessionSampleRate` to 100, so the SDK assumes every session is collected in full and draws replay across all of them. Skip the step in [the section above](#aligning-session-replay-with-the-bucket) and replay lands on sessions outside the bucket — you get the recording, but the session behind it holds no behavior data.
