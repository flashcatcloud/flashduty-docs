...
<Tabs>
  ...
  <Tab title="Resource">
    ...
    | `resource_protocol` | string | Network protocol used to fetch the resource (e.g., `h2`, `h3`, `http/1.1`) |
    | `resource_delivery_type` | string | How the resource was delivered (`cache`, `navigational-prefetch`, `other`) |
    | `resource_render_blocking_status` | string | Whether the resource blocks rendering (`blocking`, `non-blocking`) |
    | `resource_transfer_size` | number | Actual transferred size in bytes, which may be smaller than the full resource size (e.g., when served from cache or compressed) |
  </Tab>
  <Tab title="Long Task">
    ### Long Task Metrics
